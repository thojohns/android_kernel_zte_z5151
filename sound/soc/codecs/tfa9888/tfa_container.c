...
static struct nxpTfaVolumeStepRegisterInfo *tfaContGetRegForVstep(
	nxpTfaVolumeStepMax2File_t *vp, int idx)
{
	int i, j, nrMessage;
...
int vstep_idx, int vstep_msg_idx)
{
	...
	case presetHdr:
		...
		break;
	case equalizerHd:
		err = tfa_cont_write_filterbank(dev_idx,
...
enum Tfa98xx_Error tfaRunWriteFilter(Tfa98xx_handle_t dev, nxpTfaContBiquad_t *bq)
{
	...
	for (i = 0; i < runs; i++) {
		...
		if (tfa98xx_dev_family(dev) == 2) {
			error = tfa_dsp_cmd_id_write(dev, MODULE_FRAMEWORK, FW_PAR_ID_SET_MEMORY,
				sizeof(data), data);
		} else {
			error = tfa_dsp_cmd_id_write(dev, MODULE_FRAMEWORK, 4 /* param */,
				sizeof(data), data);
		}
	}
...
enum Tfa98xx_Error tfaContWriteProfile(int dev_idx, int prof_idx, int vstep_idx)
{
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		...
		tfa98xx_dump_register(getHandle(dev_idx), 2,
				"tfaContWriteProfile  after performed TFA_SET_BF_VOLATILE(dev_idx, SBSL, 0);");
			...
			do {
				if (dev_family == 2) {
					...
					*/
					...
					if (Tfa98xx_Error_Ok != err)
						pr_err("tfaContWriteProfile tfa98xx_dsp_system_stable err(%d)\n", err);
	...
	for (i = 0; i < prof->length; i++) {
		...
		default:
			tfa98xx_dump_register(getHandle(dev_idx), 2,
				"tfaContWriteProfile  before tfaContWriteItem in switching....");
			err = tfaContWriteItem(dev_idx,  &prof->list[i]);
			tfa98xx_dump_register(getHandle(dev_idx), 2,
				"tfaContWriteProfile  after  tfaContWriteItem in switching....");
			if (err != Tfa98xx_Error_Ok) {
	...
	if (prof->group != previous_prof->group || prof->group == 0) {
		...
		if (dev_family == 2) {
			...
			if (strnstr(tfaContProfileName(dev_idx, prof_idx), ".standby",
		...
		if (tfa98xx_cnt_verbose && (dev_family == 2))
			err = show_current_state(dev_idx);
