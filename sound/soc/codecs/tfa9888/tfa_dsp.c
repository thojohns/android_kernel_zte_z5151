	...
	for (dev = 0; dev < devcount; dev++) {
		...
		if ((TFA_GET_BF(dev, CFE) != 0)) {
			if (vstep[dev] != tfaContGetCurrentVstep(dev) && vstep[dev] != -1) {
				...
				pr_info("tfa_start()	tfaContWriteFilesVstep err(%d)\n", err);
				if (err != Tfa98xx_Error_Ok)
					goto error_exit;
			}
			...
			pr_info("tfa_start()	tfa_set_filters err(%d)\n", err);
		...
		if (tfa98xx_dev_family(dev) == 2) {/* Max2 */
			...
			do {
				if (strnstr(tfaContProfileName(dev, stop_profile), ".standby",
					strlen(tfaContProfileName(dev, stop_profile))) != NULL) {
