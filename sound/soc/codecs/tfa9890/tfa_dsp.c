...
enum Tfa98xx_Error tfaRunSpeakerBoost(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	pr_info("tfaRunSpeakerBoost enter, handle %d\n", handle);
...
enum Tfa98xx_Error tfaRunSpeakerStartup(Tfa98xx_handle_t handle, int force, int profile)
{
	...
	pr_info("tfaRunSpeakerStartup: force(%d), profile(%d)", force, profile);
...
enum Tfa98xx_Error tfaRunSpeakerCalibration(Tfa98xx_handle_t handle, int profile)
{
	...
	if (err == Tfa98xx_Error_Ok) {
		...
		if (spkr_count == 1) {
			...
			pr_info(" Prim:%d mOhms, Sec:%d mOhms\n",
						handles_local[handle].mohm[0],
						handles_local[handle].mohm[1]);
		}
	}
...
enum tfa_error tfa_start(int next_profile, int *vstep, int dev)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	...
	if (devcount < 1) {
		...
		pr_info("Starting device(%d) [%s]\n", dev, tfaContDeviceName(dev));
		pr_info("device(%d) active_profile:%s, next_profile:%s\n", dev,
				tfaContProfileName(dev, active_profile),
				tfaContProfileName(dev, next_profile));
		 ...
		 */
		if (tfa98xx_dev_family(dev) == 2) {
			/* If we are in state 0 or 1 we can skip setting SBSL and RST,
			     but also filters and vstep (since there is no clk)
...
enum tfa_error tfa_stop(int dev)
{
	enum Tfa98xx_Error err = Tfa98xx_Error_Ok;
	int devcount = tfa98xx_cnt_max_device();
