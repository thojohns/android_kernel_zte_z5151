...
static int tfa98xx_query_dsp_handle_index(struct tfa98xx *tfa98xx)
{
	int i, handle = -1;

	for (i = 0; i < tfa98xx_cnt_max_device(); i++) {
		if (tfa98xx_devices[i] == tfa98xx) {
			handle = i;
			break;
		}
	}
...
static enum tfa_error tfa98xx_tfa_start(struct tfa98xx *tfa98xx, int next_profile, int *vstep)
{
	...
	dev = tfa98xx_query_dsp_handle_index(tfa98xx);

	err = tfa_start(next_profile, vstep, dev);
...
static int tfa98xx_register_inputdev(struct tfa98xx *tfa98xx)
{
	...
	dev_info(tfa98xx->codec->dev, "Input device for tap-detection registered: %s\n",
		input->name);
	tfa98xx->input = input;
	return 0;
...
static int tfa98xx_dbgfs_otc_set(void *data, u64 val)
{
	...
	if (otc->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			otc->wr_value = val;
			otc->triggered = true;
			pr_info("Deferring write to OTC (%d)\n", otc->wr_value);
			return 0;
		}
	}
...
static int tfa98xx_dbgfs_mtpex_set(void *data, u64 val)
{
	...
	if (mtpex->deferrable) {
		if (err != Tfa98xx_Error_Ok && err != Tfa98xx_Error_NoClock) {
			...
			pr_info("Deferring write to MTPEX (%d)\n", mtpex->wr_value);
			return 0;
		}
	}
...
static int tfa98xx_create_controls(struct tfa98xx *tfa98xx)
{
	...
	pr_info("tfa98xx_dev_family: %d\n", tfa98xx_dev_family(tfa98xx->handle));
	...
	for (prof = 0; prof < nprof; prof++) {
		...
		if (is_profile_in_list(bprofile->basename, bprofile->len) == 0 &&
			...
			if (tfacont_get_max_vstep(tfa98xx->handle, prof)) {
				...
				if (!name) {
					devm_kfree(tfa98xx->codec->dev, tfa98xx_controls);
					devm_kfree(tfa98xx->codec->dev, name);
					devm_kfree(tfa98xx->codec->dev, bprofile);
					return -ENOMEM;
				}
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	dev_info(&tfa98xx->i2c->dev, "tfa98xx_dsp_init\n");
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		pr_info("no_start parameter set no tfa_start or tfa_stop, returning\n");
		return 0;
	}
...
static int tfa98xx_probe(struct snd_soc_codec *codec)
{
	...
	dev_info(codec->dev, "tfa98xx_probe addr %#X\n", tfa98xx->i2c->addr);
