...
error_exit:
	...
	for( dev=0; dev < devcount; dev++) {
	    /* NXP patch resolve Hissing sound problem when startup 2016.12.4 */
        /* for TFA9897, if current profile isn't bypass, we will restart DSP here */
        if (((TFA_GET_BF(dev, REV) & 0xff) == 0x97) && \
            (strstr(tfaContProfileName(dev, next_profile), "bypass") == NULL)){
            pr_err("tfa98xx----2-----%s:TFA_SET_BF_VOLATILE\n", __func__);
            TFA_SET_BF_VOLATILE(dev, CFE, 0);
            msleep_interruptible(5);
            TFA_SET_BF_VOLATILE(dev, CFE, 1);
            msleep_interruptible(5);
        }
