...
static void bq2429x_set_ship_mode_enable(struct bq2429x *bq, bool enable)
{
...
static int bq2429x_usb_phy_init(struct device *dev, struct bq2429x *bq)
{
	int rc;
...
static int bq2429x_usb_phy_ctrl(struct bq2429x *bq, bool enable)
{
	int rc;
				...
				union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		bq2429x_get_batt_property(bq, psp, val);
		break;
				       ...
				       const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (val->intval == 0) {
			...
			bq2429x_usb_phy_ctrl(bq, false);
		} else {
			bq2429x_usb_phy_ctrl(bq, true);
		}
		power_supply_changed(&bq->batt_psy);
	...
	case POWER_SUPPLY_PROP_CAPACITY:
		...
		break;
	case POWER_SUPPLY_PROP_SHIPMODE:
		bq2429x_set_ship_mode_enable(bq, val->intval);
		break;
	case POWER_SUPPLY_PROP_SYSTEM_TEMP_LEVEL:
...
static int smb_chg_proc_show(struct seq_file *seq, void *v)
{
	...
	ret = seq_puts(seq, "ti chg_20170913\n");
...
static int smbchg_smoothing_battery_prop(void)
{
	...
	*/
	pr_notice("g_smooth_prop.ui_soc = %d\n", g_smooth_prop.ui_soc);
	if (g_smooth_prop.chg_present == 1) {
		if (g_smooth_prop.battery_status == POWER_SUPPLY_STATUS_FULL) {
			...
			} else if ((g_smooth_prop.ui_soc - g_smooth_prop.current_soc) > 2) {
		...
		cancel_delayed_work(&g_smooth_prop.battery_smooth_up);
		 if ((g_smooth_prop.ui_soc == 100) &&
			...
			g_smooth_prop.soc_direction = 0;
			if (g_smooth_prop.current_soc > 90)
				schedule_delayed_work(&g_smooth_prop.battery_smooth_down, msecs_to_jiffies(60000));
	...
	if (g_smooth_prop.fg_soc <= 0) {
#ifdef CONFIG_ZTE_TI_KEEP_SOC_BATTVOLT_ABOVE_3400
		if (g_smooth_prop.avg_volt > 3400000) {
#else
		if (g_smooth_prop.avg_volt > 3400000 && g_smooth_prop.avg_temp > 50) {
#endif
...
static int bq2429x_monitor(struct bq2429x *bq)
{
	...
	if (g_smooth_prop.current_boot_sec < 30) {
		...
		if (soc <= 0) {
			if (voltage_now > 3400000 && g_smooth_prop.avg_temp > 50) {
				g_smooth_prop.ui_soc = 1;
			} else {
				g_smooth_prop.ui_soc = 0;
			}
...
out:
	if (current_boot_sec - g_smooth_prop.battinfo_print_sec > 16) {
		...
		pr_notice("avgt=%d, T=%d, presoc=%d, soc=%d",
			g_smooth_prop.avg_temp, temp, g_smooth_prop.pre_soc, soc);
		pr_notice("uisoc=%d, chg=%d, Pret=%d, Therm=%d, BatSta=%d,\n",
...
#ifdef CONFIG_ZTE_TI_NO_CANCEL_CHGDET_WORK
	/*For power monitor fake battery unable mount adb, because power monitor canot sink current*/
	pr_info("do not cancel detection work.\n");
#else
	cancel_delayed_work_sync(&bq->usb_type_detection_work);
#endif
					...
					const struct i2c_device_id *id)
{
	...
	bq2429x_usb_phy_init(&client->dev, bq);
