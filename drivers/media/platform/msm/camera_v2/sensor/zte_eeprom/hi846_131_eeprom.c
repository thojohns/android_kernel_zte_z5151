...
#define HI846_131_MODULE_INFO_GOUP_OFF_SIZE 0x11
...
#define HI846_131_AWB_GOUP_OFF_SIZE 0x1E
...
{
	uint16_t sensor_module_id = e_ctrl->cal_data.mapdata[HI846_131_ID_ADDR];
...
static int  hi846_131_get_group_id(uint8_t data)
{
	int rc = Invlid_Group;

	switch (data) {
	case HI846_131_GROUP_ONE_VALUE:
		rc = Group_One;
		break;
	case HI846_131_GROUP_TWO_VALUE:
		rc = Group_Two;
		break;
	case HI846_131_GROUP_THRESS_VALUE:
		rc = Group_Three;
		break;
	default:
...
static int  hi846_131_validgroup_check_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int  rc, flag = 0;

	rc = hi846_131_get_group_id(e_ctrl->cal_data.mapdata[CS_MI_S_ADDR - 1]);
	if (rc == Invlid_Group) {
	...
	rc = hi846_131_get_group_id(e_ctrl->cal_data.mapdata[CS_AWB_S_ADDR - 1]);
	if (rc == Invlid_Group) {
...
static int hi846_131_checksum_eeprom(struct msm_eeprom_ctrl_t *e_ctrl)
{
	int  checksum = 0;
	int j;
	int rc = 0;
		...
		for (i = 0; i < eeprom_map->memory_map_size; i++) {
			...
			case MSM_CAM_READ: {
				...
				if ((eeprom_map->mem_settings[i].reg_addr
					== HI846_131_MODULE_INFO_GOUP_VALID_ABS_ADDR)
					&& ((i + 2) <= eeprom_map->memory_map_size)) {
					group_off = hi846_131_get_group_id(*memptr);
					if (group_off == Invlid_Group) {
						...
						eeprom_map->mem_settings[i + 1].reg_addr
							+= HI846_131_MODULE_INFO_GOUP_OFF_SIZE * group_off;
						CDBG("%s: 0x%x: 0x%x group %d\n", __func__,
				...
				if ((eeprom_map->mem_settings[i].reg_addr
					== HI846_131_AWB_GOUP_VALID_ABS_ADDR)
					&& ((i + 2) <= eeprom_map->memory_map_size)) {
					group_off = hi846_131_get_group_id(*memptr);
					if (group_off == Invlid_Group) {
						pr_err("%s: 0x%x: 0x%x group invlid\n", __func__,
						...
						group_off -= Group_One;
						eeprom_map->mem_settings[i + 1].reg_addr
							+= HI846_131_AWB_GOUP_OFF_SIZE * group_off;
						CDBG("%s: 0x%x: 0x%x group %d\n", __func__,
							eeprom_map->mem_settings[i + 1].reg_addr, *memptr, group_off);
						eeprom_map->mem_settings[i + 2].reg_addr
							+= HI846_131_AWB_GOUP_OFF_SIZE * group_off;
...
static struct zte_eeprom_fn_t hi846_131_eeprom_func_tbl = {
	...
	.eeprom_checksum = hi846_131_checksum_eeprom,
	.validflag_check_eeprom = hi846_131_validgroup_check_eeprom,
