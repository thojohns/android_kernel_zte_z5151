...
#define WAKE_LOCK_GESTURE
#ifdef WAKE_LOCK_GESTURE
...
static struct device_attribute attrs[] = {
	...
	__ATTR(flashprog, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(0dbutton, (S_IRUGO | S_IWUSR | S_IWGRP)/*(S_IRUGO | S_IWUGO)*/,
			synaptics_rmi4_0dbutton_show,
			synaptics_rmi4_0dbutton_store),
	__ATTR(suspend, (S_IWUSR | S_IWGRP)/*S_IWUGO*/,
			synaptics_rmi4_show_error,
			synaptics_rmi4_suspend_store),
	__ATTR(wake_gesture, (S_IRUGO | S_IWUSR | S_IWGRP)/*(S_IRUGO | S_IWUGO)*/,
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		case F12_GLOVED_FINGER_STATUS:
			...
			input_mt_sync(rmi4_data->input_dev);
#endif
			/*input_sync(rmi4_data->input_dev);wkh*/
	...
	input_sync(rmi4_data->input_dev);/*wkh add*/
	if (touch_count == 0) {
		finger_presence = 0;
#ifdef F12_DATA_15_WORKAROUND
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	/*printk("syna_rmi4_irq0\n");*/
	if (gpio_get_value(bdata->irq_gpio) != bdata->irq_on_state)
		goto exit;

	/*printk("synaptics_rmi4_irq:receive a gesture,%d,%d\n",rmi4_data->suspend,rmi4_data->enable_wakeup_gesture);*/
#ifdef WAKE_LOCK_GESTURE
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
...
#endif
		dev_err(rmi4_data->pdev->dev.parent,
					"%s: synaptics_rmi4_irq:close irq source\n",
					__func__);
		...
		unsigned int intr_count)
{
	...
	/* Maximum number of fingers supported */
	fhandler->num_of_data_points = min_t(ctrl_23->max_reported_objects,
			(unsigned char)F12_FINGERS_TO_SUPPORT);

	num_of_fingers = fhandler->num_of_data_points;
	rmi4_data->num_of_fingers = num_of_fingers;

	rmi4_data->stylus_enable = 0;/*ctrl_23->stylus_enable; zhangzhao modified this func for some crash*/
...
flash_prog_mode:
	...
	retval = secure_memcpy(rmi->product_id_string,
			...
			&f01_query[11],
			/*(unsigned int)((int)sizeof(f01_query) - 11)*/F01_STD_QUERY_LEN - 11,
			PRODUCT_ID_SIZE);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data, bool on)
{
	int retval;
	const struct synaptics_dsx_board_data *bdata =
			rmi4_data->hw_if->board_data;
	if (on) {
		...
		if (bdata->reset_gpio >= 0) {
			tp_gpio_reset = bdata->reset_gpio;
			retval = synaptics_rmi4_gpio_setup(
				bdata->reset_gpio,
	...
	if (bdata->reset_gpio >= 0) {
		if (gpio_is_valid(bdata->reset_gpio)) {
			 ...
			 */
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: 00Failed to configure reset GPIO\n",
					__func__);

		retval = gpio_direction_input(bdata->reset_gpio);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: 11Failed to configure reset GPIO\n",
					__func__);
		}
		retval = synaptics_rmi4_gpio_setup(
				bdata->reset_gpio,
				false, 1, !bdata->reset_on_state);
		if (retval < 0) {
			dev_err(rmi4_data->pdev->dev.parent,
					"%s: Failed to configure reset GPIO\n",
					__func__);
	...
	dev_err(rmi4_data->pdev->dev.parent,
					"%s: 33Failed to configure reset GPIO\n",
					__func__);
	}
}
...
static void synaptics_rmi4_rebuild_work(struct work_struct *work)
{
	...
	input_unregister_device(rmi4_data->input_dev);
	dev_err(rmi4_data->pdev->dev.parent,
				"%s: syna,rebuild work input_dev =null\n",
				__func__);
	rmi4_data->input_dev = NULL;
	if (rmi4_data->stylus_enable) {
...
static void gesture_detect_work(struct work_struct *work)
{
	...
	dev_err(rmi4_data->pdev->dev.parent,
				"%s: gesture_detect_work:synaptics_rmi4 enable irq source\n",
				__func__);
	synaptics_rmi4_int_enable(rmi4_data, true);

	/* read gesture data via i2c */
	synaptics_rmi4_sensor_report(rmi4_data, true);

	dev_err(rmi4_data->pdev->dev.parent,
				"%s: gesture_detect_work:release geture_ws\n",
				__func__);
#ifdef WAKE_LOCK_GESTURE
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	if (!retval && rmi4_data->ts_pinctrl) {
		...
		if (retval < 0) {
			...
			dev_err(&pdev->dev,
				"%s: synpatics_dsx_pinctrl_select set success.\n",
				__func__);
		}
	} else {
		dev_err(&pdev->dev,
	...
	retval = synaptics_rmi4_set_gpio(rmi4_data, true);
	if (retval < 0) {
		dev_err(&pdev->dev,
	...
	/*add by wkh for gesture begin*/
#ifdef WAKE_LOCK_GESTURE
	gesture_ws = wakeup_source_register("autosleep");
	if (!gesture_ws)
		dev_err(&pdev->dev,
				"%s: synaptics_rmi4_probe:get gesture_ws failed\n",
				__func__);
#endif
...
static int synaptics_rmi4_suspend(struct device *dev)
{
	...
	if (!rmi4_data->suspend) {
		...
		if (rmi4_data->ts_pinctrl) {
			dev_err(&pdev->dev,
			"%s: syna,deal with pinctrl\n",
			__func__);
			retval = synpatics_dsx_pinctrl_select(rmi4_data, false);
			if (retval < 0) {
				dev_err(&pdev->dev,
					"%s: syna,Failed to synpatics_dsx_pinctrl_select\n",
					__func__);
			} else {
				dev_err(&pdev->dev,
					"%s: synpatics_dsx_pinctrl_select set success.\n",
		...
		retval = synaptics_rmi4_set_gpio(rmi4_data, false);
		if (retval < 0) {
			dev_err(dev,
					"%s: Failed to set up GPIO's\n",
					__func__);
...
/*#endif*/
	...
	if (rmi4_data->ts_pinctrl) {
		...
		dev_err(&pdev->dev,
			"%s: syna,Failed to synaptics_dsx_pinctrl_init\n",
				__func__);
	}
