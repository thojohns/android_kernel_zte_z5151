...
static int parse_dt(struct device *dev, struct synaptics_dsx_board_data *bdata)
{
	...
	if (retval < 0) {
		pr_err("%s get irq-flags failed.\n", __func__);
		bdata->irq_flags = 0x2008;
		return retval;
	}
	...
	if (prop && prop->length) {
		...
		if (retval < 0) {
			if (bdata->cap_button_map->map != NULL)
				devm_kfree(dev, bdata->cap_button_map->map);
	...
	if (prop && prop->length) {
		...
		retval = of_property_read_u32_array(np,
				...
				bdata->vir_button_map->nbuttons * 5);
		if (retval < 0) {
			if (bdata->vir_button_map->map != NULL)
				devm_kfree(dev, bdata->vir_button_map->map);
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (client->dev.of_node) {
		...
		if (!hw_if.board_data->cap_button_map) {
			dev_err(&client->dev,
					...
					__func__);
			kfree(synaptics_dsx_i2c_device);
			devm_kfree(&client->dev, hw_if.board_data);
			return -ENOMEM;
		}
		...
		if (!hw_if.board_data->vir_button_map) {
			dev_err(&client->dev,
					...
					__func__);
			kfree(synaptics_dsx_i2c_device);
			devm_kfree(&client->dev, hw_if.board_data->cap_button_map);
			devm_kfree(&client->dev, hw_if.board_data);
			return -ENOMEM;
		}
	...
	if (retval) {
		...
		if (client->dev.of_node) {
			devm_kfree(&client->dev, hw_if.board_data->cap_button_map);
			devm_kfree(&client->dev, hw_if.board_data->vir_button_map);
			devm_kfree(&client->dev, hw_if.board_data);
		}
#endif
		return -ENODEV;
