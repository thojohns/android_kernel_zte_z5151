...
static struct device_attribute attrs[] = {
	__ATTR(dorecovery, S_IWUSR | S_IWGRP/*S_IWUGO*/,
	...
	__ATTR(guestcodeblockcount, S_IRUGO,
			...
			synaptics_rmi4_store_error),
	__ATTR(writeguestcode, S_IWUSR | S_IWGRP/*S_IWUGO*/,
			synaptics_rmi4_show_error,
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (config_id[0] == 72 && config_id[1] == 70)
		return snprintf(buf, PAGE_SIZE, "Synaptics2716+BOE(%d.%d.%d.%d)\n",
			config_id[0], config_id[1], config_id[2], config_id[3]);
	else if (config_id[0] == 72 && config_id[1] == 73)
		return snprintf(buf, PAGE_SIZE, "Synaptics2716+dij(%d.%d.%d.%d)\n",
			config_id[0], config_id[1], config_id[2], config_id[3]);
	else if (config_id[0] == 73 && config_id[1] == 53)
		return snprintf(buf, PAGE_SIZE, "Synaptics2714+Lead(%d.%d.%d.%d)\n",
			config_id[0], config_id[1], config_id[2], config_id[3]);
	else
		return retval;
}
