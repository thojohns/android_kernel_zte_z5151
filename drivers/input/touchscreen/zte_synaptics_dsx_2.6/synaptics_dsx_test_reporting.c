...
static int disable_cbc(unsigned short address, unsigned char *data, unsigned short len)
{
	int retval;
...
static int test_do_preparation(void)
{
	...
	default:
		if (f54->query.touch_controller_family == 1) {
			retval = disable_cbc(f54->control.reg_7->address,
									f54->control.reg_7->data,
									sizeof(f54->control.reg_7->data));
			f54->control.reg_7->cbc_tx_carrier_selection = 0;
		} else if (f54->query.has_ctrl88) {
			retval = disable_cbc(f54->control.reg_88->address,
		...
		if (f54->query.has_0d_acquisition_control) {
			retval = disable_cbc(f54->control.reg_57->address,
									f54->control.reg_57->data,
									sizeof(f54->control.reg_57->data));
			f54->control.reg_57->cbc_tx_carrier_selection = 0;
		}
