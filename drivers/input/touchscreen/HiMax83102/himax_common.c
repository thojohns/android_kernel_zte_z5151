...
unsigned char i_CTPM_FW0[] = {
	#include HXTS_UPGRADE_FW0_APP
};
unsigned char i_CTPM_FW1[] = {
	#include HXTS_UPGRADE_FW1_APP
};
unsigned char i_CTPM_FW2[] = {
	#include HXTS_UPGRADE_FW2_APP
};
#endif
...
extern int himax_platform_pinctrl_init(struct i2c_client *client, struct himax_i2c_platform_data *pdata);
...
int himax_usb_status_event(bool status)
{
	I("%s The  \"HX_USB_DETECT_GLOBAL\" macro is not opend!!!\n", __func__);

	return 0;
}
#endif
EXPORT_SYMBOL(himax_usb_status_event);
...
void himax_esd_hw_reset(void)
{
	...
	himax_report_all_leave(private_ts);
...
void himax_report_points(struct himax_ts_data *ts)
{
	...
	finger_nums = (hx_point_num > finger_nums) ? hx_point_num : finger_nums;
	...
	/* finger leave/release */
		...
		if ((!hx_touch_data->finger_on) && (finger_presence == 1)) {
			I("touch up, finger nums: %d, coord [%d:%d]\n", finger_nums, log_x, log_y);
			finger_nums = 0;
			log_x = 0;
			log_y = 0;
			finger_presence = 0;
...
int himax_gpio_shutdown_config(void)
{
#ifdef HX_RST_PIN_FUNC
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	struct himax_ts_data *ts;
	struct himax_i2c_platform_data *pdata;
	int tp_id0, tp_id1;/*add by zte for read lcd hardware id*/
	...
	ret = himax_platform_pinctrl_init(ts->client, pdata);
	if (ret < 0) {
...
int himax_chip_common_remove(struct i2c_client *client)
{
	...
	/* remove reset and int pinctrls */
	if (ts->pdata->ts_pinctrl) {
		devm_pinctrl_put(ts->pdata->ts_pinctrl);
		ts->pdata->ts_pinctrl = NULL;
	}
