...
unsigned char tp_wakeup_gesture_pre_flag = 0xff;
extern bool tp_enable_wakeup_gesture_state;
#endif
extern int g_vendor_id;
int g_max_mutual = 0;
...
=============================================================================================================*/
#if defined(CONFIG_TOUCHSCREEN_HIMAX_DEBUG_HX83102)
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		switch (status) {
		case 0:
			ret += snprintf(buf + ret, 30, "Step : START_TEST\n");
			break;
		case 1:
			ret += snprintf(buf + ret, 30, "Step : RAW_DATA\n");
			break;
		case 2:
			ret += snprintf(buf + ret, 30, "Step : PERCENT_TEST\n");
			break;
		case 3:
			ret += snprintf(buf + ret, 30, "Step : DEV_TEST\n");
			break;
		case 4:
			ret += snprintf(buf + ret, 30, "Step : NOISE_TEST\n");
			break;
		case 9:
			ret += snprintf(buf + ret, 30, "Step : END_TEST\n");
			break;
		default:
			ret += snprintf(buf + ret, 30, "Step : Null\n");
		}

		result = ito_get_result_status();
		if (result == 0xF)
			ret += snprintf(buf + ret, 30, "ITO test is On-going!\n");
		else if (result == 0)
			ret += snprintf(buf + ret, 30, "ITO test is Pass!\n");
		else if (result == 2)
			ret += snprintf(buf + ret, 30, "Open config file fail!\n");
		else
			ret += snprintf(buf + ret, 30, "ITO test is Fail!\n");
		HX_PROC_SEND_FLAG = 1;
	} else
		HX_PROC_SEND_FLAG = 0;
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (result)
			ret += snprintf(temp_buf + ret, 30, "CRC test is Pass!\n");
		else
			ret += snprintf(temp_buf + ret, 30, "CRC test is Fail!\n");
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		#endif
		count += snprintf(temp_buf + count, 32, "FW_VER = 0x%2.2X\n", ic_data->vendor_fw_ver);

		if (IC_TYPE < 8)
			count += snprintf(temp_buf + count, 32, "CONFIG_VER = 0x%2.2X\n", ic_data->vendor_config_ver);
		else {
			count += snprintf(temp_buf + count, 32,
		...
		if (ic_data->vendor_cid_maj_ver < 0 && ic_data->vendor_cid_min_ver < 0)
			count += snprintf(temp_buf + count, 32, "CID_VER = NULL\n");
		else
			count += snprintf(temp_buf + count, 32,
			"CID_VER = 0x%2.2X\n", (ic_data->vendor_cid_maj_ver << 8 | ic_data->vendor_cid_min_ver));

		if (ic_data->vendor_panel_ver < 0)
			count += snprintf(temp_buf + count, 32, "PANEL_VER = NULL\n");
		else
			count += snprintf(temp_buf + count, 32, "PANEL_VER = 0x%2.2X\n", ic_data->vendor_panel_ver);

		count += snprintf(temp_buf + count, 32, "\n");
		count += snprintf(temp_buf + count, 32, "Himax Touch Driver Version:\n");
		count += snprintf(temp_buf + count, 32, "%s\n", HIMAX_DRIVER_VER);
		HX_PROC_SEND_FLAG = 1;
...
#ifndef CONFIG_TP_COMMON_INTERFACE
/* add by wkh for touch info begin */
static ssize_t himax_touchinfo_read(struct file *file, char *buf,
	size_t count, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG_READ) {
		himax_read_FW_ver(private_ts->client);
#ifdef HX_RST_PIN_FUNC
		himax_ic_reset(false, false);
#endif
		len += snprintf(temp_buf+len, PAGE_SIZE, "Himax-tp\n");
		len += snprintf(temp_buf+len, PAGE_SIZE, "Chip type: 83102\n");
		len += snprintf(temp_buf+len, PAGE_SIZE, "FW version:%#x\n", ic_data->vendor_fw_ver);
		len += snprintf(temp_buf+len, PAGE_SIZE, "Cfg version: 0x%2.2X\n", ic_data->vendor_touch_cfg_ver);
		len += snprintf(temp_buf+len, PAGE_SIZE, "Display version: 0x%2.2X\n", ic_data->vendor_display_cfg_ver);

		if (g_vendor_id == HX_VENDOR_ID_0) {
			len += snprintf(temp_buf+len, PAGE_SIZE, "Verdor:%s\n", HXTS_VENDOR_0_NAME);
		...
		HX_PROC_SEND_FLAG_READ = 1;
		if (copy_to_user(buf, temp_buf, len)) {
			I("%s, here:%d\n", __func__, __LINE__);
		}
	} else {
		HX_PROC_SEND_FLAG_READ = 0;
	}
...
static struct file_operations himax_proc_touchinfo_ops = {
	.owner = THIS_MODULE,
	.read = himax_touchinfo_read,
};
#endif
/* end */
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		count += snprintf(temp_buf + count, 10, "%d ", ts->irq_enabled);
		count += snprintf(temp_buf + count, 8, "\n");
		if (copy_to_user(buf, temp_buf, len))
			I("%s, here:%d\n", __func__, __LINE__);
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		count += snprintf(temp_buf + count, 16, "%d ", ts->pdata->abs_x_min);
		count += snprintf(temp_buf + count, 16, "%d ", ts->pdata->abs_x_max);
		count += snprintf(temp_buf + count, 16, "%d ", ts->pdata->abs_y_min);
		count += snprintf(temp_buf + count, 16, "%d ", ts->pdata->abs_y_max);
		count += snprintf(temp_buf + count, 16, "\n");
		if (copy_to_user(buf, temp_buf, len))
			I("%s, here:%d\n", __func__, __LINE__);
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret += snprintf(temp_buf, 30, "command:  %02X, %02X, %02X, %02X\n",
			register_command[3], register_command[2],
			register_command[1], register_command[0]);

		for (loop_i = 0; loop_i < 128; loop_i++) {
			ret += snprintf(temp_buf + ret, 16, "0x%2.2X ", data[loop_i]);
			if ((loop_i % 16) == 15)
				ret += snprintf(temp_buf + ret, 16, "\n");
		}
		ret += snprintf(temp_buf + ret, 16, "\n");
		if (copy_to_user(buf, temp_buf, len))
			I("%s, here:%d\n", __func__, __LINE__);
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
		...
		if (buf[0] == 'r') {
			...
			if (count < 3) {
				...
				himax_register_write(private_ts->client, register_command,
				...
				himax_register_write(private_ts->client,
					register_command, byte_length, &w_data[0], cfg_flag);
			}
		} else
			return len;
...
void himax_log_touch_event_detail(struct himax_ts_data *ts, int x, int y, int w,
	int loop_i, uint8_t EN_NoiseFilter, int touched, uint16_t old_finger)
{
	if (touched == HX_FINGER_ON) {
...
void himax_ts_diag_func(void)
{
	...
	for (i = 0; i < ic_data->HX_TX_NUM; i++) {
		for (j = 0; j < ic_data->HX_RX_NUM; j++) {
			tmp_hb = info_data[index + 1];
			tmp_lb = info_data[index];
			new_data = (int16_t)(tmp_hb << 8 | (tmp_lb < 0 ? 0x100 + tmp_lb : tmp_lb));
	...
	if (dsram_type == 3) {
		memcpy(mutual_data_old, mutual_data_new,
			x_channel * y_channel * sizeof(int16_t)); /*copy N data to N-1 array*/
	}
	diag_max_cnt++;
	if (dsram_type == 1 || dsram_type == 3) {
		...
		for (i = 0; i < x_channel * y_channel; i++) {
			...
			strlcat(write_buf, temp_buf, sizeof(write_buf));
		}
	...
	if (!HX_PROC_SEND_FLAG) {
		if (debug_level_cmd == 't') {
			if (fw_update_complete) {
				count += snprintf(temp_buf, 32, "FW Update Complete ");
			} else{
				count += snprintf(temp_buf, 16, "FW Update Fail ");
			}
		} else if (debug_level_cmd == 'h') {
			if (handshaking_result == 0) {
				count += snprintf(temp_buf, 64,
				...
				count += snprintf(temp_buf, 32, "Handshaking Result = error\n");
			}
			...
			if (ic_data->vendor_cid_maj_ver < 0 && ic_data->vendor_cid_min_ver < 0)
				...
				count += snprintf(temp_buf + count, 32, "CID_VER = 0x%2.2X\n",
					(ic_data->vendor_cid_maj_ver << 8 | ic_data->vendor_cid_min_ver));
			...
			count += snprintf(temp_buf + count, 32, "Himax Touch IC Information :\n");
			switch (IC_TYPE) {
			case HX_85XX_D_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX852xD\n");
				break;
			case HX_85XX_E_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX852xE\n");
				break;
			case HX_85XX_ES_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX852xES\n");
				break;
			case HX_85XX_F_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX852xF\n");
				break;
			case HX_83100A_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83100A\n");
				break;
			case HX_83102A_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83102A\n");
				break;
			case HX_83102B_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83102B\n");
				break;
			case HX_83110A_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83110A\n");
				break;
			case HX_83110B_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83110B\n");
				break;
			case HX_83111B_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83111B\n");
				break;
			case HX_83112A_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83112A\n");
				break;
			case HX_83112B_SERIES_PWON:
				count += snprintf(temp_buf + count, 32, "IC Type : HX83112B\n");
				break;
			default:
				count += snprintf(temp_buf + count, 32, "IC Type error.\n");
			}
			switch (IC_CHECKSUM) {
			case HX_TP_BIN_CHECKSUM_SW:
				count += snprintf(temp_buf + count, 32, "IC Checksum : SW\n");
				break;
			case HX_TP_BIN_CHECKSUM_HW:
				count += snprintf(temp_buf + count, 32, "IC Checksum : HW\n");
				break;
			case HX_TP_BIN_CHECKSUM_CRC:
				count += snprintf(temp_buf + count, 32, "IC Checksum : CRC\n");
				break;
			default:
				count += snprintf(temp_buf + count, 32, "IC Checksum error.\n");
			}

			if (ic_data->HX_INT_IS_EDGE) {
				count += snprintf(temp_buf + count, 64, "Driver register Interrupt : EDGE TIRGGER\n");
			} else {
				count += snprintf(temp_buf + count, 64, "Driver register Interrupt : LEVEL TRIGGER\n");
			}
			if (private_ts->protocol_type == PROTOCOL_TYPE_A) {
				count += snprintf(temp_buf + count, 32, "Protocol : TYPE_A\n");
			} else {
				count += snprintf(temp_buf + count, 32, "Protocol : TYPE_B\n");
			}
			count += snprintf(temp_buf + count, 32, "RX Num : %d\n", ic_data->HX_RX_NUM);
			count += snprintf(temp_buf + count, 32, "TX Num : %d\n", ic_data->HX_TX_NUM);
			count += snprintf(temp_buf + count, 32, "BT Num : %d\n", ic_data->HX_BT_NUM);
			count += snprintf(temp_buf + count, 32, "X Resolution : %d\n", ic_data->HX_X_RES);
			count += snprintf(temp_buf + count, 32, "Y Resolution : %d\n", ic_data->HX_Y_RES);
			count += snprintf(temp_buf + count, 32, "Max Point : %d\n", ic_data->HX_MAX_PT);
			count += snprintf(temp_buf + count, 32, "XY reverse : %d\n", ic_data->HX_XY_REVERSE);
	#ifdef HX_TP_PROC_2T2R
			if (Is_2T2R) {
				count += snprintf(temp_buf + count, 32, "2T2R panel\n");
				count += snprintf(temp_buf + count, 32, "RX Num_2 : %d\n", HX_RX_NUM_2);
				count += snprintf(temp_buf + count, 32, "TX Num_2 : %d\n", HX_TX_NUM_2);
			}
	#endif
		} else if (debug_level_cmd == 'i') {
			if (himax_read_i2c_status(private_ts->client))
				count += snprintf(temp_buf + count, 32, "I2C communication is bad.\n");
			else
				count += snprintf(temp_buf + count, 32, "I2C communication is good.\n");
		} else if (debug_level_cmd == 'n') {
			if (himax_read_ic_trigger_type(private_ts->client) == 1) /*Edgd = 1, Level = 0*/
				count += snprintf(temp_buf + count, 64, "IC Interrupt type is edge trigger.\n");
			else if (himax_read_ic_trigger_type(private_ts->client) == 0)
				count += snprintf(temp_buf + count, 64, "IC Interrupt type is level trigger.\n");
			else
				count += snprintf(temp_buf + count, 32, "Unknown IC trigger type.\n");
			if (ic_data->HX_INT_IS_EDGE)
				count += snprintf(temp_buf + count, 64, "Driver register Interrupt : EDGE TIRGGER\n");
			else
				count += snprintf(temp_buf + count, 64, "Driver register Interrupt : LEVEL TRIGGER\n");
			...
			count += snprintf(temp_buf + count, 32, "chip_monitor :%d\n",
	...
	size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'h') { /*handshaking*/
		...
		upgrade_fw = kzalloc(UPGRADE_FW_BUF_SIZE, GFP_KERNEL);
		if (upgrade_fw == NULL) {
			E("failed to kzalloc upgrade_fw\n");
			goto upgrade_fw_malloc_failed;
		}
		...
		result = filp->f_op->read(filp, upgrade_fw, UPGRADE_FW_BUF_SIZE, &filp->f_pos);
		if (result < 0) {
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (himax_read_FW_status(cmd_set, tmp_data) == NO_ERR) {
			ret += snprintf(temp_buf + ret, 32, "0x%02X%02X%02X%02X :\t",
			cmd_set[5], cmd_set[4], cmd_set[3], cmd_set[2]);
			for (loop_i = 0; loop_i < cmd_set[1]; loop_i++) {
				ret += snprintf(temp_buf + ret, 32, "%5d\t", tmp_data[loop_i]);
			}
			ret += snprintf(temp_buf + ret, 32, "\n");
		}
		cmd_set[0] = 0x02;
		...
		if (himax_read_FW_status(cmd_set, tmp_data) == NO_ERR) {
			for (loop_i = 0; loop_i < cmd_set[1]; loop_i = loop_i + 2) {
				...
				ret += snprintf(temp_buf + ret, 32, "%5d\t",
				tmp_data[loop_i] + (tmp_data[loop_i + 1] << 8));
				if ((loop_i % 16) == 14)
					ret += snprintf(temp_buf + ret, 8, "\n");
			}
		}
		ret += snprintf(temp_buf + ret, 8, "\n");
		if (copy_to_user(buf, temp_buf, len))
		...
		if (mutual_set_flag == 1) {
			if (himax_read_DD_status(cmd_set, tmp_data) == NO_ERR) {
				for (loop_i = 0; loop_i < cmd_set[0]; loop_i++) {
					if ((loop_i % 8) == 0)
						ret += snprintf(temp_buf + ret, 32, "0x%02X : ", loop_i);
					ret += snprintf(temp_buf + ret, 32, "0x%02X ", tmp_data[loop_i]);
					if ((loop_i % 8) == 7)
						ret += snprintf(temp_buf + ret, 8, "\n");
				}
			}
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		if (local_flash_fail) {
			ret += snprintf(temp_buf + ret, 32, "FlashStart:Fail\n");
			ret += snprintf(temp_buf + ret, 32, "FlashEnd");
			ret += snprintf(temp_buf + ret, 8, "\n");
			if (copy_to_user(buf, temp_buf, len))
				I("%s, here:%d\n", __func__, __LINE__);
			kfree(temp_buf);
		...
		if (!local_flash_complete) {
			ret += snprintf(temp_buf + ret, 32, "FlashStart:Ongoing:0x%2.2x\n", flash_progress);
			ret += snprintf(temp_buf + ret, 32, "FlashEnd");
			ret += snprintf(temp_buf + ret, 8, "\n");
			if (copy_to_user(buf, temp_buf, len))
				I("%s, here:%d\n", __func__, __LINE__);
		...
		if (local_flash_command == 1 && local_flash_complete) {
			ret += snprintf(temp_buf + ret, 32, "FlashStart:Complete\n");
			ret += snprintf(temp_buf + ret, 32, "FlashEnd");
			ret += snprintf(temp_buf + ret, 8, "\n");
			if (copy_to_user(buf, temp_buf, len))
				I("%s, here:%d\n", __func__, __LINE__);
		...
		if (local_flash_command == 3 && local_flash_complete) {
			ret += snprintf(temp_buf + ret, 32, "FlashStart:\n");
			for (loop_i = 0; loop_i < 128; loop_i++) {
				ret += snprintf(temp_buf + ret, 32, "x%2.2x", flash_buffer[loop_i]);
				if ((loop_i % 16) == 15) {
					ret += snprintf(temp_buf + ret, 8, "\n");
				}
			}
		...
		ret += snprintf(temp_buf + ret, 32, "FlashStart:%2.2x\n", local_flash_read_step);

		for (loop_i = 0; loop_i < 1024; loop_i++) {
			ret += snprintf(temp_buf + ret, 32, "x%2.2X",
	...
	size_t len, loff_t *pos)
{
	...
	temp_buf = kzalloc(len, GFP_KERNEL);
	I("%s: enter, HX_PROC_SEND_FLAG = %d\n", __func__, HX_PROC_SEND_FLAG);
	if (!HX_PROC_SEND_FLAG) {
	...
	if (val == 0x01) {
		ret += snprintf(temp_buf + ret, 32, "0");
	} else {
		ret += snprintf(temp_buf + ret, 32, "-1");
	}
	I("temp_buf : %s: enter, ret = %d\n", temp_buf, ret);
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		count = snprintf(temp_buf, PAGE_SIZE, 32, "%d\n", ts->HSEN_enable);
		if (copy_to_user(buf, temp_buf, len))
			I("%s, here:%d\n", __func__, __LINE__);
...
#ifdef HX_SMART_WAKEUP
void SMWP_set(struct himax_ts_data *ts, unsigned char smwp_flag)
{
	ts->SMWP_enable = smwp_flag;
	tp_enable_wakeup_gesture_state = ts->SMWP_enable;
	...
	size_t len, loff_t *pos)
{
	...
	I("%s: set SMWP %d, suspend state is %d\n", __func__, SMWP_temp, atomic_read(&ts->suspend_mode));

	if (atomic_read(&ts->suspend_mode)) {
		tp_wakeup_gesture_pre_flag = SMWP_temp;
		return len;
	}

	tp_wakeup_gesture_pre_flag = 0xff;
	SMWP_set(ts, SMWP_temp);
	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		for (i = 0; i < 16; i++)
			ret += snprintf(temp_buf + ret, 32, "ges_en[%d]=%d\n", i, ts->gesture_cust_en[i]);
		if (copy_to_user(buf, temp_buf, len))
			I("%s, here:%d\n", __func__, __LINE__);
	...
	size_t len, loff_t *pos)
{
	...
	I("%s: enter, %d\n", __func__, __LINE__);
	if (!HX_PROC_SEND_FLAG) {
		ret += snprintf(temp_buf + ret, 64, "EB_cnt = %d, EC_cnt = %d, ED_cnt = %d\n",
...
static int tpd_init_tpinfo(struct tpd_classdev_t *cdev)
{
	...
	strlcpy(cdev->ic_tpinfo.tp_name, "Himax", sizeof(cdev->ic_tpinfo.tp_name));
	...
	cdev->ic_tpinfo.chip_part_id = 0x83102;
	cdev->ic_tpinfo.module_id = g_vendor_id;
...
static int himax_i2c_reg_read(struct tpd_classdev_t *cdev, u32 addr, u8 *data, int len)
{
	...
	himax_register_read(private_ts->client, address, len, data, false);
	return 0;
}
...
static int himax_i2c_reg_write(struct tpd_classdev_t *cdev, u32 addr, u8 *data, int len)
{
	...
	address[2] = (u8)(addr >> 16);
	address[3] = (u8)(addr >> 24);
	himax_register_write(private_ts->client, address, len, data, false);
	return 0;
}
...
#ifndef CONFIG_TP_COMMON_INTERFACE
	/* add by wkh for touch info begin */
	himax_touch_touch_info_proc_dir = proc_mkdir(HIMAX_PROC_TOUCH_INFO, NULL);
	if (himax_touch_touch_info_proc_dir == NULL) {
		E(" %s: himax_touch_proc_dir file create failed!\n", __func__);
		goto fail_18;
	}
	himax_proc_touchinfo_file = proc_create(HIMAX_PROC_TOUCH_INFO_FILE, (S_IRUGO),
		himax_touch_touch_info_proc_dir, &himax_proc_touchinfo_ops);
	if (himax_proc_touchinfo_file == NULL) {
		E(" %s: proc vendor file create failed!\n", __func__);
		goto fail_19;
	}
#ifdef HX_SMART_WAKEUP
	himax_proc_SMWP_file = proc_create(HIMAX_PROC_SMWP_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		himax_touch_touch_info_proc_dir, &himax_proc_SMWP_ops);
	if (himax_proc_SMWP_file == NULL) {
		E(" %s: proc SMWP file create failed!\n", __func__);
	}
	himax_proc_GESTURE_file = proc_create(HIMAX_PROC_GESTURE_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		himax_touch_touch_info_proc_dir, &himax_proc_Gesture_ops);
	if (himax_proc_GESTURE_file == NULL) {
		E(" %s: proc GESTURE file create failed!\n", __func__);
	}
#endif
...
#ifndef CONFIG_TP_COMMON_INTERFACE
fail_19:
	remove_proc_entry(HIMAX_PROC_TOUCH_INFO, NULL);
#endif
fail_18:
#ifdef HX_ESD_RECOVERY
...
void himax_touch_proc_deinit(void)
{
	...
	/* add by wkh for touch info begin */
#ifndef CONFIG_TP_COMMON_INTERFACE
	remove_proc_entry(HIMAX_PROC_TOUCH_INFO_FILE, himax_touch_touch_info_proc_dir);
#ifdef HX_SMART_WAKEUP
	remove_proc_entry(HIMAX_PROC_GESTURE_FILE, himax_touch_proc_dir);
	remove_proc_entry(HIMAX_PROC_SMWP_FILE, himax_touch_proc_dir);
#endif
