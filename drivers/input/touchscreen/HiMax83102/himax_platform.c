...
extern unsigned char tp_wakeup_gesture_pre_flag;
extern void SMWP_set(struct himax_ts_data *ts, unsigned char smwp_flag);
#endif
				...
				struct himax_i2c_platform_data *pdata)
{
	...
	if (!gpio_is_valid(pdata->gpio_id0)) {
		I(" DT:gpio_id0 value is not valid\n");
	}
	pdata->gpio_id1 = of_get_named_gpio(dt, "himax,id1-gpio", 0);
	if (!gpio_is_valid(pdata->gpio_id1)) {
		I(" DT:gpio_id1 value is not valid\n");
	}
	I(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d, id0=%d, id1=%d", pdata->gpio_irq,
		pdata->gpio_reset, pdata->gpio_3v3_en, pdata->gpio_id0, pdata->gpio_id1);
...
#if defined(CONFIG_HMX_DB_HX83102)
...
int himax_gpio_power_config(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	I("HIMAX enter himax_gpio_power_config\n");
	...
	if (gpio_is_valid(pdata->gpio_id0)) {
		...
		error = gpio_request(pdata->gpio_id0, "hmx_id0");
		if (error) {
			E("unable to request gpio [%d]\n",
						pdata->gpio_id0);
		...
		error = gpio_direction_input(pdata->gpio_id0);
		if (error) {
			E("unable to set direction for gpio [%d]\n",
				pdata->gpio_id0);
	...
	if (gpio_is_valid(pdata->gpio_id1)) {
		...
		error = gpio_request(pdata->gpio_id1, "hmx_id1");
		if (error) {
			E("unable to request gpio [%d]\n",
						pdata->gpio_id1);
		...
		error = gpio_direction_input(pdata->gpio_id1);
		if (error) {
			E("unable to set direction for gpio [%d]\n",
				pdata->gpio_id1);
...
int himax_platform_pinctrl_init(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	int ret;
	...
	if (ret < 0) {
		E("failed to select pin to active state");
...
int himax_int_register_trigger(struct i2c_client *client)
{
	...
	if (ic_data->HX_INT_IS_EDGE) {
		...
		ret = request_threaded_irq(client->irq, NULL,
			himax_ts_thread, IRQF_TRIGGER_FALLING | IRQF_ONESHOT, client->name, ts);
	} else {
		I("%s level trigger low\n ", __func__);
		ret = request_threaded_irq(client->irq, NULL,
			himax_ts_thread, IRQF_TRIGGER_LOW | IRQF_ONESHOT, client->name, ts);
	}
	return ret;
}
				 ...
				 unsigned long event, void *data)
{
	...
	if (evdata && evdata->data && event == FB_EARLY_EVENT_BLANK && ts &&
		...
		case FB_BLANK_UNBLANK:
			if (ts->SMWP_enable == 0) {
				I("%s resume step 1: disable reset\n", __func__);
		...
		case FB_BLANK_NORMAL:
			I("%s suspend step 1: suspend\n", __func__);
			himax_common_suspend(&ts->client->dev);

			if (ts->SMWP_enable == 0) {
				I("%s suspend step 2: enable reset\n", __func__);
	...
	if (evdata && evdata->data && event == FB_EVENT_BLANK && ts &&
		...
		switch (*blank) {
		case FB_BLANK_UNBLANK:
			I("%s resume step 2: resume\n", __func__);
			himax_common_resume(&ts->client->dev);

			/* When modify wakeup_gesture mode in the time of tp suspend,
			     it is a i2c error. So do the work after resume
			*/
			#ifdef HX_SMART_WAKEUP
			if (tp_wakeup_gesture_pre_flag != 0xff) {
				SMWP_set(ts, tp_wakeup_gesture_pre_flag);
