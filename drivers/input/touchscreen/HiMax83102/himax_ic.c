...
int himax_switch_mode(struct i2c_client *client, int mode)
{
	...
	if (mode) {
		...
		himax_flash_write_burst(client, tmp_addr, tmp_data);
	} else {/*0x0A normal mode 10 frame*/
		tmp_addr[3] = 0x10; tmp_addr[2] = 0x00; tmp_addr[1] = 0x72; tmp_addr[0] = 0x94;
		tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0x00; tmp_data[0] = 0x0A;
		/* N Frame Sorting*/
		himax_flash_write_burst(client, tmp_addr, tmp_data);
...
int himax_chip_self_test(struct i2c_client *client)
{
	...
	tmp_data[7] = 0x00; tmp_data[6] = 0x00; tmp_data[5] = 0x00; tmp_data[4] = 0x64;
...
bool himax_block_erase(struct i2c_client *client, int start_addr, int length)
{
	...
	for (page_prog_start = start_addr; page_prog_start < start_addr + length;
								page_prog_start = page_prog_start + 0x10000) {
			/*=====================================*/
			/* Chip Erase*/
			/* Write Enable : 1. 0x8000_0020 ==> 0x4700_0000*/
			/*				  2. 0x8000_0024 ==> 0x0000_0006*/
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	for (i = 0; i < 5; i++) {
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x10)
			...
			if (g_vendor_id == 0) {
				I("Himax IC %s\n", HXTS_VENDOR_0_NAME);
				g_i_FW_VER = i_CTPM_FW0[FW_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW0[FW_VER_MIN_FLASH_ADDR];
				g_i_CFG_VER = i_CTPM_FW0[CFG_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW0[CFG_VER_MIN_FLASH_ADDR];
				g_i_CID_MAJ = i_CTPM_FW0[CID_VER_MAJ_FLASH_ADDR];
				g_i_CID_MIN = i_CTPM_FW0[CID_VER_MIN_FLASH_ADDR];
			} else if (g_vendor_id == 1) {
				I("Himax IC %s\n", HXTS_VENDOR_1_NAME);
				g_i_FW_VER = i_CTPM_FW1[FW_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW1[FW_VER_MIN_FLASH_ADDR];
				g_i_CFG_VER = i_CTPM_FW1[CFG_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW1[CFG_VER_MIN_FLASH_ADDR];
				g_i_CID_MAJ = i_CTPM_FW1[CID_VER_MAJ_FLASH_ADDR];
				g_i_CID_MIN = i_CTPM_FW1[CID_VER_MIN_FLASH_ADDR];
			} else if (g_vendor_id == 2) {
				I("Himax IC %s\n", HXTS_VENDOR_2_NAME);
				g_i_FW_VER = i_CTPM_FW2[FW_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW2[FW_VER_MIN_FLASH_ADDR];
				g_i_CFG_VER = i_CTPM_FW2[CFG_VER_MAJ_FLASH_ADDR] << 8
					| i_CTPM_FW2[CFG_VER_MIN_FLASH_ADDR];
	...
	&& hx_touch_data->hx_rawdata_buf[3] == diag_cmd) {
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				tmp_hb = hx_touch_data->hx_rawdata_buf[i*2 + 4 + 1];
				tmp_lb = hx_touch_data->hx_rawdata_buf[i*2 + 4];
				/*RawData Header, 1:HSB*/
				mutual_data[index + i] =
					(int16_t)(tmp_hb*256 + (tmp_lb < 0 ? 0x100 + tmp_lb : tmp_lb));
			} else {/*self*/
				temp1 = i + index;
...
void himax_guest_info_read(uint32_t start_addr, uint8_t *flash_tmp_buffer)
{
	...
	for (temp_addr = start_addr; temp_addr < (start_addr + flash_page_len);
		...
		himax_register_read(private_ts->client, tmp_addr, 128,
...
int himax_read_project_id(void)
{
	...
	for (custom_info_temp = 0; custom_info_temp < HX_GUEST_INFO_SIZE; custom_info_temp++)	{
		himax_guest_info_read(panel_color_addr + custom_info_temp * flash_page_len,
		...
		if (custom_info_temp == 0) {
			for (i = 0; i < 128; i++) {
				if (i % 16 == 0 && i > 0)
					I("\n");
				I("g_guest_str[%d][%d] = 0x%02X", custom_info_temp,
								i, g_guest_str[custom_info_temp][i]);
			}
		}
	}
