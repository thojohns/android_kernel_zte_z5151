...
#define SYN_OFILM_FW_NAME_3ND "ZTE_Z6400_TianMa_Ofilm_6P0_1920_1080_S3603_DS5_17.0.0.1085_PR2697865_55343233.img"
#define SYN_OFILM_FW_NAME_4ND "ZTE_Z6400_Lead_Ofilm_OTM1906_6P0_1920_1080_S3603_DS5_17.0.0.1085_PR2744661_55343735.img"
#define SYN_OFILM_FW_NAME_5ND "ZTE_Z6400_Lead_Ofilm_OTM1911_6P0_1920_1080_S3603_DS5_17.0.0.1085_PR2744661_55343931.img"
#endif
	...
	if (touch_module < SYN_MOUDLE_NUM_MAX) {
		...
		if ((strcmp(zte_get_lcd_panel_name(), "zteTM(NT35695)TM_1080*1920_6P0Inch") == 0) ||
			...
			if (!bootloader_mode) {
				...
				else if (fw_origin_id == 0x32 || fw_origin_id == 0x37 || fw_origin_id == 0x39)
					syna_file_name = SYN_OFILM_FW_NAME_3ND;
			} else {
				if (customer_ID == NULL) {
					pr_err("%s: customer_ID is null\n", __func__);
					return;
				}
				if (strcmp(customer_ID, CUSTOMER_ID_OLD) == 0)
					syna_file_name = SYN_OFILM_FW_NAME_2ND;
			...
			if (!bootloader_mode) {
				...
				else if (fw_origin_id == 0x37 || fw_origin_id == 0x32)
					syna_file_name = SYN_OFILM_FW_NAME_4ND;
			} else {
				if (customer_ID == NULL) {
					pr_err("%s: customer_ID is null\n", __func__);
					return;
		...
		} else if (strcmp(zte_get_lcd_panel_name(), "zteLEAD_OTM1911A_AUO_1080_1920_6P0Inch") == 0)
			syna_file_name = SYN_OFILM_FW_NAME_5ND;
#else
		syna_file_name = syn_fwfile_table[touch_module];
#endif
...
{
	int len = 0;
	char chiptype[CHIP_INFO_LEN], sensor[CHIP_INFO_LEN];
	int fw_ver = 0;
	int ready_fw_ver;
	int fw_flag = 0;
	int fw_version = 0;
...
#endif
	len += snprintf(buffer + len, sizeof(buffer) - len, "chip type : %s\n", chiptype);
	len += snprintf(buffer + len, sizeof(buffer) - len, "sensor partner : %s\n", sensor);
	len += snprintf(buffer + len, sizeof(buffer) - len, "FW Revision : %c%c\n",
		(fw_version & 0x0000ff00) >> 8, fw_version & 0x0000ff);
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_DSX_FW_UPDATE_V27
	len += snprintf(buffer + len, sizeof(buffer) - len, "update flag : 0x%02x\n", syna_update_flag);
#endif

	if ((socinfo_get_ftm_flag() == 1) || (ready_fw_ver < 0)) {
		len += snprintf(buffer + len, sizeof(buffer) - len, "need update : %s\n", "no");
		len += snprintf(buffer + len, sizeof(buffer) - len, "ready fw version : %s\n", "null");
	} else {
		len += snprintf(buffer + len, sizeof(buffer) - len, "need update : %s\n",
			(ready_fw_ver > fw_version ? "yes" : "no"));
		len += snprintf(buffer + len, sizeof(buffer) - len, "ready fw version : %c%c\n",
	...
	len += snprintf(buffer + len, sizeof(buffer) - len, "FW flag : 0x%02x\n", fw_flag);

	pr_info("%s: copy to user\n", __func__);
	...
	pr_info("%s: len = %d, size = %d\n", __func__, len, (int)size);
...
#endif
#ifdef SUSPEND_TP_REGULATOR_DISABLE
	if (syna_regulator_enable_flag) {
		synaptics_rmi4_enable_reg(rmi4_data, false);
}
#endif
