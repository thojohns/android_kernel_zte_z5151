...
static int fts_ctpm_get_vendor_id_flash(struct i2c_client *client, u8 *vendor_id)
{
	...
	msleep(20);		/* must wait, otherwise read vendor id wrong */
...
static int fts_ctpm_get_i_file(struct i2c_client *client, int fw_valid)
{
	...
	case FTS_VENDOR_3_ID:
		...
		break;
#endif
#if (FTS_GET_VENDOR_ID_NUM >= 4)
	case FTS_VENDOR_4_ID:
		g_fw_file = CTPM_FW4;
		g_fw_len = fts_getsize(FW4_SIZE);
		FTS_DEBUG("[UPGRADE]FW FILE:CTPM_FW4, SIZE:%x", g_fw_len);
		break;
#endif
	default:
...
* Return: fail <0
***********************************************************************/
static int fts_ctpm_fw_upgrade_use_buf(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[4] = { 0 };
	u32 i = 0;
	u32 packet_number;
	...
	for (j = 0; j < packet_number; j++) {
		...
		for (i = 0; i < 30; i++) {
			...
			if (i > 15) {
				msleep(20);
				FTS_DEBUG("[UPGRADE]: write flash: host : %x status : %x!!",
					  (j + 0x1000 + (0x5000 / FTS_PACKET_LENGTH)),
	...
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			if ((0x1000 +
			     ((0x5000 + packet_number * FTS_PACKET_LENGTH) / ((dw_length) % FTS_PACKET_LENGTH))) ==
			    (((reg_val[0]) << 8) | reg_val[1])) {
				break;
			}
...
* Return: fail <0
***********************************************************************/
static int fts_ctpm_lcd_cfg_upgrade_use_buf(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[4] = { 0 };
...
BOOTLOADER_ID_OK:
	...
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			if ((0x1000 + ((packet_number * FTS_PACKET_LENGTH) / ((dw_length) % FTS_PACKET_LENGTH))) ==
			    (((reg_val[0]) << 8) | reg_val[1])) {
				break;
			}
