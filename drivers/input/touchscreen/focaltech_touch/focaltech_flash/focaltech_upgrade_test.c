...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	wake_lock_init(&fts_lock, WAKE_LOCK_SUSPEND, "tp_wakelock");
	...
	do {
		...
		FTS_DEBUG("start to upgrade %d times !!", uc_UpgradeTimes);
		...
		FTS_DEBUG("upgrade %d times, error %d times!!", uc_UpgradeTimes, uc_ErrorTimes);
	} while (uc_UpgradeTimes < (FTS_UPGRADE_TEST_NUMBER));

	wake_unlock(&fts_lock);
