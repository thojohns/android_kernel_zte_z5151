...
static int fts_ft6x36gu_get_vendor_id_flash(struct i2c_client *client, u8 *vendor_id)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == chip_types.bootloader_idh)
			...
			FTS_ERROR("[UPGRADE]: read bootloader id fail!! ID1 = 0x%x,ID2 = 0x%x!!", reg_val[0],
				  reg_val[1]);
			continue;
		}
	}
...
* Return: fail <0
***********************************************************************/
static int fts_ft6x36gu_upgrade_use_buf(struct i2c_client *client, u8 *pbt_buf, u32 dw_length)
{
	u8 reg_val[2] = { 0 };
	u32 i = 0;
	u32 packet_number;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if (reg_val[0] == chip_types.bootloader_idh && reg_val[1] == chip_types.bootloader_idl) {
			/*FTS_DEBUG("[UPGRADE]: get bootload id OK :
			ID1 = 0x%x,ID2 = 0x%x!!",reg_val[0], reg_val[1]); */
...
static int fts_ft6x36gu_upgrade_with_app_bin_file(struct i2c_client *client, char *firmware_name)
{
	...
	FTS_ERROR("[UPGRADE]: app.bin length(%x) ", fwsize);
