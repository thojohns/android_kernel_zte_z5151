					  ...
					  u8 *vendor_id)
{
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		if ((reg_val[0] == chip_types.bootloader_idh) && (reg_val[1] == chip_types.bootloader_idl)) {
			FTS_DEBUG
				("[UPGRADE]: read bootloader id ok!! ID1 = 0x%x,ID2 = 0x%x!!", reg_val[0], reg_val[1]);
			goto READ_ID_OK;
		} else {
...
* Return: fail <0
***********************************************************************/
static int fts_ft5x46_upgrade_use_buf(struct i2c_client *client, u8 *pbt_buf,
				      u32 dw_length)
{
	u8 reg_val[4] = { 0 };
	u32 i = 0;
...
READ_ID_OK:
	...
	for (i = 0; i < 15; i++) {
		...
		if ((reg_val[0] == 0xF) && (reg_val[1] == 0xAA)) {
			break;
		}
		msleep(50);
	}
	...
	if ((dw_length) % FTS_PACKET_LENGTH > 0) {
		...
		for (i = 0; i < 30; i++) {
			...
			FTS_DEBUG("[UPGRADE]: reg_val[0] = %x reg_val[1] = %x  reg_val[2] = 0x%x!!",
				reg_val[0], reg_val[1],
			     (((packet_number * FTS_PACKET_LENGTH) / ((dw_length) % FTS_PACKET_LENGTH)) + 0x1000));
			/*msleep(20);*/
			fts_ctpm_upgrade_delay(1000);
		}
	}
