...
int fts_ctpm_erase_flash(struct i2c_client *client)
{
	...
	for (i = 0; i < 15; i++) {
		...
		if (reg_val[0] == 0xF0 && reg_val[1] == 0xAA) {	/*erase flash success */
			break;
		}
		msleep(50);
	}

	if ((reg_val[0] != 0xF0 || reg_val[1] != 0xAA) && (i >= 15)) {	/*erase flash fail */
			       ...
			       u8 *readbuf)
{
	...
	for (j = 0; j < packet_number; j++) {
		...
		for (i = 0; i < 30; i++) {
			/*read status and check if the app writing is finished */
			auc_i2c_write_buf[0] = 0x6a;
			reg_val[0] = reg_val[1] = 0x00;
			fts_i2c_read(client, auc_i2c_write_buf, 1, reg_val, 2);
...
u16 crc_calc(u8 *pbt_buf, u32 addr, u16 length)
{
	...
	FTS_DEBUG("[UPGRADE][ECC] : %04x  data:%04x, len:%04x!!", (addr),
		  data_word(pbt_buf, (addr)), length);
	for (i = 0; i < length; i++) {
