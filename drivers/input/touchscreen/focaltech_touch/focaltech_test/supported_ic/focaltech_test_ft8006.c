...
#define WEAK_SHORT_BYTES_PER_TIME      50
/*******************************************************************************
...
static void Save_Test_Data(int iData[TX_NUM_MAX][RX_NUM_MAX], int iArrayIndex, unsigned char Row, unsigned char Col,
...
static unsigned char FT8006_GetTxRxCB(struct FT8006 *g_FT8006, unsigned short StartNodeNo, unsigned short ReadNum,
...
unsigned char FT8006_ReadRawData(struct FT8006 *g_FT8006, unsigned char Freq, unsigned char LineNum, int ByteNum,
				 int *pRevBuffer);
...
/*tp test*/
int FT8006_get_tp_channel(int *buffer)
{
	int retval = 0;
	int tx = 0, rx = 0;
	...
	tx = g_stSCapConfEx.ChannelXNum;
	rx = g_stSCapConfEx.ChannelYNum;
	buffer[0] = tx;
	buffer[1] = rx;
...
int FT8006_get_tp_node_data(int type, char *buffer, int length)
{
	...
	FTS_TEST_INFO("Tx:%d Rx:%d, iLen:%d, buffer length:%d", g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum,
		      iLen, length);
	...
	for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; iRow++) {
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++) {
			...
			if (iLen >= length)
				break;
			if (iCol == (g_stSCapConfEx.ChannelXNum - 1))
	...
	return iLen;
TEST_ERR:
	FTS_TEST_INFO("get RawData failed.");
	return iLen;
}
...
boolean FT8006_StartTest(void)
{
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++) {
		...
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT8006_ENTER_FACTORY_MODE) {
			ReCode = FT8006_TestItem_EnterFactoryMode(&g_FT8006);
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				goto TEST_ERR;	/*if this item FAIL, no longer test. */
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}

		/*FT8006_RAWDATA_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT8006_RAWDATA_TEST) {
			ReCode = FT8006_TestItem_RawDataTest(&g_FT8006, &bTempResult);
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				g_int_tptest_result =
				    g_int_tptest_result | TEST_BEYOND_MAX_LIMIT | TEST_BEYOND_MIN_LIMIT;
				FTS_TEST_DBG("tpd rawdata failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}

		/*FT8006_CB_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT8006_CB_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				g_int_tptest_result =
				    g_int_tptest_result | TEST_BEYOND_MAX_LIMIT | TEST_BEYOND_MIN_LIMIT;
				FTS_TEST_DBG("tpd scapcb failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}

		/*FT8006_SHORT_CIRCUIT_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT8006_SHORT_CIRCUIT_TEST) {
			ReCode = FT8006_TestItem_ShortCircuitTest(&g_FT8006, &bTempResult);
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				g_int_tptest_result = g_int_tptest_result | TEST_GT_SHORT;
				FTS_TEST_DBG("tpd short circuit failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}

		/*FT8006_LCD_NOISE_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT8006_LCD_NOISE_TEST) {
			ReCode = FT8006_TestItem_LCDNoiseTest(&g_FT8006, &bTempResult);
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				bTestResult = false;
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_NG;
				FTS_TEST_DBG("tpd LCD noise failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
...
void CatSingleToOneScreen(struct FT8006 *g_FT8006)
{
	...
	if (GetCSDirection(g_FT8006->m_csInfo) == CS_LEFT_RIGHT) {
		...
		for (iRow = 0; iRow < GetMasterTx(g_FT8006->m_csInfo); ++iRow) {
			for (iCol = 0; iCol < GetMasterRx(g_FT8006->m_csInfo) + GetSlaveRx(g_FT8006->m_csInfo);
				++iCol) {
				iRelativeRx = iCol - splitRx;
				iTotalRx = GetMasterRx(g_FT8006->m_csInfo) + GetSlaveRx(g_FT8006->m_csInfo);
				if (iCol >= splitRx) {
					/*right data */
					bufferCated[iRow * iTotalRx + iCol] =
					    bufferRight[iRow * (iTotalRx - splitRx) + iRelativeRx];
				} else {
					/*left data */
		...
		memcpy(bufferCated +
		       GetMasterTx(g_FT8006->m_csInfo) * (GetMasterRx(g_FT8006->m_csInfo) +
							  GetSlaveRx(g_FT8006->m_csInfo)), bufferLeft + leftChannelNum,
		       6);
		memcpy(bufferCated +
		       GetMasterTx(g_FT8006->m_csInfo) * (GetMasterRx(g_FT8006->m_csInfo) +
							  GetSlaveRx(g_FT8006->m_csInfo)) + 6,
		       bufferRight + rightChannelNum, 6);
	}
		...
		for (iRow = 0; iRow < GetMasterTx(g_FT8006->m_csInfo) + GetSlaveTx(g_FT8006->m_csInfo); ++iRow) {
			for (iCol = 0; iCol < GetMasterRx(g_FT8006->m_csInfo); ++iCol) {
				...
				if (iRow >= splitTx) {
					/*down data */
					bufferCated[iRow * GetMasterRx(g_FT8006->m_csInfo) + iCol] =
					    bufferDown[iRelativeTx * GetMasterRx(g_FT8006->m_csInfo) + iCol];
				} else {
					/*up data */
					bufferCated[iRow * GetMasterRx(g_FT8006->m_csInfo) + iCol] =
					    bufferUp[iRow * GetMasterRx(g_FT8006->m_csInfo) + iCol];
				}
			}
		}

		memcpy(bufferCated +
		       (GetMasterTx(g_FT8006->m_csInfo) +
			GetSlaveTx(g_FT8006->m_csInfo)) * GetMasterRx(g_FT8006->m_csInfo), bufferUp + upChannelNum, 6);
		memcpy(bufferCated +
		       (GetMasterTx(g_FT8006->m_csInfo) +
			GetSlaveTx(g_FT8006->m_csInfo)) * GetMasterRx(g_FT8006->m_csInfo) + 6,
		       bufferDown + downChannelNum, 6);
	}
...
unsigned char FT8006_TestItem_RawDataTest(struct FT8006 *g_FT8006, bool *bTestResult)
{
    ...
    /*-------------------------------------To Determine RawData if in Range or not*/
	 /*VA*/ for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; iRow++) {
		...
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++) {
			...
			if (iValue < RawDataMin || iValue > RawDataMax) {
				btmpresult = false;
				fts_save_failed_node(iRow, iCol);
				FTS_TEST_INFO
	...
	for (iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; iCol++) {
		...
		if (iValue < RawDataMin || iValue > RawDataMax) {
			btmpresult = false;
			fts_save_failed_node(iRow, iCol);
			FTS_TEST_INFO("rawdata test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d)",
				      iRow + 1, iCol + 1, iValue, RawDataMin, RawDataMax);
		}
	}
	TestResultLen += snprintf(TestResult + TestResultLen, BUFF_LEN_TMP_BUFFER - TestResultLen,
		"RawData Test is %s.\n\n", (btmpresult ? "OK" : "NG"));
...
unsigned char FT8006_TestItem_CbTest(struct FT8006 *g_FT8006, bool *bTestResult)
{
	...
	ReCode =
	    FT8006_GetTxRxCB(g_FT8006, 0,
			     (short)(g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum +
	...
	for (iCol = 0; iCol < g_stSCapConfEx.KeyNumTotal; ++iCol) {
		m_CBData[g_stSCapConfEx.ChannelXNum][iCol] =
		    m_ucTempData[g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum + iCol];
...
TEST_ERR:
	...
	*bTestResult = false;
	...
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, "CB Test is NG.\n\n");
	return ReCode;
}
	...
	for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; iRow++) {
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++) {
			...
			if (iValue < iValueMin || iValue > iValueMax) {
				bTempResult = false;
				FTS_TEST_ERROR
				    (" Short Circuit test failure. Node=(%d, %d), Get_value=%d, Set_Range=(%d, %d).\n",
...
TEST_END:
	...
	TestResultLen +=
	    snprintf(TestResult + TestResultLen, BUFF_LEN_TMP_BUFFER - TestResultLen,
	    "Short Circuit Test is %s.\n\n", (bTempResult ? "OK" : "NG"));
...
unsigned char FT8006_TestItem_LCDNoiseTest(struct FT8006 *g_FT8006, bool *bTestResult)
{
	...
	/*set scan number */
	ReCode =
	    FT8006_WriteReg(g_FT8006, REG_8006_LCD_NOISE_FRAME,
			    g_stCfg_FT8006_BasicThreshold.iLCDNoiseTestFrame & 0xff);
	ReCode =
	    FT8006_WriteReg(g_FT8006, REG_8006_LCD_NOISE_FRAME + 1,
			    (g_stCfg_FT8006_BasicThreshold.iLCDNoiseTestFrame >> 8) & 0xff);
	...
	ReadBytesByKey(0x6A,
		       g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum * 2 + g_stSCapConfEx.KeyNumTotal * 2,
		       ScreenNoiseData);
	for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum; ++iRow) {
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; ++iCol) {
			index = iRow * g_stSCapConfEx.ChannelYNum + iCol;
			m_RawData[iRow][iCol] =
			    (int)(short)((ScreenNoiseData[index * 2] << 8) + ScreenNoiseData[index * 2 + 1]);
		}
	}
	...
	for (iRow = 0; iRow < g_stSCapConfEx.ChannelXNum + 1; iRow++) {
		for (iCol = 0; iCol < g_stSCapConfEx.ChannelYNum; iCol++) {
			...
			if (iValue < iValueMin || iValue > iValueMax) {
				bResultFlag = false;
				FTS_TEST_ERROR
				    (" LCD Noise test failure. Node=(%d,  %d), Get_value=%d,  Set_Range=(%d, %d).\n",
				     iRow + 1, iCol + 1, iValue, iValueMin, iValueMax);
			}
		}
	}
...
* Input: int iData[TX_NUM_MAX][RX_NUM_MAX], int iArrayIndex, unsigned char Row, unsigned char Col,
* unsigned char ItemCount
* Output: none
* Return: none
***********************************************************************/
	...
	for (i = 0 + iArrayIndex; (i < Row + iArrayIndex) && (i < TX_NUM_MAX); i++) {
		for (j = 0; (j < Col) && (j < RX_NUM_MAX); j++) {
			if (j == (Col - 1))	/*The Last Data of the Row, add "\n" */
				...
				iLen = snprintf(g_pTmpBuff, 128, "%d, ", iData[i][j]);
...
static unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum - 1) {	/*last packet */
			focal_msleep(10);
			ReCode =
			    Comm_Base_IIC_IO(NULL, 0, pReadData + BYTES_PER_TIME * i, ByteNum - BYTES_PER_TIME * i);
		} else {
			focal_msleep(10);
				      ...
				      unsigned char *pReadBuffer)
{
	...
	if (ChipHasTwoHeart(g_FT8006)) {
		...
		WorkAsMaster(&mgr);
		ReCode =
		    GetTxRxCB(0, GetMasterTx(g_FT8006->m_csInfo) * GetMasterRx(g_FT8006->m_csInfo) + 6,
			      (unsigned char *)bufferMaster);

		WorkAsSlave(&mgr);
		ReCode =
		    GetTxRxCB(0, GetSlaveTx(g_FT8006->m_csInfo) * GetSlaveRx(g_FT8006->m_csInfo) + 6,
			      (unsigned char *)bufferSlave);
...
GET_KEY_NUMBER_OK:
	...
	/*zhangjian add */
	g_ScreenSetParam.iTxNum = g_stSCapConfEx.ChannelXNum;
	g_ScreenSetParam.iRxNum = g_stSCapConfEx.ChannelYNum;
	FTS_TEST_INFO("CH_X = %d, CH_Y = %d, Key = %d", g_stSCapConfEx.ChannelXNum, g_stSCapConfEx.ChannelYNum,
		      g_stSCapConfEx.KeyNum);
	return ReCode;
}
...
static unsigned char GetRawData(struct FT8006 *g_FT8006)
{
	...
	ReCode =
	    FT8006_ReadRawData(g_FT8006, 0, 0xAD, g_stSCapConfEx.ChannelXNum * g_stSCapConfEx.ChannelYNum * 2,
			       m_iTempRawData);
	if (ReCode != ERROR_CODE_OK) {
