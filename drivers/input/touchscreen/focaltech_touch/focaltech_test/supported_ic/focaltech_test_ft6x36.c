...
/*1 Byte read and write (RW)  TP_Channel_Num  The maximum number of channels in VA area is 63 */
#define C6X36_CHANNEL_NUM   0x0A
/*1 Byte read and write(RW) TP_Key_Num The maximum number of virtual keys independent channel is 63 */
#define C6X36_KEY_NUM       0x0B
/*1 Byte    read and write(RW)  CB_addr   Normal mode CB-- address  */
#define C6X36_CB_ADDR_W 0x32
/*1 Byte  read and write(RW)  CB_addr     Normal mode CB-- address */
#define C6X36_CB_ADDR_R 0x33
/*Byte only read (RO)  CB_buf The channel is 2 bytes, and the length is 2*N */
#define C6X36_CB_BUF  0x39
/*1 Byte    read and write(RW)  RawData_addr Rawdata--address  */
#define C6X36_RAWDATA_ADDR  0x34
#define C6X36_RAWDATA_BUF   0x35
/*0: normal factory model, 1: production test factory mode 1 (using a single  +0 level of water scanning),
2: production test factory mode 2 (using a single ended + non waterproof scanning) */
...
static void Save_Test_Data(int iData[MAX_SCAP_CHANNEL_NUM], int iArrayIndex, unsigned char Row, unsigned char Col,
...
/*tp test*/
int FT6X36_get_tp_channel(int *buffer)
{
...
int FT6X36_get_tp_node_data(int type, char *buffer, int length)
{
	int i = 0;
	int iLen = 0;
	int iValue = 0;
	...
	for (i = 0; i < g_ScreenSetParam.iChannelsNum; i++) {
		iValue = m_RawData[i];
		FTS_TEST_INFO("m_RawData[i] is %d.\n", iValue);
		if (iLen >= length)
			break;
...
boolean FT6X36_StartTest(void)
{
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++) {
		...
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT6X36_ENTER_FACTORY_MODE) {
			ReCode = FT6X36_TestItem_EnterFactoryMode();
		...
		/*FT6X36_RAWDATA_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT6X36_RAWDATA_TEST) {
		...
		/*FT6X36_CB_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT6X36_CB_TEST) {
		...
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT6X36_DELTA_CB_TEST) {
...
unsigned char FT6X36_TestItem_RawDataTest(bool *bTestResult)
{
	...
	for (i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++) {
		...
		if (m_RawData[i] < RawDataMin || m_RawData[i] > RawDataMax) {
			btmpresult = false;
			fts_save_failed_node(i, 0);
			if (iNgNum == 0)
				FTS_TEST_DBG("\r\n ======= NG Data: \r");
			if (i < g_ScreenSetParam.iChannelsNum)
				FTS_TEST_DBG("Ch_%02d: %d Set_Range=(%d, %d) ,	", i + 1, m_RawData[i], RawDataMin,
					     RawDataMax);
			else
				FTS_TEST_DBG("Key_%d: %d Set_Range=(%d, %d) ,	",
					     i + 1 - g_ScreenSetParam.iChannelsNum, m_RawData[i], RawDataMin,
					     RawDataMax);
			if (iNgNum % 6 == 0) {
				FTS_TEST_DBG("\r\n");
			}
...
TEST_END:
	...
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, " RawData Test is %s.\n\n", (btmpresult ? "OK" : "NG"));
	return ReCode;
}
	...
	/*----------------------------------------------------------Judge whether or not to exceed the threshold*/
		...
		for (i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++) {
			...
			if (m_TempCbData[i] < iCbMin || m_TempCbData[i] > iCbMax) {
				...
				btmpresult = false;
				if (i < g_ScreenSetParam.iChannelsNum)
					FTS_TEST_DBG("Ch_%02d: %d Set_Range=(%d, %d) ,	", i + 1, m_TempCbData[i],
						     iCbMin, iCbMax);
				else
					FTS_TEST_DBG("Key_%d: %d Set_Range=(%d, %d),	",
						     i + 1 - g_ScreenSetParam.iChannelsNum, m_TempCbData[i], iCbMin,
						     iCbMax);
				if (iNgNum % 6 == 0) {
					FTS_TEST_DBG("\r");
				}
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char *bTestResult)
{
	...
	if (g_stCfg_FT6X36_BasicThreshold.DeltaCbTest_Include_Key_Test) {
		...
		for (i = g_ScreenSetParam.iChannelsNum; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum;
		     i++) {
...
static unsigned char GetChannelNum(void)
{
	...
	/*zhangjian add */
	g_ScreenSetParam.iTxNum = g_ScreenSetParam.iChannelsNum;
