...
int FT5X46_get_tp_channel(int *buffer)
{
	int retval = 0;
	static int tx = 0, rx = 0, onetimes = 0;
	...
	if (onetimes == 0) {
		...
		if (ReCode != ERROR_CODE_OK) {
			retval = -1;
			goto enter_factory_failed;
		}
		tx = g_ScreenSetParam.iTxNum;
		rx = g_ScreenSetParam.iRxNum;
		onetimes = 1;
	}
	...
	return retval;
enter_factory_failed:
	onetimes = 0;
	return retval;
}

int FT5X46_get_tp_node_data(int type, char *buffer, int length)
{
	unsigned char ReCode = 0;
	...
	FTS_TEST_INFO("Tx:%d Rx:%d, iLen:%d, buffer length:%d",
		      g_ScreenSetParam.iTxNum, g_ScreenSetParam.iRxNum, iLen, length);
	...
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
			...
			if (iCol == (g_ScreenSetParam.iRxNum - 1))
...
boolean FT5X46_StartTest(void)
{
	...
	for (iItemCount = 0; iItemCount < g_TestItemNum; iItemCount++) {
		...
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_ENTER_FACTORY_MODE) {
		...
		/* FT5X46_RAWDATA_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_RAWDATA_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				FTS_TEST_INFO("tpd rawdata failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}

		/* FT5X46_SCAP_CB_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_SCAP_CB_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_MAX_LIMIT
				    | TEST_BEYOND_MIN_LIMIT;
				FTS_TEST_INFO("tpd scapcb failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
		/*FT5X46_SCAP_RAWDATA_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_SCAP_RAWDATA_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_MAX_LIMIT
					| TEST_BEYOND_MIN_LIMIT;
				FTS_TEST_INFO("tpd scaprawdata failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
		/*  Code_FT5X46_PANELDIFFER_TEST, */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_PANELDIFFER_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				g_int_tptest_result = g_int_tptest_result | TEST_BEYOND_MAX_LIMIT
					| TEST_BEYOND_MIN_LIMIT;
				FTS_TEST_INFO("tpd differdata failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
		/* Code_FT5X46_WEAK_SHORT_CIRCUIT_TEST */
		if (g_stTestItem[ucDevice][iItemCount].ItemCode == Code_FT5X46_WEAK_SHORT_CIRCUIT_TEST) {
			...
			if (ReCode != ERROR_CODE_OK || (!bTempResult)) {
				...
				FTS_TEST_INFO("tpd weakshort failed\n");
			} else
				g_stTestItem[ucDevice][iItemCount].TestResult = RESULT_PASS;
		}
			...
			for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
				...
				for (iCol = 0; iCol < g_ScreenSetParam.iRxNum;
					...
					if (iValue < RawDataMin
						...
						fts_save_failed_node(iRow, iCol);
						FTS_TEST_INFO
						("rawdata failure. Node=(%d,  %d), Get_value=%d, Set_Range=(%d, %d) ",
...
TEST_ERR:
	...
	*bTestResult = false;
	...
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, " RawData Test is NG.\n\n");
	return ReCode;
...
TEST_ERR:
	*bTestResult = false;
	FTS_TEST_INFO("\n\n//SCap RawData Test is NG!");
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, " SCap RawData Test is NG.\n\n");
	return ReCode;
}
...
TEST_ERR:
	...
	*bTestResult = false;
	FTS_TEST_INFO("\n\n//SCap CB Test Test is NG!");
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, " SCap CB Test is NG.\n\n");
	return ReCode;
}
...
unsigned char FT5X46_TestItem_PanelDifferTest(bool *bTestResult)
{
	...
	for (i = 0; i < g_ScreenSetParam.iTxNum; i++) {
		...
		for (j = 0; j < g_ScreenSetParam.iRxNum; j++) {
			...
			if (g_stCfg_MCap_DetailThreshold.InvalidNode[i][j] == NODE_AST_TYPE
			    || g_stCfg_MCap_DetailThreshold.InvalidNode[i][j] == NODE_INVALID_TYPE) {
				InvalidNum++;
				continue;
			}
...
TEST_ERR:
	...
	*bTestResult = false;
	FTS_TEST_INFO("//Panel Differ Test is NG!");
	TestResultLen += snprintf(TestResult + TestResultLen,
		BUFF_LEN_TMP_BUFFER - TestResultLen, " Panel Differ Test is NG.\n\n");
	return ReCode;
}
...
unsigned char FT5X46_TestItem_WeakShortTest(bool *bTestResult)
{
	...
	int fKcal = 0;
	/* loat *fMShortResistance, *fGShortResistance; */
	...
	#if 0 /* zte modify fix weakshort test failed*/
	for (i = 0; i < 5; i++) {
		ReCode = StartScan();
		if (ReCode != ERROR_CODE_OK) {
			FTS_TEST_ERROR("StartScan Failed!\n");
	...
	for (i = 0; i < iChannelNum; i++) {
		...
		if (IcValue <= 0x05 || IcValue == 0xff) {
			...
			/* ( ( iDsen - iDoffset + 384 ) * iRsen / (iDrefn - iDsen) ) * fKcal - 1.2; */
		...
		if ((iMin_CG > fGShortResistance[i]) || (iDsen - iDoffset < 0))	 {
			iCount++;
			if (i + 1 <= iMaxTx)
				FTS_TEST_DBG("Tx%02d: %02d,",  i + 1, fGShortResistance[i]);
			else
				FTS_TEST_DBG("Rx%02d: %02d,", i + 1 - iMaxTx, fGShortResistance[i]);
			if (iCount % 10 == 0)
				FTS_TEST_DBG("\n");
		}
			  ...
			  int ByteNum, int *pRevBuffer)
{
	...
	for (i = 1; i < iReadNum; i++) {
		...
		if (i == iReadNum - 1) {
			focal_msleep(10);
			ReCode = Comm_Base_IIC_IO(NULL, 0, m_ucTempData + BYTES_PER_TIME * i,
...
unsigned char GetTxSC_CB(unsigned char index, unsigned char *pcbValue)
{
	...
	if (index < 128) {
		*pcbValue = 0;
		...
		/* Sequential Read length index-128 */
...
static unsigned char GetRawData(void)
{
	...
	if ((g_ScreenSetParam.iTxNum + g_ScreenSetParam.iRxNum) == 0) {
		ReCode = GetChannelNum();
		if (ReCode != ERROR_CODE_OK) {
	...
	memset(m_RawData, 0, sizeof(m_RawData));
	ReCode =
	    ReadRawData(1, 0xAA, (g_ScreenSetParam.iTxNum * g_ScreenSetParam.iRxNum) * 2, m_iTempRawData);
	for (iRow = 0; iRow < g_ScreenSetParam.iTxNum; iRow++) {
		for (iCol = 0; iCol < g_ScreenSetParam.iRxNum; iCol++) {
...
/* unsigned char WeakShort_GetAdcData( int AllAdcDataLen, int *pRevBuffer, int iCommMode,unsigned char chCmd ) */
static unsigned char WeakShort_GetAdcData(int AllAdcDataLen, int *pRevBuffer)
{
	unsigned char ReCode = ERROR_CODE_COMM_ERROR;
	...
	FTS_TEST_DBG(" WeakShort_GetAdcData SysDelay(200).\n");
	SysDelay(200);
	...
	iReadNum = iReadDataLen / WEAK_SHORT_BYTES_PER_TIME;

	if ((iReadDataLen % WEAK_SHORT_BYTES_PER_TIME) != 0)
		iReadNum++;

	if (iReadDataLen <= WEAK_SHORT_BYTES_PER_TIME) {
		BytesNumInTestMode1 = iReadDataLen;
	} else {
		BytesNumInTestMode1 = WEAK_SHORT_BYTES_PER_TIME;
	}

	FTS_TEST_DBG(" iReadDataLen is %d ", iReadDataLen);
	ReCode = Comm_Base_IIC_IO(pDataSend, 1, pDataSend + 1, BytesNumInTestMode1);
	if (ReCode != ERROR_CODE_OK) {
		FTS_TEST_DBG(" WeakShort_GetAdcData Comm_Base_IIC_IO Failed!1 ");
	}
	for (i = 1; i < iReadNum; i++) {
		if (ReCode != ERROR_CODE_OK)
			break;
		...
		if (i == iReadNum - 1) {
			focal_msleep(10);
			ReCode =
			    Comm_Base_IIC_IO(NULL, 0, pDataSend + 1 +  WEAK_SHORT_BYTES_PER_TIME * i,
					     iReadDataLen - WEAK_SHORT_BYTES_PER_TIME * i);
			if (ReCode != ERROR_CODE_OK) {
				FTS_TEST_DBG(" WeakShort_GetAdcData Comm_Base_IIC_IO Failed!2 ");
			}
		} else {
			focal_msleep(10);
			ReCode = Comm_Base_IIC_IO(NULL, 0,  pDataSend + 1 + WEAK_SHORT_BYTES_PER_TIME * i,
					     WEAK_SHORT_BYTES_PER_TIME);

			if (ReCode != ERROR_CODE_OK) {
				FTS_TEST_DBG(" WeakShort_GetAdcData Comm_Base_IIC_IO Failed!3 ");
			}
		}
