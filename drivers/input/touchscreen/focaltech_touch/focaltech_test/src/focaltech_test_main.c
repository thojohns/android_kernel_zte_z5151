...
/*tp test*/
int tpd_test_get_channel_setting(int *buffer)
{
	int retval = 0;
	...
	if (g_stTestFuncs.get_tp_channel) {
		retval = g_stTestFuncs.get_tp_channel(buffer);
	}
...
int tpd_test_get_tp_node_data(int type, char *buffer, int length)
{
	int retval = 0;
	...
	if (g_stTestFuncs.get_tp_node_data) {
		retval = g_stTestFuncs.get_tp_node_data(type, buffer, length);
	}
