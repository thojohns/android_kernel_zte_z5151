...
static ST_INI_FILE_DATA *g_st_ini_file_data = NULL;
static int g_used_key_num = 0;
static char CFG_SSL = '[';	/* Can be defined according to the special need to change. For example {}  */
static char CFG_SSR = ']';	/* Can be defined according to the special need to change. For example {} */
...
/*return CFG_ERR_OPEN_FILE;*/
	...
	while (1) {		/*find section */
		...
		if (buf1[0] == CFG_SSL) {
			...
			buf1[n - 1] = 0x00;
			strlcpy((char *)tmpSectionName, buf1 + 1, n);
			/*FTS_TEST_DBG("Section Name:%s, Len:%d\n",  tmpSectionName, n-2);*/

			continue;
		}
		/*get section name end*/
		/* strlcpy will add '/0' at end ,so modify strlen(tmpSectionName) + 1 */
		strlcpy(g_st_ini_file_data[g_used_key_num].pSectionName, tmpSectionName, strlen(tmpSectionName) + 1);
		...
		memcpy(g_st_ini_file_data[g_used_key_num].pKeyValue, buf1 + iEqualSign + 1,
