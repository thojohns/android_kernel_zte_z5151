...
/*test result data. (pTestData, External application for memory, buff size >= 1024*80)*/
...
extern int fts_save_failed_node(int iRow, int iCol);
...
struct StTestFuncs {
	...
	/*tp test*/
	int (*get_tp_channel)(int *buffer);
	int (*get_tp_node_data)(int type, char *buffer, int length);
};
...
#elif ((FTS_CHIP_TEST_TYPE == FT3C47_TEST) || (FTS_CHIP_TEST_TYPE == FT5822_TEST) \
	|| (FTS_CHIP_TEST_TYPE == FT5X46_TEST) || (FTS_CHIP_TEST_TYPE == FT3D47_TEST))
#define IC_Capacitance_Type     1
#elif ((FTS_CHIP_TEST_TYPE == FT8606_TEST) || (FTS_CHIP_TEST_TYPE == FT8607_TEST) \
	|| (FTS_CHIP_TEST_TYPE == FT8716_TEST)  || (FTS_CHIP_TEST_TYPE == FT8736_TEST) \
	|| (FTS_CHIP_TEST_TYPE == FTE716_TEST) || (FTS_CHIP_TEST_TYPE == FTE736_TEST) \
