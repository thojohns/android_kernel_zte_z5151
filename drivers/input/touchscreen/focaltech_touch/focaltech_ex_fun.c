...
static ssize_t fts_debug_write(struct file *filp, const char __user *buff, size_t count, loff_t *ppos)
{
	...
	case PROC_WRITE_DATA:
		...
		if (writelen > 0) {
			ret =
			    fts_i2c_write(fts_i2c_client, writebuf + 1, writelen);
			if (ret < 0) {
				FTS_ERROR("[APK]: write iic error!!");
			}
		}
				      ...
				      struct device_attribute *attr, char *buf)
{
	...
	count += snprintf(buf + count, PAGE_SIZE, "FTS_DEBUG_EN: \t\t\t%s\n", FTS_DEBUG_EN ? "ON" : "OFF");
#if defined(FTS_MT_PROTOCOL_B_EN)
	count += snprintf(buf + count, PAGE_SIZE, "TS_MT_PROTOCOL_B_EN: \t\t%s\n", FTS_MT_PROTOCOL_B_EN ? "ON" : "OFF");
#endif
	count += snprintf(buf + count, PAGE_SIZE, "FTS_GESTURE_EN: \t\t%s\n", FTS_GESTURE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_ESDCHECK_EN: \t\t%s\n", FTS_ESDCHECK_EN ? "ON" : "OFF");
#if defined(FTS_PSENSOR_EN)
	count += snprintf(buf + count, PAGE_SIZE, "FTS_PSENSOR_EN: \t\t%s\n", FTS_PSENSOR_EN ? "ON" : "OFF");
#endif
	count += snprintf(buf + count, PAGE_SIZE, "FTS_GLOVE_EN: \t\t\t%s\n", FTS_GLOVE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_COVER_EN: \t\t%s\n", FTS_COVER_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_CHARGER_EN: \t\t\t%s\n", FTS_CHARGER_EN ? "ON" : "OFF");

	count += snprintf(buf + count, PAGE_SIZE, "FTS_REPORT_PRESSURE_EN: \t\t%s\n",
		    FTS_REPORT_PRESSURE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_FORCE_TOUCH_EN: \t\t%s\n",
		    FTS_FORCE_TOUCH_EN ? "ON" : "OFF");

	count += snprintf(buf + count, PAGE_SIZE, "FTS_TEST_EN: \t\t\t%s\n", FTS_TEST_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_APK_NODE_EN: \t\t%s\n", FTS_APK_NODE_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_POWER_SOURCE_CUST_EN: \t%s\n",
		    FTS_POWER_SOURCE_CUST_EN ? "ON" : "OFF");
	count += snprintf(buf + count, PAGE_SIZE, "FTS_AUTO_UPGRADE_EN: \t\t%s\n",
				 ...
				 struct device_attribute *attr, char *buf)
{
	...
	count = snprintf(buf, PAGE_SIZE, "Log: %s\n", g_show_log ? "On" : "Off");
