...
#define LIC_ECC_REG_H_OFF           0x454
#define LIC_ECC_REG_L_OFF           0x455
...
static int read_replace_3gamma(struct i2c_client *client, u8 *buf, bool flag)
{
	...
	for (i = 0; i < 2; i++) {
		...
		tmpbuf[LIC_LCD_ECC_L_OFF] = (u8) (initcode_ecc >> 8);
		tmpbuf[LIC_LCD_ECC_H_OFF] = (u8) (initcode_ecc);
		tmpbuf[LIC_ECC_REG_L_OFF] = (u8) (initcode_ecc >> 8);
		tmpbuf[LIC_ECC_REG_H_OFF] = (u8) (initcode_ecc);
		...
		tmpbuf[LIC_CHECKSUM_L_OFF] = (u8) (initcode_checksum >> 8);
		tmpbuf[LIC_CHECKSUM_H_OFF] = (u8) (initcode_checksum);
	}
