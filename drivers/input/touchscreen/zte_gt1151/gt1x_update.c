...
int gt1x_check_firmware(void)
{
	...
	GTP_INFO("Firmware patch: %02X%02X%02X", update_info.firmware->version[0],
				update_info.firmware->version[1], update_info.firmware->version[2]);
...
int gt1x_update_judge(void)
{
	...
	fw_patch_id = fw_ver_info.patch_id;
	/* check fw status reg */
	do {
...
int gt1x_read_flash(u32 addr, int length)
{
	...
	if (ret) {
		GTP_ERROR("Error occurred.");	/*comment*/
		return ret;
	}
