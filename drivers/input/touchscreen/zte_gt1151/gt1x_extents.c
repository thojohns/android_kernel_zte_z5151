...
#define GESTURE_NODE "touchscreen/wake_gesture"
...
struct proc_dir_entry *_gProcTouchscreenInfo = NULL;
...
struct mutex gesture_weakup_mutex; /* lock for gesture weak up 20170516 chen add*/
...
static ssize_t gt1x_gesture_data_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	ptr += snprintf(ptr, 32, "gesture enable is %c\n", gesture_enabled);
	#if 0
	if (size == 4) {
		ret = copy_to_user(((u8 __user *) page), "GT1X", 4);
		return 4;
	}
	ret = simple_read_from_buffer(page, size, ppos, &gesture_enabled, sizeof(gesture_enabled));
...
int gesture_enter_doze(void)
{
	...
	if (!mutex_trylock(&gesture_weakup_mutex)) {/*20170516 chen add*/
		mutex_unlock(&gesture_weakup_mutex);
		return -EPERM;
	}
...
s32 gesture_event_handler(struct input_dev *dev)
{
	...
	*/
	...
	if (ges_type == 0 || !QUERYBIT(gestures_flag, ges_type)) {
		...
		if (gesture_doze_status == DOZE_DISABLED) {
		    GTP_INFO("Can not deal with this irq.\n");
			return -EPERM;
		}
		doze_buf[0] = 0x00;
...
static s32 hotknot_block_rw(u8 rqst_hotknot_state, s32 wait_hotknot_timeout)
{
	...
	if (wait_hotknot_timeout <= 0) {
		...
		wait_event_interruptible_timeout(bp_waiter, force_wake_flag ||
			rqst_hotknot_state == (got_hotknot_state & rqst_hotknot_state), wait_hotknot_timeout);
	}
...
s32 hotknot_event_handler(u8 *data)
{
	...
	if (hotknot_paired_flag) {
		...
		GTP_DEBUG("[0x8800~0x8803]=0x%x,0x%x,0x%x,0x%x", hn_state_buf[0],
					hn_state_buf[1], hn_state_buf[2], hn_state_buf[3]);
...
s32 gt1x_init_node(void)
{
	...
	mutex_init(&gesture_weakup_mutex);	/*20170516 chen add*/
	memset(gestures_flag, 0xff, sizeof(gestures_flag));
	...
	if (_gProcTouchscreenInfo != NULL) {
		...
		/*GTP_ERROR("CAN't create proc entry /proc/%s.", GESTURE_NODE);*/
			GTP_ERROR("Failed to create wake_gesture file node!\n");
			return -EPERM;
		}
	}
	/*GTP_INFO("Created proc entry /proc/%s.", GESTURE_NODE);*/
	GTP_ERROR("Create wake_gesture file node OK!\n");
#endif
