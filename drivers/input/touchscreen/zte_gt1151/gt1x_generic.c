...
extern struct mutex gesture_weakup_mutex;/*20170516 chen add*/
extern struct proc_dir_entry *_gProcTouchscreenInfo;
extern u32 fw_patch_id;
...
static s32 gt1x_init_info_node(void)
{
	...
	if (_gProcTouchscreenInfo == NULL) {
		GTP_ERROR("Create proc touchscreen FAILED!");
		return -EPERM;
	}
	gt1x_info_proc_entry = proc_create("touchscreen/ts_information", 0644, NULL, &gt1x_info_fops);
...
static ssize_t gt1x_info_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	/*len += snprintf(buffer + len, sizeof(buffer) - len, "==== GT1151 Version Info ====\n");*/
	len += snprintf(buffer + len, sizeof(buffer) - len, "Manufacturer : Goodix\n");
	len += snprintf(buffer + len, sizeof(buffer) - len, "chip type : gt1151\n");

	len += snprintf(buffer + len, sizeof(buffer) - len, "firmware version: %06X\n", gt1x_version.patch_id);
	if ((fw_patch_id & 0xFFFF) <= (gt1x_version.patch_id & 0xFFFF)) {
		GTP_INFO("The version of the fw is equal to the IC's!");
		len += snprintf(buffer + len, sizeof(buffer) - len, "Don't need update\n");
	} else {
		len += snprintf(buffer + len, sizeof(buffer) - len, "Do need update\n");
	}
	...
	case 0x0:
		len += snprintf(buffer + len, sizeof(buffer) - len, "module,Yili(0x%x)\n", buf[0]);/*yili*/
		break;
	case 0x1:
		len += snprintf(buffer + len, sizeof(buffer) - len, "module,Junda(0x%x)\n", buf[0]);/*junda*/
		break;
	default:
		len += snprintf(buffer + len, sizeof(buffer) - len, "module,Unknown(0x%x)\n", buf[0]);
		break;
	}

#if 0
	gt1x_i2c_read(GTP_REG_FW_CHK_MAINSYS, temp_data, 12);
	len += snprintf(buffer + len, sizeof(buffer) - len, "ProductID: GT%c%c%c%c\n",
					temp_data[0], temp_data[1], temp_data[2], temp_data[3]);
	len += snprintf(buffer + len, sizeof(buffer) - len, "PatchID: %02X%02X\n", temp_data[4], temp_data[5]);
	len += snprintf(buffer + len, sizeof(buffer) - len, "MaskID: %02X%02X\n", temp_data[7], temp_data[8]);
	len += snprintf(buffer + len, sizeof(buffer) - len, "SensorID: %02X\n", temp_data[10] & 0x0F);
#endif

	pr_info("%s: copy to user, len = %d\n", __func__, len);
...
static ssize_t gt1x_debug_read_proc(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
	...
	memset(buffer, 0, sizeof(buffer));
	len += snprintf(buffer + len, sizeof(buffer) - len, "==== GT1X default config setting in driver====\n");

	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		len += snprintf(buffer + len, sizeof(buffer) - len, "0x%02X,", gt1x_config[i]);
		if (i % 10 == 9)
	...
	len += snprintf(buffer + len, sizeof(buffer) - len, "==== GT1X config read from chip====\n");
	i = gt1x_i2c_read(GTP_REG_CONFIG_DATA, temp_data, GTP_CONFIG_MAX_LENGTH);
	GTP_INFO("I2C TRANSFER: %d", i);
	for (i = 0; i < GTP_CONFIG_MAX_LENGTH; i++) {
		len += snprintf(buffer + len, sizeof(buffer) - len, "0x%02X,", temp_data[i]);
	...
	len += snprintf(buffer + len, sizeof(buffer) - len, "\n");
	/* Touch PID & VID */
	len += snprintf(buffer + len, sizeof(buffer) - len, "==== GT1X Version Info ====\n");
...
static ssize_t gt1x_debug_write_proc(struct file *file, const char *buffer, size_t count, loff_t *ppos)
{
	...
	if (sscanf(buf, "%s %d", (char *)&mode_str, &mode) == 2) {
		GTP_INFO("Get the right para.");
	}
...
#endif
	if (sscanf(buf, "%s %s", (char *)&mode_str, (char *)&arg1) == 2) {
		GTP_INFO("get the right para.");
	}
	...
	while (retry--) {
		...
		if (!ret) {
			...
			if (checksum == 0 &&	/* first 3 bytes must be number or char */
				...
				buf[10] != 0xFF) {	/*sensor id == 0xFF, retry */
				break;
			}
...
static s32 gt1x_ps_operate(void *self, u32 command, void *buff_in, s32 size_in,
...
static int gt1x_ps_init(void)
{
	...
	/*  create sysfs files*/
	{
		...
		for (i = 0; i < ARRAY_SIZE(ps_attrs) / sizeof(ps_attrs[0]); i++) {
...
static void gt1x_ps_deinit(void)
{
	if (gt1x_ps_dev) {
		...
		for (; i < ARRAY_SIZE(ps_attrs) / sizeof(ps_attrs[0]); i++) {
...
#define _cfg_array_(n)   GTP_SMART_COVER_CFG_GROUP##n
	...
	do {
		...
		if (sensor_id >= ARRAY_SIZE(cfgs) / sizeof(cfgs[0])) {
...
int gt1x_resume(void)
{
	...
	mutex_lock(&gesture_weakup_mutex);		/*20170516 chen add*/
...
#endif
	gt1x_resume_flag = 0;
	mutex_unlock(&gesture_weakup_mutex);		/*20170516 chen add*/
