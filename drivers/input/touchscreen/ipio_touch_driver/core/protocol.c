...
static void free_func_hash(void)
{
	...
	for (i = 0; i < FUNC_NUM; i++) {
		if (hashArray[i] != NULL) {
			ipio_kfree((void **)&(hashArray[i]->cmd));
		}
...
static void create_func_hash(void)
{
	...
	insert_func(ILITEK_SENSE_CTRL_KEY, protocol->func_ctrl_len, protocol->sense_ctrl, "sense_ctrl");
	insert_func(ILITEK_SLEEP_CTRL_KEY, protocol->func_ctrl_len, protocol->sleep_ctrl, "sleep_ctrl");
	insert_func(ILITEK_GLOVE_CTRL_KEY, protocol->func_ctrl_len, protocol->glove_ctrl, "glove_ctrl");
	insert_func(ILITEK_STYLUS_CTRL_KEY, protocol->func_ctrl_len, protocol->stylus_ctrl, "stylus_ctrl");
	insert_func(ILITEK_TP_SCAN_MODE_KEY, protocol->func_ctrl_len, protocol->tp_scan_mode, "tp_scan_mode");
	insert_func(ILITEK_LPWG_CTRL_KEY, protocol->func_ctrl_len, protocol->lpwg_ctrl, "lpwg_ctrl");
	insert_func(ILITEK_GESTURE_CTRL_KEY, protocol->func_ctrl_len, protocol->gesture_ctrl, "gesture_ctrl");
	insert_func(ILITEK_PHONE_COVER_CTRL_KEY, protocol->func_ctrl_len,
		protocol->phone_cover_ctrl, "phone_cover_ctrl");
	insert_func(ILITEK_FINGER_SENSE_CTRL_KEY, protocol->func_ctrl_len,
		protocol->finger_sense_ctrl, "finger_sense_ctrl");
	insert_func(ILITEK_PHONE_COVER_WINDOW_KEY, protocol->window_len,
		protocol->phone_cover_window, "phone_cover_window");
	insert_func(ILITEK_FINGER_SENSE_CTRL1_KEY, protocol->func_ctrl_len,
		protocol->finger_sense_ctrl, "finger_sense_ctrl");
	insert_func(ILITEK_PROXIMITY_CTRL_KEY, protocol->func_ctrl_len, protocol->proximity_ctrl, "proximity_ctrl");
	insert_func(ILITEK_PLUG_CTRL_KEY, protocol->func_ctrl_len, protocol->plug_ctrl, "plug_ctrl");
}
