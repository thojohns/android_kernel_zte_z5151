...
#define PARSER_MAX_KEY_VALUE_LEN	360
...
int ini_file_buf_malloc(void)
{
	int ret = 0;

	ilitek_ini_file_data = kcalloc(PARSER_MAX_KEY_NUM, sizeof(struct ini_file_data), GFP_KERNEL);
	if (!ilitek_ini_file_data) {
		ipio_err("failed to alloc ilitek_ini_file_data\n");
		ret = -ENOMEM;
	}
...
void ini_file_buf_free(void)
{
	ipio_kfree((void **)&ilitek_ini_file_data);
}
...
/* char M_CFG_NIS = ':'; */
	...
	tmpSectionName = kzalloc((PARSER_MAX_KEY_NAME_LEN) * sizeof(char), GFP_KERNEL);
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				ini_buf[n - 1] = 0x00;
				strlcpy((char *)tmpSectionName, ini_buf + 1, PARSER_MAX_KEY_NAME_LEN);
				banchmark_flag = 0;
				nodetype_flag = 0;
		...
		strlcpy(ilitek_ini_file_data[g_ini_items].pSectionName, tmpSectionName,
		...
		if (isEqualSign == 0) {
			...
			if (strnstr(ilitek_ini_file_data[g_ini_items].pSectionName,
					"Benchmark_Data", PARSER_MAX_KEY_NAME_LEN) > 0) {
				banchmark_flag = 1;
				isEqualSign = -1;
			} else if (strnstr(ilitek_ini_file_data[g_ini_items].pSectionName,
					"Node Type", PARSER_MAX_KEY_NAME_LEN) > 0) {
				nodetype_flag = 1;
				isEqualSign = -1;
			} else {
				continue;
