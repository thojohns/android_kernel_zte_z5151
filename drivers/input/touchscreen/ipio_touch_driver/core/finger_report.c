...
struct mutual_touch_info {
	uint8_t touch_num;
	uint8_t key_code;
	struct mutual_touch_point mtp[MAX_TOUCH_NUM];
};
...
static int finger_report_ver_5_0(void)
{
...
#else /* INTERFACE == I2C_INTERFACE */
#ifdef ESD_CHECK
		if (res == I2C_ESD_CHECK) {
			ipio_err("I2C ESD check read %d !\n", res);
			if (i2c_esd_cnt++ > 5) {
				res = ilitek_platform_tp_hw_reset(true);
				i2c_esd_cnt = 0;
				if (res < 0)
					ipio_err("ESD recovery fail\n");
			}
		}
#endif
#endif
	...
	if (g_mutual_data.touch_num > 0) {
		...
		for (i = 0; i < g_mutual_data.touch_num; i++) {
			input_report_key(core_fr->input_device, BTN_TOUCH, 1);
			core_fr_touch_press(g_mutual_data.mtp[i].x, g_mutual_data.mtp[i].y,
				g_mutual_data.mtp[i].pressure, g_mutual_data.mtp[i].id);
...
void core_fr_handler(void)
{
	...
	while (i < ARRAY_SIZE(fr_t)) {
		if (protocol->major == fr_t[i].protocol_marjor_ver) {
			...
			if (ipd->debug_node_open || core_fr->isEnableNetlink) {
				...
				if (g_total_len < 2048) {
					...
					if (g_fr_uart != NULL)
						memcpy(tdata + g_fr_node->len,
							g_fr_uart->data, g_fr_uart->len);
				} else {
				...
				if (ipd->debug_node_open) {
					...
					memset(ipd->debug_buf[ipd->debug_data_frame], 0x00,
					       (uint8_t) sizeof(uint8_t) * ILITEK_DEBUG_BUFF_RANK_SIZE);
					...
					if (ipd->debug_data_frame > ILITEK_DEBUG_BUFF_ROW_SIZE - 1) {
						ipio_err("ipd->debug_data_frame = %d > 1024\n",
							ipd->debug_data_frame);
						ipd->debug_data_frame = ILITEK_DEBUG_BUFF_ROW_SIZE - 1;
					}
