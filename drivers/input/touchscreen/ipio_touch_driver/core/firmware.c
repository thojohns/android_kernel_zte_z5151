...
#include "mp_test.h"
	...
	static unsigned char CTPM_FW0[] = {
		#include ILITEK_UPGRADE_FW0_APP
	};

	static unsigned char CTPM_FW1[] = {
		#include ILITEK_UPGRADE_FW1_APP
	};

	static unsigned char CTPM_FW2[] = {
		#include ILITEK_UPGRADE_FW2_APP
	};

	static unsigned char CTPM_FW3[] = {
		#include ILITEK_UPGRADE_FW3_APP
	};
#endif
...
static uint32_t tddi_check_data(uint32_t start_addr, uint32_t end_addr)
{
	...
	if (timer >= 0) {
		...
		iram_check =  core_firmware->isCRC ? core_config_ice_mode_read(0x4101C) :
			core_config_ice_mode_read(0x041018);
	} else {
		ipio_err("TIME OUT\n");
		goto out;
	}
...
static int tddi_check_fw_upgrade(void)
{
	...
	ipio_info("New FW ver is no greater than old, need to check flash crc if it's correct\n");
...
static int do_program_flash(uint32_t start_addr)
{
	...
	ipio_info("%cUpgrading firmware ... start_addr = 0x%x, %02d%c",
...
static int iram_upgrade(void)
{
	...
	for (i = core_firmware->start_addr; i < core_firmware->end_addr; i += upl) {
		...
		ipio_info("%cupgrade firmware(ap code), %02d%c", 0x0D, core_firmware->update_status, '%');
...
static int write_download(uint32_t start, uint32_t size, uint8_t *w_buf, uint32_t w_len)
{
	...
	for (addr = start, i = 0; addr < end; addr += w_len, i += w_len) {
		...
		ipio_info("%cupgrade firmware(mp code), %02d%c", 0x0D, update_status, '%');
	}
...
int tddi_host_download(bool mode)
{
	...
	if (core_fr->actual_fw_mode == protocol->test_mode) {
		...
		if (mode) {
			...
			if (write_download(core_gesture->ap_start_addr, core_gesture->ap_length,
					gesture_ap_buf, core_gesture->ap_length) < 0) {
...
int core_firmware_boot_host_download(void)
{
	...
	core_gesture->ap_start_addr = (ap_fw[0xFFD3] << 24) + (ap_fw[0xFFD2] << 16)
			+ (ap_fw[0xFFD1] << 8) + ap_fw[0xFFD0];
	...
	core_gesture->area_section = (ap_fw[0xFFCF] << 24) + (ap_fw[0xFFCE] << 16)
			+ (ap_fw[0xFFCD] << 8) + ap_fw[0xFFCC];
...
int tddi_fw_upgrade(bool isIRAM)
{
	...
	ipio_msleep(core_firmware->delay_after_upgrade);
...
static int convert_hex_array(void)
{
	...
	case ILITEK_VENDOR_ID_0:
		CTPM_FW = CTPM_FW0;
		fw_len = (int)ARRAY_SIZE(CTPM_FW0);
		break;
	case ILITEK_VENDOR_ID_1:
		CTPM_FW = CTPM_FW1;
		fw_len = (int)ARRAY_SIZE(CTPM_FW1);
		break;
	case ILITEK_VENDOR_ID_2:
		CTPM_FW = CTPM_FW2;
		fw_len = (int)ARRAY_SIZE(CTPM_FW2);
		break;
	case ILITEK_VENDOR_ID_3:
		CTPM_FW = CTPM_FW3;
		fw_len = (int)ARRAY_SIZE(CTPM_FW3);
		break;
	default:
		CTPM_FW = CTPM_FW0;
		fw_len = (int)ARRAY_SIZE(CTPM_FW0);
	...
	ipio_info("id=%d, CTPM_FW size = %d\n", ilitek_vendor_id, fw_len);
	...
	/* Get hex fw vers */
	core_firmware->new_fw_cb = (CTPM_FW[FW_VER_ADDR] << 24) | (CTPM_FW[FW_VER_ADDR + 1] << 16) |
			(CTPM_FW[FW_VER_ADDR + 2] << 8) | (CTPM_FW[FW_VER_ADDR + 3]);
	...
	for (i = 0; i < g_total_sector; i++) {
		ipio_debug(DEBUG_FIRMWARE, "g_flash_sector[%d]: ss_addr = 0x%x, se_addr = 0x%x, length = %x\n",
				i, g_flash_sector[i].ss_addr, g_flash_sector[i].se_addr, g_flash_sector[index].dlength);
		ipio_debug(DEBUG_FIRMWARE, "data = %d, inside_block = %d\n",
...
int core_firmware_boot_upgrade(void)
{
	...
	if (protocol->mid >= 0x3) {
		...
		core_firmware->old_fw_cb = (core_firmware->old_fw_ver[0] << 24)
			| (core_firmware->old_fw_ver[1] << 16) | (core_firmware->old_fw_ver[2] << 8)
...
static int convert_hex_file(uint8_t *pBuf, uint32_t nSize, bool isIRAM)
{
				...
				if (isIRAM) {
					...
					if (nAddr < 0x10000) {
						...
						if (nAddr < DLM_HEX_ADDRESS + MAX_DLM_FIRMWARE_SIZE) {
							dlm_fw[nAddr - DLM_HEX_ADDRESS + k]
								= HexToDec(&pBuf[i + 9 + j], 2);
						}
					...
					if (nAddr > MAX_AP_FIRMWARE_SIZE && do_once == 0) {
						do_once = 1;
						core_gesture->ap_start_addr
								= (ap_fw[0xFFD3] << 24) + (ap_fw[0xFFD2] << 16)
								+ (ap_fw[0xFFD1] << 8) + ap_fw[0xFFD0];
						core_gesture->ap_length = MAX_GESTURE_FIRMWARE_SIZE;
						core_gesture->start_addr = (ap_fw[0xFFDB] << 24) + (ap_fw[0xFFDA] << 16)
								+ (ap_fw[0xFFD9] << 8) + ap_fw[0xFFD8];
						core_gesture->end_addr = (ap_fw[0xFFDF] << 24) + (ap_fw[0xFFDE] << 16)
								+ (ap_fw[0xFFDD] << 8) + ap_fw[0xFFDC];
						core_gesture->length = MAX_GESTURE_FIRMWARE_SIZE;
						core_gesture->area_section
								= (ap_fw[0xFFCF] << 24) + (ap_fw[0xFFCE] << 16)
								+ (ap_fw[0xFFCD] << 8) + ap_fw[0xFFCC];
						ipio_info("gesture_start_addr = 0x%x, gesture_end_addr = 0x%x, gesture_length = 0x%x\n",
									core_gesture->start_addr,
									core_gesture->end_addr,
									core_gesture->length);
					...
					if (nAddr >= core_gesture->start_addr
							&& nAddr < core_gesture->start_addr
							+ MAX_GESTURE_FIRMWARE_SIZE) {
						gesture_fw[nAddr - core_gesture->start_addr + k]
								= HexToDec(&pBuf[i + 9 + j], 2);
					}
#else
	...
	/* DEBUG: for showing data with address that will write into fw or be erased */
	for (i = 0; i < g_total_sector; i++) {
		ipio_debug(DEBUG_FIRMWARE, "g_flash_sector[%d]: ss_addr = 0x%x, se_addr = 0x%x\n",
			i, g_flash_sector[i].ss_addr, g_flash_sector[i].se_addr);

		ipio_debug(DEBUG_FIRMWARE, "length = %x, data = %d, inside_block = %d\n",
			g_flash_sector[index].dlength, g_flash_sector[i].data_flag,
			g_flash_sector[i].inside_block);
	}
...
int core_firmware_upgrade(const char *pFilePath, bool isIRAM)
{
	...
	if (isIRAM) {
		iram_fw = kcalloc(MAX_IRAM_FIRMWARE_SIZE, sizeof(uint8_t), GFP_KERNEL);
		if (ERR_ALLOC_MEM(iram_fw)) {
			ipio_err("Failed to allocate iram_fw memory, %ld\n", PTR_ERR(iram_fw));
			res = -ENOMEM;
			goto out;
		}
	}
