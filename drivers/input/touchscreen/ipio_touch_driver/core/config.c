...
#define ILITEK_READ_BUF_SIZE 128
...
uint32_t core_config_read_write_onebyte(uint32_t addr)
{
	int res = 0;
	uint32_t data = 0;
	uint8_t szOutBuf[4] = { 0 };

	szOutBuf[0] = 0x25;
	szOutBuf[1] = (uint8_t)((addr & 0x000000FF) >> 0);
	szOutBuf[2] = (uint8_t)((addr & 0x0000FF00) >> 8);
	szOutBuf[3] = (uint8_t)((addr & 0x00FF0000) >> 16);
...
{
	int res = 0, i;
	uint8_t szOutBuf[8] = { 0 };

	if (size > 4) {
		ipio_err("size(%d) is more than 4\n", size);
		res = -EINVAL;
		return res;
	}
...
void core_config_sense_ctrl(bool start)
{
	...
	return core_protocol_func_control(ILITEK_SENSE_CTRL_KEY, start);
}
...
void core_config_sleep_ctrl(bool out)
{
	...
	return core_protocol_func_control(ILITEK_SLEEP_CTRL_KEY, out);
}
...
void core_config_glove_ctrl(bool enable, bool seamless)
{
	...
	return core_protocol_func_control(ILITEK_GLOVE_CTRL_KEY, cmd);
}
...
void core_config_stylus_ctrl(bool enable, bool seamless)
{
	...
	return core_protocol_func_control(ILITEK_STYLUS_CTRL_KEY, cmd);
}
...
void core_config_tp_scan_mode(bool mode)
{
	...
	return core_protocol_func_control(ILITEK_TP_SCAN_MODE_KEY, mode);
}
...
void core_config_lpwg_ctrl(bool enable)
{
	...
	return core_protocol_func_control(ILITEK_LPWG_CTRL_KEY, enable);
}
...
void core_config_gesture_ctrl(uint8_t func)
{
	...
	return core_protocol_func_control(ILITEK_GESTURE_CTRL_KEY, func);
}
...
void core_config_phone_cover_ctrl(bool enable)
{
	...
	return core_protocol_func_control(ILITEK_PHONE_COVER_CTRL_KEY, enable);
}
...
void core_config_finger_sense_ctrl(bool enable)
{
	...
	return core_protocol_func_control(ILITEK_FINGER_SENSE_CTRL_KEY, enable);
}
...
void core_config_proximity_ctrl(bool enable)
{
	...
	return core_protocol_func_control(ILITEK_PROXIMITY_CTRL_KEY, enable);
}
...
void core_config_plug_ctrl(bool out)
{
	...
	return core_protocol_func_control(ILITEK_PLUG_CTRL_KEY, out);
}
...
int core_config_get_key_info(void)
{
	int res = 0, i;
	uint8_t cmd[2] = { 0 };
	uint8_t read_buf[ILITEK_READ_BUF_SIZE] = {0};
	...
	res = core_read(core_config->slave_i2c_addr, &read_buf[0], protocol->key_info_len);
	if (res < 0) {
...
int core_config_get_tp_info(void)
{
	...
	res = core_read(core_config->slave_i2c_addr, &read_buf[0], protocol->tp_info_len);
	if (res < 0) {
		ipio_err("Failed to read data via I2C, %d\n", res);
		goto out;
	}
...
int core_config_get_protocol_ver(void)
{
	...
	res = core_read(core_config->slave_i2c_addr, &read_buf[0], protocol->pro_ver_len);
	if (res < 0) {
		ipio_err("Failed to read data via I2C, %d\n", res);
		goto out;
	}
...
int core_config_get_core_ver(void)
{
	...
	res = core_read(core_config->slave_i2c_addr, &read_buf[0], protocol->core_ver_len);
	if (res < 0) {
		ipio_err("Failed to read data via I2C, %d\n", res);
		goto out;
	}
...
int core_config_get_fw_ver(void)
{
	...
	res = core_read(core_config->slave_i2c_addr, &read_buf[0], protocol->fw_ver_len);
	if (res < 0) {
	...
	if (protocol->mid >= 0x3) {
		ipio_info("Firmware Version = %d.%d.%d.%d\n", core_config->firmware_ver[1],
			core_config->firmware_ver[2], core_config->firmware_ver[3], core_config->firmware_ver[4]);
	} else {
