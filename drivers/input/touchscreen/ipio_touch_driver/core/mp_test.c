...
struct mp_test_items tItems[] = {
	{.name = "mutual_dac", .desp = "Untouch Calibration Data(DAC) - Mutual",
			.result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_bg", .desp = "Baseline Data(BG)", .result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_signal", .desp = "Untouch Signal Data(BG-Raw-4096) - Mutual",
			.result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_no_bk", .desp = "Untouch Raw Data(No BK) - Mutual", .result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_has_bk", .desp = "Raw Data(Have BK)", .result = "FAIL", .catalog = MUTUAL_TEST},
	{.name = "mutual_bk_dac", .desp = "Untouch Raw Data(Have BK) - Mutual",
			.result = "FAIL", .catalog = MUTUAL_TEST},
	...
	{.name = "self_signal", .desp = "Untouch Signal Data(BG-Raw-4096) - Self",
			.result = "FAIL", .catalog = SELF_TEST},
	...
	{.name = "rx_short", .desp = "Short Test (Rx)", .result = "FAIL", .catalog = SHORT_TEST},
	...
	{.name = "noise_peak_to_peak_ic", .desp = "Noise Peak to Peak(IC Only)",
			.result = "FAIL", .catalog = PEAK_TO_PEAK_TEST},
	{.name = "noise_peak_to_peak_panel", .desp = "Noise Peak To Peak(With Panel)",
			.result = "FAIL", .catalog = PEAK_TO_PEAK_TEST},

	{.name = "noise_peak_to_peak_ic_lcm_off", .desp = "Noise Peak to Peak(IC Only) (LCM OFF)",
			.result = "FAIL", .catalog = PEAK_TO_PEAK_TEST},
	...
	{.name = "mutual_has_bk_lcm_off", .desp = "Raw Data(Have BK) (LCM OFF)",
			.result = "FAIL", .catalog = MUTUAL_TEST},
	...
	{.name = "doze_p2p_td_lcm_off", .desp = "Peak To Peak_TD (LCM OFF)",
			.result = "FAIL", .catalog = PEAK_TO_PEAK_TEST},
};
...
static unsigned char g_str_save_file_path[FILE_PATH_SIZE];
static unsigned char g_str_ini_file_path[FILE_PATH_SIZE];
static unsigned char g_str_ini_filename[FILE_NAME_SIZE];
					 ...
					 const char *buf)
{
	...
	snprintf(g_str_save_file_path, FILE_PATH_SIZE, "%s", buf);

	ipio_info("save file path:%s\n", g_str_save_file_path);
					...
					const char *buf)
{
	...
	snprintf(g_str_ini_file_path, FILE_PATH_SIZE, "%s", buf);
...
static int tpd_test_filename_store(struct tpd_classdev_t *cdev, const char *buf)
{
	...
	snprintf(g_str_ini_filename, FILE_NAME_SIZE, "%s", buf);

	ipio_info("ini_name:%s\n", g_str_ini_filename);
...
{
	strlcpy(cdev->ic_tpinfo.tp_name, "Ilitek", 10);
	cdev->ic_tpinfo.chip_model_id = TS_CHIP_ILITEK;
	cdev->ic_tpinfo.chip_part_id = TP_TOUCH_IC;
	cdev->ic_tpinfo.module_id = ilitek_vendor_id;
	cdev->ic_tpinfo.chip_ver = 0;
	cdev->ic_tpinfo.firmware_ver = 0;
	cdev->ic_tpinfo.i2c_type = 0;
	cdev->ic_tpinfo.i2c_addr = 0x41;

	return 0;
}

int mp_path_init(void)
{
	ipio_info("\n");	/*show version*/

	strlcpy(g_str_save_file_path, CSV_FILE_PATH, FILE_PATH_SIZE);
	strlcpy(g_str_ini_file_path, INI_FILE_PATH, FILE_PATH_SIZE);
	strlcpy(g_str_ini_filename, INI_NAME_PATH, FILE_NAME_SIZE);
...
void get_ini_file_path(char *filepath, int count)
{
	snprintf(filepath, count, "%s%s", g_str_ini_file_path, g_str_ini_filename);
	ipio_info("filepath=%s\n", filepath);
}
...
void dump_data(void *data, int type, int len, int row_len, const char *name)
{
	...
	if (ipio_debug_level & DEBUG_MP_TEST) {
		...
		DUMP(DEBUG_MP_TEST, "\n\n");
		DUMP(DEBUG_MP_TEST, "Dump %s data\n", name);
		...
		for (i = 0; i < len; i++) {
			if (type == 8) {
				DUMP(DEBUG_MP_TEST, " %4x ", p8[i]);
			} else if (type == 32) {
				DUMP(DEBUG_MP_TEST, " %4x ", p32[i]);
			} else if (type == 10) {
				DUMP(DEBUG_MP_TEST, " %4d ", p32[i]);
			}
			if ((i % row) == row-1)
				DUMP(DEBUG_MP_TEST, "\n");
		}
...
static void mp_print_csv_header(char *csv, int *csv_len, int *csv_line)
{
	...
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len, "@ ILI TECHNOLOGY CORP. All Rights Reserved.\n");
	tmp_line++;
	tmp_len += snprintf(csv + tmp_len, CSV_FILE_SIZE - tmp_len,
		"==============================================================================\n");
	tmp_line++;
...
static void dump_benchmark_data(int32_t *max_ptr, int32_t *min_ptr)
{
	...
	if (ipio_debug_level & DEBUG_MP_TEST) {
		...
		for (i = 0; i < core_mp->frame_len; i++) {
			DUMP(DEBUG_MP_TEST, "%d, ", max_ptr[i]);
		...
		for (i = 0; i < core_mp->frame_len; i++) {
			DUMP(DEBUG_MP_TEST, "%d, ", min_ptr[i]);
			if (i % core_mp->xch_len == core_mp->xch_len - 1)
				DUMP(DEBUG_MP_TEST, "\n");
		}
	}
...
void dump_node_type_buffer(int32_t *node_ptr, uint8_t *name)
{
	...
	if (ipio_debug_level & DEBUG_MP_TEST) {
		for (i = 0; i < core_mp->frame_len; i++) {
			DUMP(DEBUG_MP_TEST, "%d, ", node_ptr[i]);
			if (i % core_mp->xch_len == core_mp->xch_len-1)
				DUMP(DEBUG_MP_TEST, "\n");
		}
	}
		...
		if (tItems[index].spec_option == BENCHMARK) {
			if (tItems[index].bench_mark_max == NULL) {
				...
				if (ERR_ALLOC_MEM(tItems[index].bench_mark_max)) {
					...
					tItems[index].bench_mark_max = NULL;
					return -ENOMEM;
				}
			}
			if (tItems[index].bench_mark_min == NULL) {
				...
				if (ERR_ALLOC_MEM(tItems[index].bench_mark_min)) {
					...
					tItems[index].bench_mark_min = NULL;
					return -ENOMEM;
				}
			}
		}
...
static void mp_calc_nodp(bool long_v)
{
	...
	if (!long_v) {
		...
		tmp = ((tshd << 2) - (at << 6) - tp_tsdh_wait
				- ddi_width * (multi_term_num - 1) - 64 - dp2tp - twc) * 5;
		...
		real_tx_dura = (((tshd << 2) - (at << 6) - ddi_width * (11) - 64 - dp2tp - twc) * 5
			- (phase << 5) - ((twcm * 5 + (phase << 5) + (tp2dp * 5 << 6)) * (11))) / (12 * 5);
...
static void run_pixel_test(int index)
{
	...
	for (y = 0; y < core_mp->ych_len; y++) {
		for (x = 0; x < core_mp->xch_len; x++) {
			...
			shift = y * core_mp->xch_len;
			centre = p_comb[shift + x];
	...
	int Charge_AA, int Charge_Border, int Charge_Notch)
{
	...
	for (k = 0; k < 8; k++) {
		...
		if ((tempX < 0) || (tempX >= core_mp->xch_len) || (tempY < 0)
				|| (tempY >= core_mp->ych_len)) /*out of range */
			continue;
...
int allnode_open_cdc_data(int mode, int *buf, int *dac)
{
	...
	char str[128] = {0};
	char tmp[128] = {0};
	static const char *const key[] = {"OPEN DAC", "OPEN Raw1", "OPEN Raw2", "OPEN Raw3"};
	...
	res = core_parser_get_int_data("PV5_4 Command", (char *)key[mode], str);
	if (res < 0) {
	...
	for (i = 0; i < core_mp->frame_len; i++) {
		if (mode == 0) {
			...
			if ((tmp & 0x8000) == 0x8000)
				...
				buf[i] = tmp;
			buf[i] = (int)((int)(dac[i] * 2 * 10000 * 161 / 100) - (int)(16384 / 2 - (int)buf[i])
					* 20000 * 7 / 16384 * 36 / 10) / 31 / 2;
		}
	}
...
static int open_test_sp(int index)
{
	...
	ipio_debug(DEBUG_MP_TEST,
			"pen test frame_cont %d, AA %d,Border %d, Notch %d, full_open_rate %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (ipio_debug_level & DEBUG_MP_TEST)
			dump_data(&tItems[index].buf[(i * core_mp->frame_len)], 10,
...
int mp_test_data_sort_average(int32_t *oringin_data, int index, int32_t *avg_result)
{
		...
		for (i = 0; i < core_mp->frame_len; i++) {
			for (j = 0 ; j < tItems[index].frame_count ; j++) {
				if ((i < len) || (i >= (core_mp->frame_len-len)))
					DUMP(DEBUG_MP_TEST, "%d,", u32data_buff[j * core_mp->frame_len + i]);
			}
			if ((i < len) || (i >= (core_mp->frame_len-len)))
				DUMP(DEBUG_MP_TEST, "\n");
		}
	...
	if (ipio_debug_level & DEBUG_MP_TEST) {
		...
		for (i = 0; i < core_mp->frame_len; i++) {
			for (j = u32down_frame; j < tItems[index].frame_count - u32up_frame; j++) {
				if ((i < len) || (i >= (core_mp->frame_len - len)))
					DUMP(DEBUG_MP_TEST, "%d,", u32data_buff[i + j * core_mp->frame_len]);
			}
			if ((i < len) || (i >= (core_mp->frame_len-len)))
				DUMP(DEBUG_MP_TEST, "\n");
		}
	...
	if (ipio_debug_level & DEBUG_MP_TEST) {
		...
		for (i = 0; i < core_mp->frame_len; i++) {
			if ((i < len) || (i >= (core_mp->frame_len-len)))
				DUMP(DEBUG_MP_TEST, "%d,", avg_result[i]);
		}
		if ((i < len) || (i >= (core_mp->frame_len-len)))
			DUMP(DEBUG_MP_TEST, "\n");
	}
...
#define BOARD_ID_SIZE 20
#define BOARD_ID_PATH "/persist/factoryinfo/board_id"
static void mp_test_get_save_filename(char *save_filename, int len, bool result)
{
	...
	pfile = filp_open(BOARD_ID_PATH, O_RDONLY, 0);
	if (IS_ERR(pfile)) {
		ipio_info("error occurred while opening file %s.", BOARD_ID_PATH);

		if (result)
			snprintf(save_filename, len, "%s%s.csv", g_str_save_file_path, ret_pass_name);
		else
			snprintf(save_filename, len, "%s%s.csv", g_str_save_file_path, ret_fail_name);
	} else {
		inode = pfile->f_dentry->d_inode;
		fsize = (inode->i_size > BOARD_ID_SIZE) ? BOARD_ID_SIZE : inode->i_size;
		old_fs = get_fs();
		set_fs(KERNEL_DS);
		pos = 0;
		...
		if (result)
			snprintf(save_filename, len, "%s%s_%s.csv", g_str_save_file_path, ret_pass_name, board_id);
		else
			snprintf(save_filename, len, "%s%s_%s.csv", g_str_save_file_path, ret_fail_name, board_id);
	}
}

#define CSV_NAME_SIZE 128
bool core_mp_show_result(void)
{
	int i, x, y, j, csv_len = 0, line_count = 0, get_frame_cont = 1;
	int32_t *max_threshold = NULL, *min_threshold = NULL;
	char *csv = NULL;
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (tItems[i].spec_option == BENCHMARK) {
			...
			mp_compare_cdc_show_result(tItems[i].bench_mark_max, csv, &csv_len,
					TYPE_BENCHMARK, max_threshold, min_threshold, "Max_Bench");
			mp_compare_cdc_show_result(tItems[i].bench_mark_min, csv, &csv_len,
					TYPE_BENCHMARK, max_threshold, min_threshold, "Min_Bench");
		} else {
		...
		if (tItems[i].catalog == KEY_TEST) {
			...
			mp_compare_cdc_show_result(core_mp->tx_max_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold, "TX Max Hold");
			mp_compare_cdc_show_result(core_mp->tx_min_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold, "TX Min Hold");

			for (j = 0; j < core_mp->frame_len; j++) {
				max_threshold[j] = core_mp->RxDeltaMax;
				min_threshold[j] = core_mp->RxDeltaMin;
			}
			mp_compare_cdc_show_result(core_mp->rx_max_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold, "RX Max Hold");
			mp_compare_cdc_show_result(core_mp->rx_min_buf, csv, &csv_len,
					TYPE_JUGE, max_threshold, min_threshold, "RX Min Hold");

		} else {
			/* general result */
			if (tItems[i].trimmed_mean && tItems[i].catalog != PEAK_TO_PEAK_TEST) {
				mp_compare_cdc_show_result(tItems[i].result_buf, csv, &csv_len,
						TYPE_JUGE, max_threshold, min_threshold, "Mean result");
			} else {
				mp_compare_cdc_show_result(tItems[i].buf, csv, &csv_len,
						TYPE_JUGE, max_threshold, min_threshold, "Max Hold");
				mp_compare_cdc_show_result(tItems[i].buf, csv, &csv_len,
						TYPE_JUGE, max_threshold, min_threshold, "Min Hold");
			}
			if (tItems[i].catalog != PEAK_TO_PEAK_TEST && tItems[i].catalog != UNTOUCH_P2P)
			...
			for (j = 0; j < get_frame_cont; j++) {
				...
				mp_compare_cdc_show_result(&tItems[i].buf[(j*core_mp->frame_len)], csv, &csv_len,
	...
	/* fix .csv file name */
	result = (core_mp->final_result > 0) ? true : false;
	mp_test_get_save_filename(csv_name, CSV_NAME_SIZE, result);
		...
		if (strncmp(item, tItems[i].desp, strlen(item)) == 0) {
			if (ini) {
				...
				ipio_debug(DEBUG_MP_TEST, "%s: run = %d, max = %d, min = %d, frame_count = %d\n",
					tItems[i].desp, tItems[i].run, tItems[i].max,
					tItems[i].min, tItems[i].frame_count);
			}
...
int core_mp_move_code(void)
{
	...
	if (core_config_check_cdc_busy(300, 10) < 0) {
		ipio_err("Check busy is timout ! Enter Test Mode failed\n");
		return -EPERM;
	}
#endif
...
void core_mp_test_free(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		snprintf(tItems[i].result, strlen("FAIL") + 1, "%s", "FAIL");
...
static int mp_test_init_item(void)
{
	int i;
	int res = 0;
	...
	for (i = 0; i < ARRAY_SIZE(tItems); i++) {
		...
		if (ERR_ALLOC_MEM(tItems[i].result)) {
			ipio_err("failed to malloc result[%d] buffer, %ld\n", i, PTR_ERR(tItems[i].result));
			res = -ENOMEM;
			goto out;
		}
		snprintf(tItems[i].result, strlen("FAIL") + 1, "%s", "FAIL");
...
int core_mp_init(void)
{
	...
	if (!ERR_ALLOC_MEM(core_config->tp_info)) {
		if (core_mp == NULL) {
			...
			res = mp_test_init_item();
			if (res < 0) {
				ipio_err("mp_test_init_item failed\n");
				goto init_failed;
			}
		}
	} else {
