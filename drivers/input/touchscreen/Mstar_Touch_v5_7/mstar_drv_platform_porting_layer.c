...
{BUTTON_W / 2 * 1, TOUCH_SCREEN_Y_MAX + BUTTON_H / 2, BUTTON_W, BUTTON_H},
...
static const struct file_operations gtp_proximity_fops = {
	...
	.release = NULL,
		     /*_DrvPlatformLyrProximityRelease,*/
...
static ssize_t _DrvPlatformLyrProximityDetectionStore(struct device *dev, struct device_attribute *attr,
...
__ATTR(proximity, 0666 /*0664 */, _DrvPlatformLyrProximityDetectionShow, _DrvPlatformLyrProximityDetectionStore);
...
/*Indicate HwReset is triggered by Device Driver instead of Firmware or Touch IC */
	g_IsHwResetByDriver = 1;
#endif /*CONFIG_ENABLE_ESD_PROTECTION */
}
...
/*zhangjian add for TP gesture*/
void BG_enable_irq_wake(bool enable)
{
	if (enable)
...
/*ABS_MT_TRACKING_ID is used for MSG26xxM/MSG28xx only */
...
/*
    ...
    set_bit(TOUCH_KEY_MENU, g_InputDevice->keybit);
    set_bit(TOUCH_KEY_HOME, g_InputDevice->keybit);
    set_bit(TOUCH_KEY_BACK, g_InputDevice->keybit);
    set_bit(TOUCH_KEY_SEARCH, g_InputDevice->keybit);
#endif
*/
	...
	input_set_capability(g_InputDevice, EV_KEY, KEY_GESTURE_DOUBLEC);
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
/*ABS_MT_TRACKING_ID is used for MSG26xxM/MSG28xx only */
		input_set_abs_params(g_InputDevice, ABS_MT_TRACKING_ID, 0, (MUTUAL_MAX_TOUCH_NUM - 1), 0, 0);
	}
...
s32 DrvPlatformLyrTouchDeviceRegisterFingerTouchInterruptHandler(void)
{
	...
	if (DrvIcFwLyrIsRegisterFingerTouchInterruptHandler()) {
		...
		nRetVal =
		    request_threaded_irq(_gIrq /*MS_TS_MSG_IC_GPIO_INT */, NULL,
			...
			if (pDeviceNode) {
				...
				nRetVal =
				    request_threaded_irq(_gIrq /*MS_TS_MSG_IC_GPIO_INT */, NULL,
							 ...
							 /*IRQF_TRIGGER_NONE*/|IRQF_ONESHOT/* | IRQF_NO_SUSPEND */,
							 "touch_panel-eint", NULL);
#if 0
				nRetVal =
				    request_irq(_gIrq /*MS_TS_MSG_IC_GPIO_INT */,
						_DrvPlatformLyrFingerTouchInterruptHandler,
						IRQF_TRIGGER_RISING
						/* IRQF_TRIGGER_FALLING *//*IRQF_TRIGGER_NONE *//* | IRQF_NO_SUSPEND */
						,
						"touch_panel-eint", NULL);
#endif
				if (nRetVal != 0) {
...
/*enable_irq(_gIrq);*/
		...
		mt_eint_registration(CUST_EINT_TOUCH_PANEL_NUM, CUST_EINT_TOUCH_PANEL_TYPE /* EINTF_TRIGGER_RISING */,
...
void DrvPlatformLyrSetIicDataRate(struct i2c_client *pClient, u32 nIicDataRate)
{
	...
	for how to set i2c data rate on SPRD platform */
	sprd_i2c_ctl_chg_clk(pClient->adapter->nr, nIicDataRate);
	mdelay(100);
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
	/*TODO : Please FAE colleague to confirm with customer device driver engineer
	for how to set i2c data rate on QCOM platform */
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
