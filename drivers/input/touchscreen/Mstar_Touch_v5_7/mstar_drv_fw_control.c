...
/*for MSG22xx : array size = MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE*1024+MSG22XX_FIRMWARE_INFO_BLOCK_SIZE,
for MSG28xx : array size = MSG28XX_FIRMWARE_WHOLE_SIZE*1024 */
...
static u8 _gPriorPress[SELF_MAX_TOUCH_NUM] = { 0 };	/*for MSG21xxA/MSG22xx */
...
u8 g_DemoModePacket[MUTUAL_DEMO_MODE_PACKET_LENGTH] = { 0 };
...
u8 g_DebugModePacket[MUTUAL_DEBUG_MODE_PACKET_LENGTH] = { 0 };
/*for MSG21xxA/MSG22xx : DEBUG_MODE_PACKET_LENGTH = SELF_DEBUG_MODE_PACKET_LENGTH,
for MSG26xxM/MSG28xx : DEBUG_MODE_PACKET_LENGTH = MUTUAL_DEBUG_MODE_PACKET_LENGTH */
u8 g_LogModePacket[MUTUAL_DEBUG_MODE_PACKET_LENGTH] = { 0 };
...
#else
/*for MSG21xxA : packet length = DEMO_MODE_PACKET_LENGTH,
for MSG22xx/MSG26xxM/MSG28xx : packet length = GESTURE_WAKEUP_PACKET_LENGTH */
...
/*=============================================================*/
#ifndef CONFIG_UPDATE_FIRMWARE_BY_SW_ID	/*zhangjian modify */
...
/*Set start address for tp vendor code on info block(Actually, start reading from 0xC1E8) */
...
static s32 _DrvFwCtrlSelfParsePacket(u8 *pPacket, u16 nLength, SelfTouchInfo_t *pInfo)
...
#ifdef CONFIG_ENABLE_ESD_PROTECTION
	if (g_ChipType == CHIP_TYPE_MSG22XX) {
		if (pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x08
		    && pPacket[3] == PACKET_TYPE_ESD_CHECK_HW_RESET && pPacket[4] == 0xFF && pPacket[5] == 0xFF
		    && pPacket[6] == 0xFF) {
			DBG(&g_I2cClient->dev, "ESD HwReset check : g_IsUpdateFirmware=%d, g_IsHwResetByDriver=%d\n",
			    g_IsUpdateFirmware, g_IsHwResetByDriver);

			if (g_IsUpdateFirmware == 0 && g_IsHwResetByDriver == 0
#ifdef CONFIG_ENABLE_ITO_MP_TEST
			    && g_IsInMpTest == 0
#endif /*CONFIG_ENABLE_ITO_MP_TEST              */
			    ) {
				...
				if (g_EnableTpProximity == 1) {
...
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
					{
						...
						nErr = hwmsen_get_interrupt_data(ID_PROXIMITY, &tSensorData);
						if (nErr) {
							DBG(&g_I2cClient->dev,
...
*/
	...
	if (g_GestureWakeupFlag == 1) {
		...
		if (g_ChipType == CHIP_TYPE_MSG22XX && pPacket[0] == 0xA7 && pPacket[1] == 0x00 && pPacket[2] == 0x06
		    && pPacket[3] == PACKET_TYPE_GESTURE_WAKEUP) {
			nWakeupMode = pPacket[4];
			bIsCorrectFormat = 1;
		}
#ifdef CONFIG_ENABLE_GESTURE_DEBUG_MODE
		else if (g_ChipType == CHIP_TYPE_MSG22XX && pPacket[0] == 0xA7 && pPacket[1] == 0x00
			 && pPacket[2] == 0x80 && pPacket[3] == PACKET_TYPE_GESTURE_DEBUG) {
			u32 a = 0;
			...
			if (!(pPacket[5] >> 7)) {	/*LCM Light Flag = 0 */
			 ...
			 && pPacket[2] == 0x80 && pPacket[3] == PACKET_TYPE_GESTURE_INFORMATION) {
			u32 a = 0;
			u32 nTmpCount = 0;
			u32 nWidth = 0;
			u32 nHeight = 0;
...
#endif /*CONFIG_ENABLE_GESTURE_INFORMATION_MODE */
		else if (g_ChipType == CHIP_TYPE_MSG21XXA && pPacket[0] == 0x52 && pPacket[1] == 0xFF
			 && pPacket[2] == 0xFF && pPacket[3] == 0xFF && pPacket[4] == 0xFF && pPacket[6] == 0xFF) {
...
/*input_report_key(g_InputDevice, RESERVER51, 0);*/
	...
	"pPacket[0]=%x\n pPacket[1]=%x pPacket[2]=%x pPacket[3]=%x pPacket[4]=%x\n",
		pPacket[0], pPacket[1], pPacket[2], pPacket[3], pPacket[4]);
	DBG(&g_I2cClient->dev,
	"pPacket[5]=%x pPacket[6]=%x pPacket[7]=%x\n", pPacket[5], pPacket[6], pPacket[7]);
	...
	if ((pPacket[nCheckSumIndex] == nCheckSum) && (pPacket[0] == 0x52)) {
		...
		/*
		 * pPacket[0]:id, pPacket[1]~pPacket[3]:the first point abs, pPacket[4]~pPacket[6]:
		 the relative distance between the first point abs and the second point abs
		 ...
		 */
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF)
		    && (pPacket[6] == 0xFF)) {
...
/*if ((nDeltaX == 0) && (nDeltaY == 0))*/
			...
			if (_gPrevTouchStatus == 1) {
				...
				if (/*(pInfo->nFingerNum == 1)&& */(nPrevTouchNum == 2)) {
					if (_DrvFwCtrlPointDistance(szX[0], szY[0], szPrevX[0], szPrevY[0]) >
					    _DrvFwCtrlPointDistance(szX[0], szY[0], szPrevX[1], szPrevY[1])) {
						nChangePoints = 1;
					}
				} else if ((pInfo->nFingerNum == 2) && (nPrevTouchNum == 1)) {
					if (szPrevPress[0] == 1) {
						if (_DrvFwCtrlPointDistance(szX[0], szY[0], szPrevX[0], szPrevY[0]) >
						    _DrvFwCtrlPointDistance(szX[1], szY[1], szPrevX[0], szPrevY[0])) {
							nChangePoints = 1;
						}
					} else {
						if (_DrvFwCtrlPointDistance(szX[0], szY[0], szPrevX[1], szPrevY[1]) <
						    _DrvFwCtrlPointDistance(szX[1], szY[1], szPrevX[1], szPrevY[1])) {
							nChangePoints = 1;
						}
					}
		...
		/*
		 * pPacket[0]:id, pPacket[1]~pPacket[4]:the first point abs, pPacket[13]~
		 pPacket[16]:the relative distance between the first point abs and the second point abs
		 ...
		 */
		if ((pPacket[1] == 0xFF) && (pPacket[2] == 0xFF) && (pPacket[3] == 0xFF) && (pPacket[4] == 0xFF)
		    && (pPacket[5] == 0xFF) && (pPacket[6] == 0xFF) && (pPacket[7] == 0xFF)) {
			pInfo->tPoint[0].nX = 0;	/*final X coordinate */
			pInfo->tPoint[0].nY = 0;	/*final Y coordinate */
...
/*}*/
			...
			if (g_TouchKObj != NULL) {
				char *pEnvp[2];
				s32 nRetVal = 0;
/*Copy g_DebugModePacket to g_LogModePacket for avoiding the debug mode data
 which is received by MTPTool APK may be modified. */
				memcpy(g_LogModePacket, g_DebugModePacket, nLength);
...
static s32 _DrvFwCtrlMutualParsePacket(u8 *pPacket, u16 nLength, MutualTouchInfo_t *pInfo)
{/*for MSG26xxM/MSG28xx */
	int ret = 0;
	u32 i;
	u8 nCheckSum = 0;
	u32 nX = 0, nY = 0;
	...
	"pPacket[5]=%x pPacket[6]=%x pPacket[7]=%x pPacket[8]=%x\n",
		pPacket[5], pPacket[6], pPacket[7], pPacket[8]);
	...
	if (g_GestureWakeupFlag == 1) {
		...
		if (bIsCorrectFormat) {
			pr_notice("nWakeupMode = 0x%x\n", nWakeupMode);

			switch (nWakeupMode) {
			case 0x58:
				_gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_DOUBLE_CLICK_FLAG;

				pr_notice("Light up screen by DOUBLE_CLICK gesture wakeup.\n");

				input_report_key(g_InputDevice, KEY_GESTURE_DOUBLEC, 1);
				input_sync(g_InputDevice);
				input_report_key(g_InputDevice, KEY_GESTURE_DOUBLEC, 0);
				input_sync(g_InputDevice);
				break;
			case 0x60:
				_gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_UP_DIRECT_FLAG;
...
/*input_report_key(g_InputDevice, KEY_DOWN, 0);*/
			...
			case 0x62:
				...
				pr_notice("Light up screen by LEFT_DIRECT gesture wakeup.\n");
...
/*input_report_key(g_InputDevice, KEY_LEFT, 0);*/
			...
			case 0x63:
				...
				pr_notice("Light up screen by RIGHT_DIRECT gesture wakeup.\n");
...
/*input_report_key(g_InputDevice, KEY_RIGHT, 0);*/
			...
			case 0x64:
				...
				pr_notice("Light up screen by m_CHARACTER gesture wakeup.\n");

/*input_report_key(g_InputDevice, KEY_M, 1);*/
				input_report_key(g_InputDevice, KEY_GESTURE_M, 1);
				input_sync(g_InputDevice);
/*input_report_key(g_InputDevice, KEY_M, 0);*/
				input_report_key(g_InputDevice, KEY_GESTURE_M, 0);
				input_sync(g_InputDevice);
				break;
			case 0x65:
				_gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_W_CHARACTER_FLAG;

				pr_notice("Light up screen by W_CHARACTER gesture wakeup.\n");

/*input_report_key(g_InputDevice, KEY_W, 1);*/
				input_report_key(g_InputDevice, KEY_GESTURE_W, 1);
				input_sync(g_InputDevice);
/*input_report_key(g_InputDevice, KEY_W, 0);*/
				input_report_key(g_InputDevice, KEY_GESTURE_W, 0);
				input_sync(g_InputDevice);
				break;
			case 0x66:
				_gGestureWakeupValue[0] = GESTURE_WAKEUP_MODE_C_CHARACTER_FLAG;

				pr_notice("Light up screen by C_CHARACTER gesture wakeup.\n");
...
/*input_report_key(g_InputDevice, RESERVER51, 0);*/
	...
	if (IS_FIRMWARE_DATA_LOG_ENABLED) {
		...
		} else if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE && pPacket[0] != 0xA5 && pPacket[0] != 0xAB
	...
	if (pPacket[0] == 0x5A) {
		...
		if (((DemoHotKnotCmdRet_t *) pPacket)->nIdentify == DEMO_PD_PACKET_IDENTIFY) {
			...
			ret = -EPERM;
			goto out; /*return 0 will run key procedure  */
		} else
#endif /*CONFIG_ENABLE_HOTKNOT  */
		{
			for (i = 0; i < MAX_TOUCH_NUM; i++) {
			...
			/*Notify android application to retrieve demo mode packet from device driver by sysfs.*/
				if (g_TouchKObj != NULL) {
					...
					strlcat(szRspDemoModePacket, "VALUE=", PAGE_SIZE);

					for (i = 0; i < nLength; i++) {
						snprintf(szValue, sizeof(szValue), "%02x", g_DemoModePacket[i] & 0xFF);
						strlcat(szRspDemoModePacket, szValue, PAGE_SIZE);
					}
					...
					nRetVal = kobject_uevent_env(g_TouchKObj, KOBJ_CHANGE, pEnvp);
					DBG(&g_I2cClient->dev,
					    "kobject_uevent_env() STATUS=GET_DEMO_MODE_PACKET, nRetVal = %d\n",
					    nRetVal);
				}
			}
		}
		...
		for (i = 0; i < MAX_TOUCH_NUM; i++) {
			if ((pPacket[(3 * i) + 4] == 0xFF) && (pPacket[(3 * i) + 5] == 0xFF)
			    && (pPacket[(3 * i) + 6] == 0xFF)) {
#ifdef CONFIG_ENABLE_TYPE_B_PROTOCOL
				_gCurrentTouch[i] = 0;
#endif /*CONFIG_ENABLE_TYPE_B_PROTOCOL */
...
#ifdef CONFIG_TP_HAVE_KEY
	if (pPacket[0] == 0x5A) {
/*Since the key value is stored in 0th~3th bit of variable "button", we can only retrieve 0th~3th bit of it. */
...
/*if (nButton)*/
		...
		if (pPacket[0] == 0xA5) {
			...
			if (pPacket[0] == 0xAB) {
				/*The pressed virtual key is stored in 4th byte for debug mode packet 0xAB.*/
				nButton = pPacket[3];
			} else if (pPacket[0] == 0xA7 && pPacket[3] == PACKET_TYPE_TOOTH_PATTERN) {
...
static void _DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(u16 nStartAddr, u8 *pFirstData, u32 nBlockSize, u16 nPageNum,
...
static void _DrvFwCtrlMsg28xxProgramEmem(EmemType_e eEmemType)
{
	...
	|| defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_32_BYTE_EACH_TIME)
	u32 k;
#endif
	...
	if (eEmemType == EMEM_MAIN) {	/*Program main block */
		...
		/*Write the first two pages(page 0 & page 1)*/
		...
		for (i = 2; i < nPageNum; i++) {
			if (i == 2) {
				...
				for (j = 0; j < nLength; j++) {
					szWriteData[3 + j] =
					    g_FwData[i / 8][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - (8 * (i / 8))) +
							    j];
				}
...
/*szWriteData[1] = 0x00;*/
		...
		for (i = 0; i < nPageNum; i++) {
			if (i == 0) {
				...
				_DrvFwCtrlMsg28xxIspBurstWriteEFlashStart(nIndex, &szFirstData[0],
		...
		for (i = 1; i < (nPageNum - 1); i++) {	/*Skip the first 128 byte and the last 128 byte of info block */
			if (i == 1) {
				...
				szFirstData[0] =
				    g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE];
				szFirstData[1] =
				    g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE +
									       1];
				szFirstData[2] =
				...
				/*124%8 = 4 */
				_DrvFwCtrlMsg28xxIspBurstWriteEFlashDoWrite(&szBufferData[0], (nLength % 8));
				...
				/*124%8 = 28 */
				_DrvFwCtrlMsg28xxIspBurstWriteEFlashDoWrite(&szBufferData[0], (nLength % 32));
				...
				if (i < 8) {	/*1 < i < 8 */
					...
					for (j = 0; j < (nLength / 8); j++) {	/*128/8 = 16 */
						for (k = 0; k < 8; k++) {
							szBufferData[k] =
							    g_FwData[MSG28XX_FIRMWARE_MAIN_BLOCK_SIZE +
								     1][MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE * (i - 8) +
									(8 * j) + k];
						}
...
static u32 _DrvFwCtrlMsg28xxRetrieveFirmwareCrcFromEFlash(EmemType_e eEmemType)
{
	...
	if (eEmemType == EMEM_MAIN) {	/*Read main block CRC(128KB-4) from main block */
		...
		nReadAddr = 0x7FFF;
/*Read info block CRC(2KB-MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE-4) from info block */
	} else if (eEmemType == EMEM_INFO) {
...
static s32 _DrvFwCtrlMsg28xxCheckFirmwareBinIntegrity(u8 szFwData[][1024])
{
	...
	nCrcInfo =
	    _DrvFwCtrlMsg28xxCalculateCrc(_gOneDimenFwData,
					  ...
					  MSG28XX_FIRMWARE_INFO_BLOCK_SIZE * 1024 -
					  MSG28XX_EMEM_SIZE_BYTES_PER_ONE_PAGE - MSG28XX_EMEM_SIZE_BYTES_ONE_WORD);
...
static s32 _DrvFwCtrlMsg28xxUpdateFirmware(u8 szFwData[][1024], EmemType_e eEmemType)
{
	...
	DBG(&g_I2cClient->dev,
		"nCrcMain=0x%x, nCrcInfo=0x%x, nCrcMainHardware=0x%x, nCrcInfoHardware=0x%x,\n",
				nCrcMain, nCrcInfo, nCrcMainHardware, nCrcInfoHardware);
	DBG(&g_I2cClient->dev,
		"nCrcMainEflash=0x%x, nCrcInfoEflash=0x%x\n", nCrcMainEflash, nCrcInfoEflash);
	g_FwDataCount = 0;	/*Reset g_FwDataCount to 0 after update firmware */
...
/*for specific TP vendor which store some important information in info block,
only update firmware for main block, do not update firmware for info block. */
		if (_gTpVendorCode[0] == 'C' && _gTpVendorCode[1] == 'N' && _gTpVendorCode[2] == 'T') {
	...
	/*Force to update firmware, do not check whether the vendor id of the update firmware bin file
	is equal to the vendor id on e-flash. */
		if (IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED) {
			_DrvFwCtrlMsg28xxUpdateFirmware(szFwData, EMEM_MAIN);
			goto out;
		} else {
			u16 eSwId = 0x0000;
			u16 eVendorId = 0x0000;
			/*Retrieve major from info block */
			...
			/*Check if the vendor id of the update firmware bin file is equal to the vendor id on e-flash.
			 YES => allow update, NO => not allow update */
			if (eSwId != eVendorId) {
				/*Reset HW here to avoid touch may be not worked after get sw id. */
				DrvPlatformLyrTouchDeviceResetHw();

				DBG(&g_I2cClient->dev,
				    "The vendor id is different from the vendor id on e-flash. Not allow to update.\n");
				g_FwDataCount = 0;	/*Reset g_FwDataCount to 0 */
...
static s32 _DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
	...
	if ((eSwId == eVendorId) || (IS_FORCE_TO_UPDATE_FIRMWARE_ENABLED)) {
		if ((g_ChipType == CHIP_TYPE_MSG21XXA && fsize == 33792 /* 33KB */)
		    || (g_ChipType == CHIP_TYPE_MSG22XX && fsize == 49664 /* 48.5KB */)
		    || (g_ChipType == CHIP_TYPE_MSG26XXM && fsize == 40960 /* 40KB */)) {
			nRetVal = _DrvFwCtrlUpdateFirmwareCash(g_FwData, EMEM_ALL);
		} else if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)
			   && fsize == 133120 /* 130KB */) {
		/*For MSG28xx sine mode requirement, update main block only, do not update info block. */
...
u32 DrvFwCtrlReadDQMemValue(u16 nAddr)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		ret = (rx_data[3] << 24 | rx_data[2] << 16 | rx_data[1] << 8 | rx_data[0]);
	} else {
		DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
static void _DrvFwCtrlMsg22xxProgramEmem(EmemType_e eEmemType)
{
	...
	if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05) {
#if defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A)
/*Set Bit0 = 1 for enable I2C 400KHz burst write mode, let e-flash discard the last 2 dummy byte. */
		RegSet16BitValueOn(0x160A, BIT0);
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
		RegSet16BitValueOn(0x160A, BIT0);
/*Set Bit0 = 1, Bit1 = 1 for enable I2C 400KHz burst write mode, let e-flash discard the last 3 dummy byte.*/
		RegSet16BitValueOn(0x160A, BIT1);
#endif /*CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A */
	}
#endif /*CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K */
...
#elif defined(CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_B)
		RegSet16BitValueOff(0x160A, BIT0);
		RegSet16BitValueOff(0x160A, BIT1);
#endif /*CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K_BY_METHOD_A */
	}
#endif /*CONFIG_ENABLE_UPDATE_FIRMWARE_WITH_SUPPORT_I2C_SPEED_400K */
	...
	if (_gIsUpdateInfoBlockFirst == 1) {
		if ((g_IsUpdateFirmware & 0x10) == 0x10) {
			...
			if (nCrcInfoA == nCrcInfoB) {
				...
				nCrcMainA =
				    _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(_gOneDimenFwData, EMEM_MAIN);
...
/*_gIsUpdateInfoBlockFirst == 0*/
		...
		if ((g_IsUpdateFirmware & 0x10) == 0x10) {
			...
			if (nCrcMainA == nCrcMainB) {
				...
				nCrcInfoA =
				    _DrvFwCtrlMsg22xxRetrieveFrimwareCrcFromBinFile(_gOneDimenFwData, EMEM_INFO);
				nCrcInfoB = _DrvFwCtrlMsg22xxGetFirmwareCrcByHardware(EMEM_INFO);
...
void _DrvFwCtrlMsg22xxCheckFirmwareUpdateBySwId(void)
{
	...
	if (nCrcMainA == nCrcMainB && nCrcInfoA == nCrcInfoB) {	/*Case 1. Main Block:OK, Info Block:OK */
		...
		for (j = 0; j < nSwIdListNum; j++) {
			DBG(&g_I2cClient->dev, "_gSwIdData[%d].nSwId = 0x%x\n", j, _gSwIdData[j].nSwId);
			if (eSwId == _gSwIdData[j].nSwId) {
				nUpdateBinMajor =
				    (*(_gSwIdData[j].pUpdateBin + 0xBFF5)) << 8 |
				    (*(_gSwIdData[j].pUpdateBin + 0xBFF4));
				nUpdateBinMinor =
				    (*(_gSwIdData[j].pUpdateBin + 0xBFF7)) << 8 |
				    (*(_gSwIdData[j].pUpdateBin + 0xBFF6));
				nIsSwIdValid = 1;
				nFinalIndex = j;
		...
		if (nUpdateBinMinor > nMinor) {
			if (nIsSwIdValid == 1) {
				...
				for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE + 1); i++) {
					if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) {/*i < 48 */
						_DrvFwCtrlStoreFirmwareData((_gSwIdData[nFinalIndex].pUpdateBin +
									     i * 1024), 1024);
					} else {	/*i == 48 */
			...
			if (eSwId < MSG22XX_SW_ID_UNDEFINED && eSwId != 0x0000 && eSwId != 0xFFFF) {
				/*Reset g_FwDataCount to 0 after copying update firmware data to temp buffer */
				g_FwDataCount = 0;
		...
		for (j = 0; j < nSwIdListNum; j++) {
			...
			if (eSwId == _gSwIdData[j].nSwId) {
				for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE + 1); i++) {
					if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) {	/*i < 48 */
						_DrvFwCtrlStoreFirmwareData((_gSwIdData[j].pUpdateBin + i * 1024),
									    1024);
					} else {	/*i == 48 */

						_DrvFwCtrlStoreFirmwareData((_gSwIdData[j].pUpdateBin + i * 1024), 512);
					}
				}
				nIsSwIdValid = 1;
		...
		for (j = 0; j < nSwIdListNum; j++) {
			...
			if (eSwId == _gSwIdData[j].nSwId) {
				for (i = 0; i < (MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE + 1); i++) {
					if (i < MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE) {	/*i < 48 */
					_DrvFwCtrlStoreFirmwareData((_gSwIdData[j].pUpdateBin + i * 1024), 1024);
					} else {	/*i == 48 */
			...
			if (i == 31) {
				...
				for (j = 0; j < 4; j++) {
					...
					DBG(&g_I2cClient->dev, "((nCrcTemp>>(8*%d)) & 0xFF)=%x\n",
						j, ((nCrcTemp >> (8 * j)) & 0xFF));
					DBG(&g_I2cClient->dev,
					    "Update main clock crc32 into bin buffer szFwData[%d][%d]=%x\n", i,
					    (1020 + j), szFwData[i][1020 + j]);
				}
			} else {
				for (j = 0; j < 1024; j++) {
					nCrcMain = DrvCommonCrcGetValue(szFwData[i][j], nCrcMain);
...
void _DrvFwCtrlMsg21xxaCheckFirmwareUpdateBySwId(void)
{
	...
	u8 *pVersion = NULL;
	Msg21xxaSwId_e eMainSwId = MSG21XXA_SW_ID_UNDEFINED, eInfoSwId = MSG21XXA_SW_ID_UNDEFINED, eSwId =
	...
	if (nCrcMainA == nCrcMainB) {
		...
		if (eSwId == MSG21XXA_SW_ID_XXXX) {
			...
			nUpdateBinMajor =
			    msg21xxa_xxxx_update_bin[31][0x34F] << 8 | msg21xxa_xxxx_update_bin[31][0x34E];
			nUpdateBinMinor =
			    msg21xxa_xxxx_update_bin[31][0x351] << 8 | msg21xxa_xxxx_update_bin[31][0x350];
...
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY	/*By two dimensional array */
			nUpdateBinMajor =
			    msg21xxa_yyyy_update_bin[31][0x34F] << 8 | msg21xxa_yyyy_update_bin[31][0x34E];
			nUpdateBinMinor =
			    msg21xxa_yyyy_update_bin[31][0x351] << 8 | msg21xxa_yyyy_update_bin[31][0x350];
#else /*By one dimensional array */
...
static s32 _DrvFwCtrlMsg28xxUpdateFirmwareBySwId(void)
{
	...
	if (_gIsUpdateInfoBlockFirst == 1)
	{
		if ((g_IsUpdateFirmware & 0x10) == 0x10)
		{
			...
			if (nCrcInfoA == nCrcInfoB)
			{
				DrvFwCtrlMsg28xxEraseEmem(EMEM_MAIN);
				_DrvFwCtrlMsg28xxProgramEmem(EMEM_MAIN);
	...
	if (nCrcMainA == nCrcMainB) {
...
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY	/*By two dimensional array */
				nUpdateBinMajor =
				    ((*(_gSwIdData[j].pUpdateBin + 127))[1013]) << 8 |
				    ((*(_gSwIdData[j].pUpdateBin + 127))[1012]);
				nUpdateBinMinor =
				    ((*(_gSwIdData[j].pUpdateBin + 127))[1015]) << 8 |
				    ((*(_gSwIdData[j].pUpdateBin + 127))[1014]);
#else /*By one dimensional array */
				nUpdateBinMajor =
				    (*(_gSwIdData[j].pUpdateBin + 0x1FFF5)) << 8 |
				    (*(_gSwIdData[j].pUpdateBin + 0x1FFF4));
				nUpdateBinMinor =
				    (*(_gSwIdData[j].pUpdateBin + 0x1FFF7)) << 8 |
				    (*(_gSwIdData[j].pUpdateBin + 0x1FFF6));
		...
		if (nUpdateBinMinor > nMinor) {
			if (nIsSwIdValid == 1) {
				...
				for (i = 0; i < MSG28XX_FIRMWARE_WHOLE_SIZE; i++) {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY	/*By two dimensional array */
					_DrvFwCtrlStoreFirmwareData(*(_gSwIdData[nFinalIndex].pUpdateBin + i), 1024);
#else /*By one dimensional array */
					_DrvFwCtrlStoreFirmwareData((_gSwIdData[nFinalIndex].pUpdateBin + i * 1024),
								    1024);
#endif /*CONFIG_UPDATE_FIRMWARE_BY_TWO_DIMENSIONAL_ARRAY */
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
	...
	if (nRetVal == 0) {
		...
		if (g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM
		    || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
			_gIsUpdateInfoBlockFirst = 0;
			g_IsUpdateFirmware = 0x00;
		}
	} else {/*nRetVal == -1 */
...
static s32 _DrvFwCtrlMsg21xxaUpdateFirmwareC33(u8 szFwData[][1024], EmemType_e eEmemType)
{
	...
	if (_gDwIicInfoData[0] == 'M' && _gDwIicInfoData[1] == 'S' && _gDwIicInfoData[2] == 'T'
	    && _gDwIicInfoData[3] == 'A' && _gDwIicInfoData[4] == 'R' && _gDwIicInfoData[5] == 'T'
	    && _gDwIicInfoData[6] == 'P' && _gDwIicInfoData[7] == 'C') {
		...
		if (i < 32) {	/*emem_main */
			if (i == 31) {
				...
				for (j = 0; j < 4; j++) {
					...
					DBG(&g_I2cClient->dev, "((nCrcTemp>>(8*%d)) & 0xFF)=%x\n",
						j, ((nCrcTemp >> (8 * j)) & 0xFF));
					DBG(&g_I2cClient->dev, "szFwData[%d][%d]=%x\n",
		...
		while (nRemainSize > 0) {
			...
			if (g_Msg22xxChipRevision >= CHIP_TYPE_MSG22XX_REVISION_U05) {
				...
				for (i = 0; i < nBlockSize; i++) {
					for (j = 0; j < 3; j++) {
						szTxData[3 + (i * 3) + j] =
						    _gOneDimenFwData[(MSG22XX_FIRMWARE_MAIN_BLOCK_SIZE * 1024) +
...
{				/*support MSG28xx only */
	...
	if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
		...
		*pMinor |= szTmpData[2];
		...
		snprintf(*ppVersion, PAGE_SIZE, "%05d.%05d", *pMajor, *pMinor);
...
void DrvFwCtrlGetPlatformFirmwareVersion(u8 **ppVersion)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG22XX) {
		...
		/*RIU password */
		RegSet16BitValue(0x161A, 0xABBA);
/*Set start address for platform firmware version on info block(Actually, start reading from 0xC1F0) */
...
/*DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+0, szRxData[i*4+0], szRxData[i*4+0]);*/
/*DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+1, szRxData[i*4+1], szRxData[i*4+1]);*/

			szRxData[i * 4 + 2] = (nRegData2 & 0xFF);
			szRxData[i * 4 + 3] = ((nRegData2 >> 8) & 0xFF);

/*DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+2, szRxData[i*4+2], szRxData[i*4+2]);*/
/*DBG(&g_I2cClient->dev, "szRxData[%d] = 0x%x , %c\n", i*4+3, szRxData[i*4+3], szRxData[i*4+3]);*/
		}
		...
		snprintf(*ppVersion, PAGE_SIZE, "%c%c%c%c%c%c%c%c%c%c", szRxData[2], szRxData[3], szRxData[4],
		...
		snprintf(*ppVersion, PAGE_SIZE, "%.16s", szRxData);
		...
		snprintf(*ppVersion, PAGE_SIZE, "%.10s", szRxData);
	} else {
		if (*ppVersion == NULL) {
			*ppVersion = kzalloc(sizeof(u8) * 10, GFP_KERNEL);
		}

		snprintf(*ppVersion, PAGE_SIZE, "%s", "N/A");
	}
...
s32 DrvFwCtrlUpdateFirmwareBySdCard(const char *pFilePath)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX || g_ChipType == CHIP_TYPE_MSG26XXM
	    || g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA) {
...
u16 DrvFwCtrlChangeFirmwareMode(u16 nMode)
{
	...
	if (g_ChipType == CHIP_TYPE_MSG26XXM) {
		...
		mutex_unlock(&g_Mutex);
		/*Resume finger touch ISR handling after device driver have sent i2c command to firmware. */
...
void DrvFwCtrlSelfGetFirmwareInfo(SelfFirmwareInfo_t *pInfo)
{				/*for MSG21xxA/MSG22xx */
	u8 szTxData[1] = { 0 };
	u8 szRxData[8] = { 0 };
	u32 i = 0;
	...
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	/*Skip finger touch ISR handling temporarily for device driver can send
		get firmware info i2c command to firmware. */
	g_IsDisableFingerTouch = 1;
	...
	DBG(&g_I2cClient->dev,
	    "nFirmwareMode=0x%x, nLogModePacketHeader=0x%x, nLogModePacketLength=%d, nIsCanChangeFirmwareMode=%d\n",
	    pInfo->nFirmwareMode, pInfo->nLogModePacketHeader, pInfo->nLogModePacketLength,
	    pInfo->nIsCanChangeFirmwareMode);
/*Resume finger touch ISR handling after device driver have sent i2c command to firmware. */
	g_IsDisableFingerTouch = 0;
}

void DrvFwCtrlMutualGetFirmwareInfo(MutualFirmwareInfo_t *pInfo)
{/*for MSG26xxM/MSG28xx */
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
...
/*pInfo->nSs = szRxData[2];*/
		...
		if (pInfo->nLogModePacketHeader == 0xA5) {
			if (pInfo->nMy != 0 && pInfo->nMx != 0) {
				...
				DBG(&g_I2cClient->dev,
				    "Failed to retrieve channel number or subframe number for debug mode packet 0xA5.\n");
			}
		} else if (pInfo->nLogModePacketHeader == 0xAB) {
			if (pInfo->nMy != 0 && pInfo->nMx != 0) {
				/*for parsing debug mode packet 0xAB */
				pInfo->nLogModePacketLength =
				    1 + 1 + 1 + 1 + 10 * 3 + pInfo->nMy * pInfo->nMx * 2 + pInfo->nMy * 2 +
				    pInfo->nMx * 2 + 2 * 2 + 8 * 2 + 1;
			} else {
				DBG(&g_I2cClient->dev,
				    "Failed to retrieve channel number or subframe number for debug mode packet 0xAB.\n");
			}
			...
			if (pInfo->nMy != 0 && pInfo->nMx != 0 && pInfo->nSd != 0 && pInfo->nSs != 0) {
				...
				pInfo->nLogModePacketLength =
				    1 + 1 + 1 + 1 + 1 + 10 * 3 + pInfo->nMy * pInfo->nMx * 2 + pInfo->nSd * 2 +
				    pInfo->nSs * 2 + 10 * 2 + 1;
			} else {
				DBG(&g_I2cClient->dev,
				    "Failed to retrieve channel number or subframe number for debug mode packet 0xA7.\n");
			}
		} else {
	...
	if (g_ChipType == CHIP_TYPE_MSG21XXA || g_ChipType == CHIP_TYPE_MSG22XX) {
		if (g_IsSwitchModeByAPK == 1) {
			...
			if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE
				...
				g_FirmwareMode = DrvFwCtrlChangeFirmwareMode(FIRMWARE_MODE_DEBUG_MODE);
				   ...
				   && FIRMWARE_MODE_RAW_DATA_MODE != tInfo.nFirmwareMode) {
		...
		if (g_IsSwitchModeByAPK == 1) {
			...
			/*Since reset_hw() will reset the firmware mode to demo mode,
			 we must reset the firmware mode again after reset_hw(). */
			if (g_FirmwareMode == FIRMWARE_MODE_DEBUG_MODE
			    && FIRMWARE_MODE_DEMO_MODE == DrvFwCtrlGetFirmwareMode()) {
				g_FirmwareMode = DrvFwCtrlChangeFirmwareMode(FIRMWARE_MODE_DEBUG_MODE);
			} else {
				DBG(&g_I2cClient->dev, "firmware mode is not restored\n");
			}
...
{/*used for MSG28xx only */
	...
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
	/*Skip finger touch ISR handling temporarily for device driver
		can send i2c command to firmware. */
	g_IsDisableFingerTouch = 1;
...
void DrvFwCtrlOpenLeatherSheathMode(void)
{				/*used for MSG28xx only */
	s32 rc = 0;
	u8 szTxData[3] = { 0 };
	u32 i = 0;
...
void DrvFwCtrlCloseLeatherSheathMode(void)
{				/*used for MSG28xx only */
	s32 rc = 0;
	u8 szTxData[3] = { 0 };
	u32 i = 0;
	...
	DBG(&g_I2cClient->dev, "*** %s() ***\n", __func__);
/*Skip finger touch ISR handling temporarily for device driver can send i2c command to firmware. */
	g_IsDisableFingerTouch = 1;
...
void DrvFwCtrlGetLeatherSheathInfo(u8 *pLeatherSheathMode)
{				/*used for MSG28xx only */
	u8 szTxData[3] = { 0 };
	u8 szRxData[2] = { 0 };
	u32 i = 0;
	...
	if (IS_FIRMWARE_DATA_LOG_ENABLED) {
		if (g_FirmwareMode == FIRMWARE_MODE_DEMO_MODE) {
			...
			if (g_FwSupportSegment == 0) {
				DBG(&g_I2cClient->dev,
				    "g_FwPacketDataAddress & g_FwPacketFlagAddress is un-initialized\n");
				return -EPERM;
			}
			...
			if (_DrvFwCtrlCheckFingerTouchPacketFlagBit1() != 0) {
				DBG(&g_I2cClient->dev,
				    "Bit1 is not 0. FW is not ready for providing debug mode packet to Device Driver\n");
				return -EPERM;
			}

			rc = IicSegmentReadDataBySmBus(g_FwPacketDataAddress, &pPacket[0], nReportPacketLength,
...
void _DrvFwCtrlSelfHandleFingerTouch(void)
{				/*for MSG21xxA/MSG22xx */
...
/*DBG(&g_I2cClient->dev, "*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug*/
	...
	if (_DrvFwCtrlSelfParsePacket(pPacket, nReportPacketLength, &tInfo) == 0) {
		/*report... */
			...
			if (tInfo.nTouchKeyCode != 0) {
#ifdef CONFIG_TP_HAVE_KEY
#if defined(CONFIG_TOUCH_DRIVER_RUN_ON_SPRD_PLATFORM) || defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
				if (tInfo.nTouchKeyCode == 1) {	/*TOUCH_KEY_HOME 4 */
					nTouchKeyCode = g_TpVirtualKey[1];
				} else if (tInfo.nTouchKeyCode == 4) {	/*TOUCH_KEY_MENU 1 */
				...
				for (i = 0; i < MAX_TOUCH_NUM; i++) {
					if (tInfo.nFingerNum != 0) {
						if (_gCurrPress[i]) {
							input_report_key(g_InputDevice, BTN_TOUCH, 1);
							DrvPlatformLyrFingerTouchPressed(tInfo.tPoint[i].nX,
											 tInfo.tPoint[i].nY, 1, i);
				...
				for (i = 0; i < tInfo.nFingerNum; i++) {
					DrvPlatformLyrFingerTouchPressed(tInfo.tPoint[i].nX, tInfo.tPoint[i].nY, 1, 0);
				}
#endif /*CONFIG_ENABLE_TYPE_B_PROTOCOL */
...
void _DrvFwCtrlMutualHandleFingerTouch(void)
{				/*for MSG26xxM/MSG28xx */
...
/*DBG(&g_I2cClient->dev, "*** %s() *** mutex_lock(&g_Mutex)\n", __func__);  // add for debug*/
	...
	if (IS_FIRMWARE_DATA_LOG_ENABLED) {
		if (g_FirmwareMode == FIRMWARE_MODE_DEMO_MODE) {
			...
			if (g_MutualFirmwareInfo.nLogModePacketHeader != 0xA5
			    ...
			    && g_MutualFirmwareInfo.nLogModePacketHeader != 0xA7) {
				DBG(&g_I2cClient->dev, "WRONG DEBUG MODE HEADER : 0x%x\n",
	...
	if (g_GestureWakeupFlag == 1) {
		...
		if (_DrvFwCtrlReadFingerTouchData(&pPacket[0], nReportPacketLength) != 0) {
			goto TouchHandleEnd;
		}
	}
#else
	...
	if (_DrvFwCtrlMutualParsePacket(pPacket, nReportPacketLength, &tInfo) == 0) {
#ifdef CONFIG_TP_HAVE_KEY
		if (tInfo.nKeyCode != 0xFF) {	/*key touch pressed */
			...
			if (tpd_dts_data.use_tpd_button) {
				DBG(&g_I2cClient->dev,
				    "tInfo.nKeyCode=%x, nLastKeyCode=%x, tpd_dts_data.tpd_key_local[%d]=%d\n",
				    tInfo.nKeyCode, nLastKeyCode, tInfo.nKeyCode,
				    tpd_dts_data.tpd_key_local[tInfo.nKeyCode]);
			}
#else
			DBG(&g_I2cClient->dev, "tInfo.nKeyCode=%x, nLastKeyCode=%x, g_TpVirtualKey[%d]=%d\n",
			    tInfo.nKeyCode, nLastKeyCode, tInfo.nKeyCode, g_TpVirtualKey[tInfo.nKeyCode]);
				...
				if (tInfo.nKeyCode != nLastKeyCode) {
					...
					if (tpd_dts_data.use_tpd_button) {
						input_report_key(g_InputDevice,
								 tpd_dts_data.tpd_key_local[tInfo.nKeyCode], 1);
					}
#else
					input_report_key(g_InputDevice, g_TpVirtualKey[tInfo.nKeyCode], 1);
#endif /*CONFIG_PLATFORM_USE_ANDROID_SDK_6_UPWARD */
		...
		if (tInfo.nCount > 0) {	/*point touch pressed */
			...
			for (i = 0; i < tInfo.nCount; i++) {
				input_report_key(g_InputDevice, BTN_TOUCH, 1);
				DrvPlatformLyrFingerTouchPressed(tInfo.tPoint[i].nX, tInfo.tPoint[i].nY,
								 tInfo.tPoint[i].nP, tInfo.tPoint[i].nId);
				input_report_key(g_InputDevice, BTN_TOOL_FINGER, 1);
			}
