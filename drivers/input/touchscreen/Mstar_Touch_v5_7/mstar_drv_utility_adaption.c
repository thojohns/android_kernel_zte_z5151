...
void DmaAlloc(void)
{
	if (I2CDMABuf_va == NULL) {
		if (g_InputDevice != NULL) {
			...
			I2CDMABuf_va =
			    (u8 *) dma_alloc_coherent(&g_InputDevice->dev, MAX_I2C_TRANSACTION_LENGTH_LIMIT,
...
void DmaFree(void)
{
	if (I2CDMABuf_va != NULL) {
		if (g_InputDevice != NULL) {
			dma_free_coherent(&g_InputDevice->dev, MAX_I2C_TRANSACTION_LENGTH_LIMIT, I2CDMABuf_va,
					  I2CDMABuf_pa);
		}
		I2CDMABuf_va = NULL;
		I2CDMABuf_pa = 0;
...
void SetCfb(u8 Cfb)
{
	...
	case _50p:		/*Cfb = 50p */
		...
		RegMask16BitValue(0x1523, (u16) 0x0700, (u16) 0x0000, ADDRESS_MODE_16BIT);
		break;

	case _40p:		/*Cfb = 40p */
		RegMask16BitValue(0x1528, (u16) 0x0070, (u16) 0x0020, ADDRESS_MODE_16BIT);
		/*0x1523[10:8] = 0x1, Rfb: 225kohm */
		RegMask16BitValue(0x1523, (u16) 0x0700, (u16) 0x0100, ADDRESS_MODE_16BIT);
		break;

	case _30p:		/*Cfb = 30p */
		RegMask16BitValue(0x1528, (u16) 0x0070, (u16) 0x0040, ADDRESS_MODE_16BIT);
		/*0x1523[10:8] = 0x2, Rfb: 300kohm */
		RegMask16BitValue(0x1523, (u16) 0x0700, (u16) 0x0200, ADDRESS_MODE_16BIT);
		break;
...
s32 IicWriteData(u8 nSlaveId, u8 *pBuf, u16 nSize)
{
	...
	if (g_I2cClient != NULL) {
		if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA)
		    && nSlaveId == SLAVE_I2C_ID_DWI2C && (g_IsUpdateFirmware != 0 || g_IsBypassHotknot != 0)) {
			...
			while (nLeft > 0) {
				...
				if (nLeft > nSegmentLength) {
					if ((nRegAddr + nSegmentLength) < MAX_TOUCH_IC_REGISTER_BANK_SIZE) {
						...
						nLength = nSegmentLength;
						nLeft -= nSegmentLength;
					/*((nRegAddr + nSegmentLength) > MAX_TOUCH_IC_REGISTER_BANK_SIZE) */
					} else {
					...
					if ((nRegAddr + nLeft) < MAX_TOUCH_IC_REGISTER_BANK_SIZE) {
						...
						/*((nRegAddr + nLeft) > MAX_TOUCH_IC_REGISTER_BANK_SIZE) */
						nNextRegAddr = 0x00;
						/*shift to read data from next register bank */
						nNextRegBank = nRegBank + 1;
