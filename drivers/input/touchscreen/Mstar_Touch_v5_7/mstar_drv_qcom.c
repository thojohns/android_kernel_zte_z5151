					    ...
					    const struct i2c_device_id *id)
{
	...
	if (ret) {
		pr_err("Could not set to 2800mv.\n");
	}
...
static int __init touch_driver_init(void)
{
	...
	if (ret < 0) {
		pr_notice("add MStar touch device driver i2c driver failed.\n");
		return -ENODEV;
	}
	pr_notice("add MStar touch device driver i2c driver.\n");
...
{
	pr_notice("remove MStar touch device driver i2c driver.\n");
