...
int himax_gpio_power_config(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	E("HIMAX enter himax_gpio_power_config\n");
...
int himax_gpio_power_config(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
	...
	if (gpio_is_valid(pdata->gpio_id0)) {
		...
		error = gpio_request(pdata->gpio_id0, "himax_id0");
		if (error) {
			I("unable to request gpio [%d]\n",
						pdata->gpio_id0);
		...
		if (error) {
			I("unable to set direction for gpio [%d]\n",
				pdata->gpio_id0);
	...
	if (gpio_is_valid(pdata->gpio_id1)) {
		...
		error = gpio_request(pdata->gpio_id1, "himax_id1");
		if (error) {
			I("unable to request gpio [%d]\n",
						pdata->gpio_id1);
