...
extern unsigned long	CID_VER_MIN_FLASH_LENG;
	...
	extern unsigned char i_CTPM_FW0[];
...
int himax_switch_mode(struct i2c_client *client, int mode)
{
	...
	tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = mode_write_cmd; tmp_data[0] = mode_write_cmd;
	himax_flash_write_burst(client, tmp_addr, tmp_data);
	/*msleep(30);*/
	...
	if (mode) {
		...
		tmp_data[3] = 0x00; tmp_data[2] = 0x00; tmp_data[1] = 0x00; tmp_data[0] = 0x50;
		himax_flash_write_burst(client, tmp_addr, tmp_data);
	...
	I("mode_write_cmd(0)=0x%2.2X, mode_write_cmd(1)=0x%2.2X\n", tmp_data[0], tmp_data[1]);
	while (retry != 0) {
		I("[%d]Read 10007F04!\n", retry);
		tmp_addr[3] = 0x10; tmp_addr[2] = 0x00; tmp_addr[1] = 0x7F; tmp_addr[0] = 0x04;
		himax_register_read(client, tmp_addr, 4, tmp_data, false);
...
int himax_chip_self_test(struct i2c_client *client)
{
	...
	do {
		...
		/* 0x10007f18 -> 0x00006AA6   modified address to 100007F8 not for sure on 5464*/
		...
		tmp_data[3] = 0x00; tmp_data[2] = 0xff; tmp_data[1] = 0x0A; tmp_data[0] = 0x5A;
		...
		for (i = 0; i < 1000; i++) {
			tmp_addr[3] = 0x10; tmp_addr[2] = 0x00; tmp_addr[1] = 0x7f; tmp_addr[0] = 0x18;
			himax_register_read(client, tmp_addr, 4, tmp_data, false);
			I("%s: tmp_data[0] = 0x%02X, tmp_data[1] = 0x%02X", __func__, tmp_data[0], tmp_data[1]);
			I("tmp_data[2] = 0x%02X, tmp_data[3] = 0x%02X, cnt=%d\n", tmp_data[2], tmp_data[3], i);
			msleep(20);
		...
		himax_sense_on(client, 1);
		msleep(120);
		/*we have get the test result from register and save it in self_test_info[0],
		now, check it : AA means pass, DD means fail*/
		if (test_result_id == 0xAA) {
				pf_value = 0x1;/*pass ,set pf_value 0x1*/
				break;
		} else {
				cnt++;
				I("[Himax]:ST:%d times,%x.\n", cnt, test_result_id);
				pf_value = 0x0;/*fail ,set pf_value 0x0*/
		}
	} while (cnt < 5);
	if (pf_value == 0x1) {
...
himax_sense_off(client);
	...
	do {
		...
		himax_register_read(client, tmp_addr, 4, tmp_data, false);
		I("%s: set_SMWP_enable tmp_data[0]=%d, tmp_data[1]=%d", __func__, tmp_data[0], tmp_data[1]);
		I("tmp_data[2]=%d, tmp_data[3]=%d", tmp_data[2], tmp_data[3]);
		I("SMWP_enable=%d, retry_cnt=%d\n", SMWP_enable, retry_cnt);
		retry_cnt++;
...
int himax_register_read(struct i2c_client *client,
...
void himax_sram_write(struct i2c_client *client, uint8_t *FW_content)
{
	...
	for (i = 0; i < FW_length; i = i + 128) {
		...
		himax_flash_write_burst_length(client, tmp_addr, tmp_data, 128);
	}

	if (!wait_wip(client, 100))
		E("%s:83112_Sram_Write Fail\n", __func__);
}
...
bool himax_sram_verify(struct i2c_client *client, uint8_t *FW_File, int FW_Size)
{
	...
	for (i = 0; i < 0x4000; i = i + 128) {
		...
		if (i2c_himax_write(client, 0x00, out_buffer, 4, HIMAX_REG_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return false;
		}

		out_buffer[0] = 0x00;
		if (i2c_himax_write(client, 0x0C, out_buffer, 1, HIMAX_REG_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return false;
		}

		if (i2c_himax_read(client, 0x08, in_buffer, 128, HIMAX_REG_RETRY_TIMES) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return false;
		}
...
void himax_flash_page_write(struct i2c_client *client, uint8_t *write_addr, uint8_t *write_data)
{
	...
	for (page_prog_start = start_address;
		page_prog_start < 36 + start_address;
		...
		for (i = page_prog_start - start_address, j = 0;
		i < 16 + page_prog_start - start_address;
		i++, j++) {   /*<------ bin file*/
		...
		for (j = 0; j < 5; j++) {
			if (i < 36)
				for (i = (page_prog_start - start_address + 16 + (j * 48)), k = 0;
					i < 48 && i < (page_prog_start - start_address + 16 + (j * 48)) +
						program_length;
					i++, k++) {	/*<------ bin file*/
...
void himax_touch_information(struct i2c_client *client)
{
	...
	if (IC_TYPE == HX_83112A_SERIES_PWON) {
		cmd[3] = 0x10; cmd[2] = 0x00; cmd[1] = 0x070; cmd[0] = 0xF4;
		...
		cmd[0] = 0xE8;
		himax_register_read(client, cmd, 4, data, false);
		/*I("%s: data[0] = 0x%02X,data[1] = 0x%02X,data[2] = 0x%02X,
		...
		if (g_vendor_id == HX_VENDOR_ID_DIJING)
			ic_data->HX_RX_NUM				= FIX_HX_RX_NUM_DIJING;
		else if (g_vendor_id == HX_VENDOR_ID_LEAD  ||  g_vendor_id == HX_VENDOR_ID_XINLI)
			ic_data->HX_RX_NUM				= FIX_HX_RX_NUM_LEAD;
	#endif
#endif
	if (g_vendor_id == HX_VENDOR_ID_0)
			ic_data->HX_RX_NUM				= HX_FIX_RX_NUM_0;
	else if (g_vendor_id == HX_VENDOR_ID_1)
			ic_data->HX_RX_NUM				= HX_FIX_RX_NUM_1;
	else if (g_vendor_id == HX_VENDOR_ID_2)
			ic_data->HX_RX_NUM				= HX_FIX_RX_NUM_2;
	else if (g_vendor_id == HX_VENDOR_ID_3)
			ic_data->HX_RX_NUM				= HX_FIX_RX_NUM_3;
...
void himax_read_FW_ver(struct i2c_client *client)
{
	...
	if (update_flag == 1) {
		I("[HXTP]:firmware update is going and return.\n");
		return;
	}
	while (reload_status == 0) {
		...
		cmd[0] = 0x00;
		himax_register_read(client, cmd, 4, data, false);
		I("7f10,data[0]=%d,data[1]=%d,data[2]=%d,data[3]=%d\n", data[0], data[1], data[2], data[3]);
		cmd[3] = 0x10;
		cmd[2] = 0x00;
		cmd[1] = 0x72;
		cmd[0] = 0xC0;
		himax_register_read(client, cmd, 4, data_1, false);
		I("72C0,data_1[0]=%d,data_1[1]=%d,data_1[2]=%d,data[3]_1=%d\n",
		data_1[0], data_1[1], data_1[2], data_1[3]);
		if ((data[3] == 0x00 && data[2] == 0x00 && data[1] == 0x3A && data[0] == 0xA3) ||
		(data_1[3] == 0x00 && data_1[2] == 0x00 && data_1[1] == 0x72 && data_1[0] == 0xC0)) {
			I("reload OK!\n");
			reload_status = 1;
			/*break;*/
		} else {
	...
	himax_sense_off(client); /* 20180117 modify for no touch while booting open again*/
...
bool himax_ic_package_check(struct i2c_client *client)
{
	...
	for (i = 0; i < 3; i++) {
		...
		&& ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2b))) {
			...
			FW_VER_MAJ_FLASH_ADDR   = 49157;  /*0xC005*/
			FW_VER_MAJ_FLASH_LENG   = 1;
			FW_VER_MIN_FLASH_ADDR   = 49158;  /*0xC006*/
			...
			if (g_vendor_id == HX_VENDOR_ID_BOE) {
			I("Himax IC boe\n");
			g_i_FW_VER = i_CTPM_FW0[FW_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW0[FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER = i_CTPM_FW0[CFG_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW0[CFG_VER_MIN_FLASH_ADDR];
			g_i_CID_MAJ = i_CTPM_FW0[CID_VER_MAJ_FLASH_ADDR];
			g_i_CID_MIN = i_CTPM_FW0[CID_VER_MIN_FLASH_ADDR];
			} else {
			g_i_FW_VER = i_CTPM_FW0[FW_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW0[FW_VER_MIN_FLASH_ADDR];
			g_i_CFG_VER = i_CTPM_FW0[CFG_VER_MAJ_FLASH_ADDR] << 8 | i_CTPM_FW0[CFG_VER_MIN_FLASH_ADDR];
			g_i_CID_MAJ = i_CTPM_FW0[CID_VER_MAJ_FLASH_ADDR];
			...
			I("%s:fw version = %X,%X,%d\n", __func__, g_i_FW_VER, g_i_CFG_VER, g_vendor_id);
			#endif
#endif
			if (g_vendor_id == HX_VENDOR_ID_0) {
			I("Himax IC dijing\n");
...
void himax_get_DSRAM_data(struct i2c_client *client, uint8_t *info_data)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_i2c_size) {
			himax_register_read(client, tmp_addr, max_i2c_size,
...
void diag_parse_raw_data(struct himax_report_data *hx_touch_data,
	int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data,
	int32_t *self_data)
{
	int RawDataLen_word;
	int index = 0;
	...
	&& hx_touch_data->hx_rawdata_buf[3] == diag_cmd) {
		...
		for (i = 0; i < RawDataLen_word; i++) {
			...
			if (temp1 < mul_num) { /*mutual*/
				mutual_data[index + i] = hx_touch_data->hx_rawdata_buf[i*2 + 4 + 1]*256
...
uint8_t himax_read_DD_status(uint8_t *cmd_set, uint8_t *tmp_data)
{
	...
	for (cnt = 0; cnt < 100; cnt++) { /*doing hand shaking 0xAA -> 0xBB*/
		...
		/*I("%s: tmp_data[0] = 0x%02X, tmp_data[1] = 0x%02X,
			tmp_data[2] = 0x%02X, tmp_data[3] = 0x%02X, cnt=%d\n",
...
int himax_read_FW_status(uint8_t *state_addr, uint8_t *tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		state_addr[1] = 0x04;
		state_addr[2] = status_addr[0]; state_addr[3] = status_addr[1];
		state_addr[4] = status_addr[2]; state_addr[5] = status_addr[3];
		req_size = 0x04;
		...
		state_addr[1] = 0x30;
		state_addr[2] = cmd_addr[0]; state_addr[3] = cmd_addr[1];
		state_addr[4] = cmd_addr[2]; state_addr[5] = cmd_addr[3];
		req_size = 0x30;
