	...
	size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG_READ) {
		...
		if (g_vendor_id == HX_VENDOR_ID_0)
			ret += snprintf(temp_buf, 100,
			"Himax-tp\nChip type:83112a\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
			ic_data->vendor_fw_ver, ic_data->vendor_config_ver, HX_VENDOR_0_NAME);
			else if (g_vendor_id == HX_VENDOR_ID_1)
				ret += snprintf(temp_buf, 100,
			"Himax-tp\nChip type:83112a\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
			ic_data->vendor_fw_ver, ic_data->vendor_config_ver, HX_VENDOR_1_NAME);
			else if (g_vendor_id == HX_VENDOR_ID_2)
				ret += snprintf(temp_buf, 100,
			"Himax-tp\nChip type:83112a\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
			ic_data->vendor_fw_ver, ic_data->vendor_config_ver, HX_VENDOR_2_NAME);
			else if (g_vendor_id == HX_VENDOR_ID_3)
				ret += snprintf(temp_buf, 100,
			"Himax-tp\nChip type:83112a\nFW version:%#x\nCfg version = 0x%2.2X\nVerdor:%s\n",
			ic_data->vendor_fw_ver, ic_data->vendor_config_ver, HX_VENDOR_3_NAME);
...
/* end */
#ifdef HX_USB_DETECT_GLOBAL
static ssize_t himax_charger_read(struct file *file, char *buf,
	size_t len, loff_t *pos)
{
	/*struct himax_ts_data *ts = private_ts;*/
	size_t count = 0;
	...
	if (!HX_PROC_SEND_FLAG) {
		count += snprintf(temp_buf + count, 10, "%d ", USB_detect_flag);
...
static ssize_t himax_charger_write(struct file *file, const char *buff,
	size_t len, loff_t *pos)
{
	/*struct himax_ts_data *ts = private_ts;*/
	char buf_tmp[12] = {0};
...
static struct file_operations himax_proc_charger_ops = {
	.owner = THIS_MODULE,
	.read = himax_charger_read,
	.write = himax_charger_write,
};
#endif
	...
	size_t len, loff_t *pos)
{
	...
	if (val == 0x01) {
		ret += snprintf(temp_buf + ret, 32, "Self_Test Pass\n");
	} else {
		ret += snprintf(temp_buf + ret, 32, "Self_Test Fail\n");
	}
...
#ifdef HX_USB_DETECT_GLOBAL
	himax_proc_charger_file = proc_create(HIMAX_PROC_CHARGER_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
		himax_touch_touch_info_proc_dir, &himax_proc_charger_ops);
	if (himax_proc_charger_file == NULL) {
		E(" %s: proc usb state  file create failed!\n", __func__);
	}
#endif
#ifdef HX_SMART_WAKEUP
	himax_proc_SMWP_file = proc_create(HIMAX_PROC_SMWP_FILE, (S_IWUSR|S_IRUGO|S_IWUGO),
