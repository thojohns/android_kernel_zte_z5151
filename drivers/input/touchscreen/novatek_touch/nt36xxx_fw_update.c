 ...
 *
 * Revision: 20544
 * Date: 2017-12-20 11:08:15 +0800 (2017)
 *
...
int32_t Erase_Flash(void)
{
	...
	while (1) {
		...
		if (unlikely(retry > 100)) {
			NVT_ERR("Check 0xAA (Read Status for Write Status Register) failed");
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	case NVT_VENDOR_ID_0:
		snprintf(firmware_name, sizeof(firmware_name), NVT_UPGRADE_FW0_APP);
		break;
	case NVT_VENDOR_ID_1:
		snprintf(firmware_name, sizeof(firmware_name), NVT_UPGRADE_FW1_APP);
		break;
	case NVT_VENDOR_ID_2:
		snprintf(firmware_name, sizeof(firmware_name), NVT_UPGRADE_FW2_APP);
		break;
	case NVT_VENDOR_ID_3:
		snprintf(firmware_name, sizeof(firmware_name), NVT_UPGRADE_FW3_APP);
		break;
	default:
		snprintf(firmware_name, sizeof(firmware_name), NVT_UPGRADE_FW0_APP);
		break;
...
static ssize_t nvt_force_fwupdate_store(struct device *dev,
							...
							const char *buf, size_t count)
{
	...
	syna_update_flag = 1;*/
	if (update_firmware_request(fwname) == 0) {
		NVT_LOG("update success\n");
	} else {
		NVT_LOG("update fail\n");
	}
...
static DEVICE_ATTR(nvtfwupdate_force, S_IRUGO | S_IWUSR, NULL, nvt_force_fwupdate_store);
int nvt_fwupdate_init(struct i2c_client *client)
{
	int ret;
	...
	if (fts_fw_kobj == NULL) {
		...
		if (fts_fw_kobj == NULL) {
			NVT_LOG("%s: subsystem_register failed\n", __func__);
			ret = -ENOMEM;
			return ret;
		}
	}

	ret = sysfs_create_file(fts_fw_kobj, &dev_attr_nvtfwupdate_force.attr);
	if (ret) {
		NVT_LOG("sysfs_create_file failed\n");
		return ret;
	}

	NVT_LOG("nvt firmware update init succeed!\n");
	return 0;

}


int nvt_fwupdate_deinit(struct i2c_client *client)
{
	struct kobject *fts_fw_kobj = NULL;

	fts_fw_kobj = kobject_get(firmware_kobj);
	if (!firmware_kobj) {
		NVT_LOG("error get kobject\n");
		return -EINVAL;
	}

	sysfs_remove_file(firmware_kobj, &dev_attr_nvtfwupdate_force.attr);
