 ...
 *
 * Revision: 21288
 * Date: 2018-01-05 11:38:47 +0800 (2018)
 *
...
static int nvt_mp_buffer_init(void)
{
	...
	if (!RecordResult_Short) {
		...
		retval = -ENOMEM;
		goto err_recordresult_short;
	}

	RecordResult_Short_Diff = RecordResult_Short;

	RecordResult_Short_Base = kzalloc(RecordResult_BufSize, GFP_KERNEL);
	if (!RecordResult_Short_Base) {
		NVT_ERR("kzalloc for RecordResult_Short_Base failed!\n");
		retval = -ENOMEM;
		goto err_recordresult_short_base;
	}
	...
	if (!RecordResult_FWMutual) {
		...
		retval = -ENOMEM;
		goto err_recordresult_fwmutual;
	}
	...
	if (!RecordResult_FW_CC) {
		...
		retval = -ENOMEM;
		goto err_recordresult_fw_cc;
	}

	RecordResult_FW_CC_I = RecordResult_FW_CC;

	RecordResult_FW_CC_Q = kzalloc(RecordResult_BufSize, GFP_KERNEL);
	if (!RecordResult_FW_CC_Q) {
		NVT_ERR("kzalloc for RecordResult_FW_CC_Q failed!\n");
		retval = -ENOMEM;
		goto err_recordresult_fw_cc_q;
	}
	...
	if (!RecordResult_FW_DiffMax) {
		...
		retval = -ENOMEM;
		goto err_recordresult_fw_diffmax;
	}
	...
	if (!RecordResult_FW_DiffMin) {
		...
		retval = -ENOMEM;
		goto err_recordresult_fw_diffmin;
	}
...
err_rawdata_short_base:
	kfree(RawData_Short_Base);

err_rawdata_short:
	kfree(RawData_Short);

err_recordresult_fw_diffmin:
	kfree(RecordResult_FW_DiffMin);

err_recordresult_fw_diffmax:
	kfree(RecordResult_FW_DiffMax);

err_recordresult_fw_cc_q:
	kfree(RecordResult_FW_CC_Q);

err_recordresult_fw_cc:
	kfree(RecordResult_FW_CC);

err_recordresult_fwmutual:
	kfree(RecordResult_FWMutual);

err_recordresult_open:
	kfree(RecordResult_Open);

err_recordresult_short_base:
	kfree(RecordResult_Short_Base);

err_recordresult_short:
	kfree(RecordResult_Short);

	return retval;
}

static void parse_array_buf_print(uint32_t *array, int32_t len)
{
	char *buf_tmp = NULL;
	int i = 0;
	int count = 0;

	buf_tmp = kzalloc(len * NVT_PRE_PARAM_SIZE, GFP_KERNEL);
	if (!buf_tmp) {
		NVT_ERR("kzalloc failed!\n");
		return;
	}
	for (i = 0; i < len; i++) {
		count += snprintf(buf_tmp + count, NVT_PRE_PARAM_SIZE, "%d ", array[i]);
	}
...
static void nvt_print_lmt_array(int32_t *array, int32_t x_ch, int32_t y_ch)
{
	...
	for (j = 0; j < y_ch; j++) {
		parse_array_buf_print(array + j * x_ch, x_ch);
	}
#if TOUCH_KEY_NUM > 0
	parse_array_buf_print(array + j * x_ch, Key_Channel);
#endif /* #if TOUCH_KEY_NUM > 0 */
}
...
static int32_t nvt_save_rawdata_to_csv(int32_t *rawdata, uint8_t x_ch,
	uint8_t y_ch, const char *file_name, uint32_t offset)
{
	int32_t x = 0;
	int32_t y = 0;
	...
	snprintf(file_path, sizeof(file_path), "%s%s", g_str_save_file_path, file_name);
	NVT_LOG("file_path %s\n", file_path);
...
#endif /* #if TOUCH_KEY_NUM > 0 */
		pr_notice("%s:RawData_CC_Q:\n", __func__);
		if (nvt_save_rawdata_to_csv(RawData_FW_CC_Q, X_Channel, Y_Channel,
			FW_CC_CSV_FILE, rawdata_cc_q_offset) < 0) {
			NVT_ERR("save rawdata to CSV file failed\n");
			return -EAGAIN;
		}
	} else {
...
static int32_t nvt_read_fw_short(int32_t *xdata)
{
	...
	for (y = 0; y < Y_Channel; y++) {
		for (x = 0; x < X_Channel; x++) {
			...
			xdata[iArrayIndex] = (int16_t)(rawdata_buf[iArrayIndex * 2] +
	...
	if (ts->carrier_system) {
		...
		for (y = 0; y < Y_Channel; y++) {
			for (x = 0; x < X_Channel; x++) {
				iArrayIndex = y * X_Channel + x;
				RawData_Short_Base[iArrayIndex] = (int16_t)(rawdata_buf[iArrayIndex * 2] +
					256 * rawdata_buf[iArrayIndex * 2 + 1]);
			}
		}
#if TOUCH_KEY_NUM > 0
	...
	if (ts->carrier_system)
		pr_notice("%s:RawData_Short_Diff:\n", __func__);
	else
		pr_notice("%s:RawData_Short\n", __func__);
...
#endif /* #if TOUCH_KEY_NUM > 0 */
		pr_notice("%s:RawData_Short_Base:\n", __func__);
...
*******************************************************/
static int32_t RawDataTest_SinglePoint_Sub(int32_t rawdata[], uint8_t RecordResult[],
	uint8_t x_ch, uint8_t y_ch, int32_t Rawdata_Limit_Postive[], int32_t Rawdata_Limit_Negative[])
{
	int32_t i = 0;
	int32_t j = 0;
#if TOUCH_KEY_NUM > 0
...
static void result_buf_print(struct seq_file *m, uint8_t *RecordResult, uint8_t x_len, uint8_t y_len)
{
	char *buf_tmp = NULL;
	int i = 0;
	int j = 0;
...
#endif
	int count = 0;
	int buf_size = x_len * NVT_PRE_PARAM_SIZE;
	int32_t iArrayIndex = 0;
	...
	for (i = 0; i < y_len; i++) {
		for (j = 0; j < x_len; j++) {
			...
			count += snprintf(buf_tmp + count, NVT_PRE_PARAM_SIZE, "%02d ", RecordResult[iArrayIndex]);
		}
...
static void rawdata_buf_print(struct seq_file *m, int32_t *rawdata, uint8_t x_len, uint8_t y_len)
{
	char *buf_tmp = NULL;
	int i = 0;
	int j = 0;
	...
	for (i = 0; i < y_len; i++) {
		for (j = 0; j < x_len; j++) {
			iArrayIndex = i * x_len + j;
			count += snprintf(buf_tmp + count, NVT_PRE_PARAM_SIZE, "%05d ", rawdata[iArrayIndex]);
		}
		nvt_mp_seq_printf(m, "%s\n", buf_tmp);
		count = 0;
	}
	...
	uint8_t RecordResult[], int32_t rawdata[], uint8_t x_len, uint8_t y_len)
{
	...
	case -1:
		...
		result_buf_print(m, RecordResult, x_len, y_len);
		nvt_mp_seq_printf(m, "ReadData:\n");
		rawdata_buf_print(m, rawdata, x_len, y_len);
		break;
	default:
		nvt_mp_seq_printf(m, " default!TestResult=%d\n", TestResult);
	}
	nvt_mp_seq_printf(m, "\n");
}

bool NTV_RAWDATA_TEST_VALIDITY = false;
/*******************************************************
Description:
...
static int32_t c_show_selftest(struct seq_file *m, void *v)
{
	...
	if ((TestResult_Short == 0) || (TestResult_Short == 1)) {
		...
		if (ts->carrier_system) {
			nvt_mp_seq_printf(m, "Failed\n");
			test_result = -1;
			if (TestResult_Short_Diff == -1) {
				nvt_mp_seq_printf(m, "Short Diff");
				print_selftest_result(m, TestResult_Short_Diff, RecordResult_Short_Diff,
					RawData_Short_Diff, X_Channel, Y_Channel);
			}
			if (TestResult_Short_Base == -1) {
				nvt_mp_seq_printf(m, "Short Base");
				print_selftest_result(m, TestResult_Short_Base, RecordResult_Short_Base,
					RawData_Short_Base, X_Channel, Y_Channel);
			}
		} else {
		...
		if (TestResult_FW_CC == -1) {
			if (ts->carrier_system) {
				if (TestResult_FW_CC_I == -1) {
					...
					print_selftest_result(m, TestResult_FW_CC_I, RecordResult_FW_CC_I,
						RawData_FW_CC_I, X_Channel, Y_Channel);
				}
				if (TestResult_FW_CC_Q == -1) {
					nvt_mp_seq_printf(m, "FW CC_Q");
					print_selftest_result(m, TestResult_FW_CC_Q, RecordResult_FW_CC_Q,
	...
	if (NTV_RAWDATA_TEST_VALIDITY) {
		seq_printf(m, "%d, %d, %d\n", test_result, X_Channel, Y_Channel);
	} else {
		seq_printf(m, "%d, %d, %d\n", 0, X_Channel, Y_Channel);
	}
	NVT_LOG("--\n");
...
static int32_t nvt_selftest_open(struct inode *inode, struct file *file)
{
	...
	NTV_RAWDATA_TEST_VALIDITY = false;
	...
	if (nvt_read_baseline(RawData_FWMutual) != 0) {
		...
		TestResult_FWMutual = RawDataTest_SinglePoint_Sub(RawData_FWMutual, RecordResult_FWMutual, X_Channel,
						Y_Channel, PS_Config_Lmt_FW_Rawdata_P, PS_Config_Lmt_FW_Rawdata_N);
	}
	...
	if (nvt_read_CC(RawData_FW_CC) != 0) {
		...
		if (ts->carrier_system) {
			TestResult_FW_CC_I = RawDataTest_SinglePoint_Sub(RawData_FW_CC_I, RecordResult_FW_CC_I,
					X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_I_P, PS_Config_Lmt_FW_CC_I_N);
			TestResult_FW_CC_Q = RawDataTest_SinglePoint_Sub(RawData_FW_CC_Q, RecordResult_FW_CC_Q,
					X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_Q_P, PS_Config_Lmt_FW_CC_Q_N);
			if ((TestResult_FW_CC_I == -1) || (TestResult_FW_CC_Q == -1))
				TestResult_FW_CC = -1;
			else
				TestResult_FW_CC = 0;
		} else {
			TestResult_FW_CC = RawDataTest_SinglePoint_Sub(RawData_FW_CC, RecordResult_FW_CC,
					X_Channel, Y_Channel, PS_Config_Lmt_FW_CC_P, PS_Config_Lmt_FW_CC_N);
		}
	}
	...
	if (nvt_read_fw_noise(RawData_Diff) != 0) {
		...
		TestResult_FW_DiffMax = RawDataTest_SinglePoint_Sub(RawData_Diff_Max, RecordResult_FW_DiffMax,
			X_Channel, Y_Channel, PS_Config_Lmt_FW_Diff_P, PS_Config_Lmt_FW_Diff_N);
	...
	if (nvt_read_fw_short(RawData_Short) != 0) {
		...
		if (ts->carrier_system) {
			TestResult_Short_Diff = RawDataTest_SinglePoint_Sub(RawData_Short_Diff,
				RecordResult_Short_Diff, X_Channel, Y_Channel, PS_Config_Lmt_Short_Diff_P,
				PS_Config_Lmt_Short_Diff_N);
			TestResult_Short_Base = RawDataTest_SinglePoint_Sub(RawData_Short_Base,
				RecordResult_Short_Base, X_Channel, Y_Channel, PS_Config_Lmt_Short_Base_P,
			...
			TestResult_Short = RawDataTest_SinglePoint_Sub(RawData_Short, RecordResult_Short,
	...
	if (nvt_read_fw_open(RawData_Open) != 0) {
		...
		TestResult_Open = RawDataTest_SinglePoint_Sub(RawData_Open, RecordResult_Open,
				X_Channel, Y_Channel, PS_Config_Lmt_Open_Rawdata_P, PS_Config_Lmt_Open_Rawdata_N);
	}
...
#ifdef CONFIG_OF
#if NVT_DEBUG
static void parse_buf_print(uint8_t *array, int32_t len)
{
	char *buf_tmp = NULL;
	int i = 0;
	int count = 0;
	...
	for (i = 0; i < len; i++) {
		count += snprintf(buf_tmp + count, NVT_PRE_PARAM_SIZE, "%02d ", array[i]);
	}
		...
		int32_t size)
{
	...
	if ((!data) || (!len) || (len < size)) {
		...
		for (j = 0; j < Y_Channel; j++) {
			parse_array_buf_print(array + j * X_Channel, X_Channel);
		}
#if TOUCH_KEY_NUM > 0
		parse_array_buf_print(array + j * X_Channel, Key_Channel);
#endif
#endif
	}
}
...
void nvt_mp_parse_dt(struct device_node *root, const char *node_compatible)
{
	...
	NTV_RAWDATA_TEST_VALIDITY = true;
					 ...
					 const char *buf)
{
	...
	NVT_LOG("save file path:%s\n", g_str_save_file_path);

	return 0;
}
int nvt_mp_init(void)
{
	int err = 0;

	NVT_LOG("\n");

	strlcpy(g_str_save_file_path, TEST_CSV_FILE_DIR, 256);
